	...
	size_t right_length)
{
	...
	for (size_t index = 0U; index < left_length; index++) {
		if (ascii_fold(left[index]) != ascii_fold((unsigned char)right[index]))
			return 0;
	}
	...
	const unsigned char *entry)
{
	...
	for (unsigned index = 0U; index < 11U; index++) {
		result = (uint8_t)(((result & 1U) != 0U ? 0x80U : 0U) + (result >> 1U) + entry[index]);
	}
	...
	const unsigned char *entry)
{
	...
	for (unsigned index = 0U; index < FAT_LFN_CHARACTERS; index++) {
		size_t position = (size_t)(ordinal - 1U) * FAT_LFN_CHARACTERS + index;
		...
		if (position >= sizeof(state->long_name) ||
		    value < 0x21U ||
		    value > 0x7eU ||
		    (state->length_known && position >= state->long_name_length)) {
			state->invalid = 1U;
			continue;
		}
...
static int
long_name_matches(
	const struct zbl_bios_fat_directory_state *state,
	const unsigned char *entry,
	...
	size_t component_length)
{
	...
	return ascii_equal(state->long_name,
			   state->long_name_length,
			   component,
			   component_length);
}
...
int
zbl_bios_fat_directory_search(
	struct zbl_bios_fat_directory_state *state,
	const void *sector_pointer,
	size_t sector_size,
	const char *component,
	size_t component_length)
{
	...
	if (state == NULL ||
	    ...
	    component == NULL ||
	    component_length == 0U ||
	    component_length > 255U ||
	    sector_size == 0U ||
	    sector_size % FAT_ENTRY_SIZE != 0U)
		return -2;

	for (size_t offset = 0U; offset < sector_size; offset += FAT_ENTRY_SIZE) {
