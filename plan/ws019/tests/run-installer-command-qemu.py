...
try:
    ...
    canonical = json.loads((REPO / 'plan/ws019/temp/q133-staging/result.json').read_text())
    assert canonical['result'] == 'PASS staging'
    assert canonical['data.img_sha256'] == f['digest'](REPO / 'plan/ws019/temp/q133-staging/data.img')
    assert canonical['swapfile_sha256'] == f['digest'](REPO / 'plan/ws019/temp/q133-staging/swapfile')
