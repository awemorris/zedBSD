...
def format_cell(guest):
    ...
    guest.run("mkfs -t ufs /q078/empty", status=1)
    guest.run("mkswap /q078/short", status=1)
    guest.run("ln -s /q078/data.img /q078-link")
    guest.run("mkfs -t ufs /q078-link", status=1)
