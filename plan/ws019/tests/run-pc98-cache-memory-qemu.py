...
r=Path(__file__).resolve().parents[3];s=importlib.util.spec_from_file_location('pc',r/'plan/ws025/tests/run-p032-pc98-session.py');m=importlib.util.module_from_spec(s);s.loader.exec_module(m)
p=Path(sys.argv[1]).resolve();p.relative_to(r/'plan/ws019/temp');p.mkdir(parents=True)
...
subprocess.run(['mcopy','-i',str(p/'source.img')+'@@1048576',str(Path(sys.argv[2]).resolve()) if len(sys.argv)>2 else str(r/'plan/ws019/tests/pc98-cache-memory.noct'),'::check.nct'],check=True)
