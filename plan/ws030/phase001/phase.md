<!-- awesome-plan project=zedbsd record=ws030-p001 -->

# WS030 p001: 公開header・共有library・dispatch

<!-- awesome-plan-current:start -->
Status: cleared
Phase disposition: normal
Parent: [WS030](https://github.com/awemorris/zedBSD/issues/388)
Last Queue: q308 / q308-i01 cleared
Active Queue: none
Acceptance: q308-lifecycle-003 and final API/ABI/ownership/style evidence
<!-- awesome-plan-current:end -->

Combined ID: ws030-p001
Primary Milestone: MG006

## 目標・手順・受け入れ

公開header、ABI宣言、libvulkan.soのbuild/install、global/instance/device dispatchを提供する。固定したKhronos由来headerから選択する宣言とimplementationの境界を明記し、再生成と集合照合を可能にする。

手順: 全137coreと対象WSIの宣言集合を固定→型・enum・callbackとABIを生成→既存PIC/libc/rtldへ統合→version/extension/proc-address/enumerationとallocator/handle基盤を実装。header/sysrootとDT_SONAME/DT_NEEDED、公開exportに内部helperが出ないことを確認する。

受け入れ: 標準headerだけをincludeする独立clientがcompile/linkでき、137coreと採用WSI宣言に欠落・重複・ABI不一致がない。実装済み入口と未実装範囲を明示し、symbolの存在だけで全Vulkan対応を宣言しない。依存するmemory/WSIの完成はp002/p003が担う。
