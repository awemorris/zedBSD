<!-- awesome-plan project=zedbsd record=ws030-p003 -->

# WS030 p003: 全Vulkan1.0 core・描画とdirect-display WSI

<!-- awesome-plan-current:start -->
Status: cleared
Phase disposition: normal
Parent: [WS030](https://github.com/awemorris/zedBSD/issues/388)
Last Queue: q308 / q308-i03 cleared
Active Queue: none
Acceptance: q308-lifecycle-003 and final API/ABI/ownership/style evidence
<!-- awesome-plan-current:end -->

Combined ID: ws030-p003
Primary Milestone: MG006

## 目標・手順・受け入れ

残る全core関数の意味論とgraphics/compute、command/query、direct-display WSIを実装し、標準アプリに必要な完成したlibrary出力を供給する。

手順: shader/pipeline/descriptor/renderpass/framebuffer、graphicsとcompute、各cmd/copy/blit/clear/barrier/query、primary/secondary commandと再利用を実装。必須features/limits/formatsと任意feature宣言を実装能力で照合する。KHR_surface/display/swapchain/display_swapchainの適用関数・revision・依存を固定し、display/plane/mode、surface、swapchain、acquire/submit/presentと回収を実装する。

受け入れ: 137関数を単なるstubで埋めず、仕様上合法な入力・結果を機能群ごとに確認する。FIFOの進行・順序・tearing制約、acquire通知とimage再利用、複数sessionの表示所有権、normal/failed recreationを検証する。queue受付やsleepで不足を覆わない。WS014 p005へ標準headerと共有library、実際に使えるgraphics/memory/sync/direct-display出力を渡す。
