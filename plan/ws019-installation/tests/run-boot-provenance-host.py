...
out=Path(sys.argv[1]).resolve();out.relative_to(repo/'plan/ws019-installation/temp');out.mkdir(parents=True,exist_ok=False)
sources=['src/kern/boot.c', 'bootloader/uefi/volume-discovery.c', 'src/hal/amd64/bsp-pcat/handoff-validation.c', 'plan/ws019-installation/tests/boot-provenance-host.c']
