...
out=$(mktemp -d "${TMPDIR:-/tmp}/zedbsd-tmpfs-unmount.XXXXXX")
...
for mode in ordinary sanitize; do
    ...
    ${HOSTCC:-cc} -O1 -g -pthread $extra -c \
        "$repo/plan/ws018-kernel-architecture/tests/mount-thread-host.c" -o "$out/thread.o"
    ${HOSTCC:-cc} -std=c11 -O1 -g -Wall -Wextra -Werror \
        ...
        "$repo/plan/ws019-installation/tests/tmpfs-unmount-host.c" \
        "$repo/src/kern/tmpfs.c" "$repo/src/kern/mount.c" "$repo/src/kern/inode.c" "$repo/src/kern/namei.c" \
        "$repo/src/kern/namecache.c" "$repo/src/kern/cwdinfo.c" "$repo/src/kern/acl.c" "$out/thread.o" \
