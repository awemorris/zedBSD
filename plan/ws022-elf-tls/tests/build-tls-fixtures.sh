...
for arch in amd64 i386; do
	...
	"$repo/build/llvm/bin/clang" --target="$triple" -ffreestanding -nostdinc -I"$repo/libc/include" -I"$repo/include/uapi" \
		...
		"$repo/plan/ws022-elf-tls/tests/tls-image.c" -o "$output/$arch.o"
	...
	"$repo/build/llvm/bin/ld.lld" -static \
		-T "$repo/plan/ws022-elf-tls/tests/tls-image.ld" \
	...
	for kind in empty zero; do
		...
		"$repo/build/llvm/bin/clang" --target="$triple" -ffreestanding -nostdinc \
			...
			"$repo/plan/ws022-elf-tls/tests/tls-empty-image.c" -o "$output/$arch-$kind.o"
		"$repo/build/llvm/bin/ld.lld" -static \
			-T "$repo/plan/ws022-elf-tls/tests/tls-image.ld" \
			"$output/$arch-$kind.o" "$output/$arch-syscall.o" -o "$output/$arch-$kind.elf"
	done
	sed 's/ .tdata :/ . += 4; .tdata :/' "$repo/plan/ws022-elf-tls/tests/tls-image.ld" > "$output/offset.ld"
...
python3 "$repo/plan/ws022-elf-tls/tests/mutate-tls-fixtures.py" "$output"
