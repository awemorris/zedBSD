<!-- awesome-plan project=zedbsd record=ws027-p005 -->

# ws027-p005: PPCユーザーABI・libcとinit到達
...
PowerPC syscall/return、ユーザーモード・例外、ELF実行、stack/TLS、libcのsyscall/context/setjmp等、image内のendiannessを監査し、初期init/シェルに必要な部分を実装する。ディスクに依存しないRAM rootの実装可否を先に確認し、追加が大きければp004後のUSB root検証と結合する判断を記録する。
...
[ws027-p003](https://github.com/awemorris/zedBSD/issues/377)
...
[ws003-p037](https://github.com/awemorris/zedBSD/issues/371)から計画を移管。未実行のままであり、旧IDの再利用・試験結果の新規追加ではない。
