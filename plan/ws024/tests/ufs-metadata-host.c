...
#else
#include "../../ws025/temp/p031-driver-fragments/src/drivers/fs/ufs/ufs-vfs.c"
#endif
...
#define AUDIT_CG_MAGIC UFS_CG_MAGIC
#define AUDIT_CG_MAGIC_VALUE UFS_CG_MAGIC_VALUE
#define AUDIT_CG_CGX UFS_CG_CGX
#define AUDIT_CG_NDBLK UFS_CG_NDBLK
#define AUDIT_CG_IUSEDOFF UFS_CG_IUSEDOFF
#define AUDIT_CG_FREEOFF UFS_CG_FREEOFF
#define AUDIT_CG_NEXTFREEOFF UFS_CG_NEXTFREEOFF
#define AUDIT_CG_NBFREE UFS_CG_NBFREE
...
static unsigned storage_reads, failure_read;
...
#ifndef UFS_AUDIT_CUSTOM_ALLOC
void *kern_malloc(size_t size)
{
	if (size == allocation_failure_size) { allocation_failure_size = 0; return NULL; }
	return malloc(size);
}
...
/* These legacy oracles deliberately decline optional cache reuse. The real
 * buffer view lifecycle is exercised by the WS025 production-cache fixture. */
#ifndef UFS_AUDIT_CUSTOM_VIEW
...
{ buf_view_release(view); return disk_read(disk, first, count, buffer); }
#endif

static int (*disk_read_hook)(struct disk *, uint64_t, uint32_t, void *);
int disk_read(struct disk *disk, uint64_t first, uint32_t count, void *buffer)
{
	if (disk_read_hook != NULL) return disk_read_hook(disk, first, count, buffer);
	(void)disk;
	if (storage_mode) {
		REQUIRE(first + count <= 512);
		storage_reads++;
		if (failure_read == storage_reads) return EIO;
...
#ifndef UFS_AUDIT_CUSTOM_NAMESPACE
...
#if defined(UFS_AUDIT_CUSTOM_IO) && !defined(UFS_AUDIT_CUSTOM_WRITEBACK)
#define UFS_AUDIT_WRITEBACK_STUB_DEFINED
#ifndef UFS_AUDIT_DEFERRED_DEFAULT
#define UFS_AUDIT_DEFERRED_DEFAULT 0
#endif
static int audit_writeback_active = UFS_AUDIT_DEFERRED_DEFAULT;
int writeback_mount_active(struct mount *mountp)
{ (void)mountp;return audit_writeback_active; }
#endif
...
#ifndef UFS_AUDIT_CUSTOM_CREATION
...
static void allocation_and_truncate(void)
{
	...
	for (int fail = 0; fail <= 8; fail++) {
		for (int kind = 0; kind < 3; kind++) {
			...
			failure_write = kind < 2 ? fail : 0;
			failure_sync = kind == 2 ? fail : 0;
			int error = ufs_setxattr(&node.inode, "user.x", "v", 1, 0);
			REQUIRE(error == 0 || error == EIO);
			if (error == 0) {
				storage_writes = storage_syncs = 0;
				error = ufs_removexattr(&node.inode, "user.x");
				REQUIRE(error == 0 || error == EIO);
			}
			free(filesystem.cg);
		}
	...
	REQUIRE(ufs_setxattr(&node.inode, "user.x", "v", 1, 0) == EIO);
	...
	REQUIRE(!bit_test(storage + 32 * 512 + 264, 160));
	{
		...
		REQUIRE(ufs_unlink(&node.inode, &name) == EROFS);
		REQUIRE(ufs_rmdir(&node.inode, &name) == EROFS);
		REQUIRE(ufs_link(&node.inode, &name, &node.inode) == EROFS);
		REQUIRE(ufs_setattr(&node.inode, &status, INODE_ATTR_MODE) == EROFS);
		REQUIRE(ufs_inode_sync(&node.inode) == EROFS);
