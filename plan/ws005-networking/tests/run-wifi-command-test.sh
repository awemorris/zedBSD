...
consumer_file=$repo/plan/ws005-networking/tests/wifi-produced-stream.c
...
$cc $common "$source_file" "$consumer_file" $discard -o "$temporary/wifi-command-test"
"$temporary/wifi-command-test"

# shellcheck disable=SC2086
$cc $common -fsanitize=address,undefined -fno-omit-frame-pointer \
	"$source_file" "$consumer_file" $discard -o "$temporary/wifi-command-sanitize"
...
# shellcheck disable=SC2086
$cc $common -fanalyzer "$source_file" "$consumer_file" $discard \
