/*
 * KA-T021: UFS journal and snapshot behavior fixture
...
memory_flush(void *context)
{
	...
	disk->flushes++;
	if (disk->flushes == disk->fail_flush) {
		if (disk->fail_reads_after_flush)
			disk->fail_reads = 1;
		return EIO;
	}
	return 0;
	...
	int committed)
{
	...
	put32(descriptor + 16, 1);
	put32(descriptor + 20, sectors);
	put64(descriptor + 32, target);
	put32(descriptor + 40, sectors);
	put32(descriptor + 44, payload_digest);
	put32(descriptor + 28, digest(descriptor, SECTOR_SIZE));
	...
	if (committed) {
		...
		put32(commit + 24, digest(commit, 24));
	}
	memcpy(disk->sector[JOURNAL_FIRST + JOURNAL_SECTORS - 1U], commit,
	    sizeof(commit));
}
...
test_journal(void)
{
	...
	CHECK(JOURNAL_INIT(NULL, &io, JOURNAL_FIRST, JOURNAL_SECTORS, JOURNAL_FIRST - 1) ==
	    EINVAL);
	CHECK(JOURNAL_INIT(&journal, &io, JOURNAL_FIRST, 2, JOURNAL_FIRST - 1) == EINVAL);
	CHECK(JOURNAL_INIT(&journal, &io, JOURNAL_FIRST, JOURNAL_SECTORS, JOURNAL_FIRST - 1) == 0);
	...
	/* Retire old commit, flush redo, publish commit, install home, clear slot.
	 * Preserve the original error at all five durability boundaries. */
	for (boundary = 1; boundary <= 5; boundary++) {
		memset(&disk, 0, sizeof(disk));
		CHECK(JOURNAL_INIT(&journal, &io, JOURNAL_FIRST, JOURNAL_SECTORS, JOURNAL_FIRST - 1) == 0);
		disk.fail_flush = boundary;
		CHECK(JOURNAL_COMMIT(&journal, 10, payload, 2) == EIO);
		CHECK(journal.poisoned == 0);
		CHECK(JOURNAL_REPLAY(&journal) == 0);
		CHECK(memcmp(disk.sector[10], boundary < 3 ? zeros : payload,
		    sizeof(payload)) == 0);
	}

	/* Refuse subsequent commits when recovery cannot establish ownership. */
	memset(&disk, 0, sizeof(disk));
	CHECK(JOURNAL_INIT(&journal, &io, JOURNAL_FIRST, JOURNAL_SECTORS, JOURNAL_FIRST - 1) == 0);
	disk.fail_flush = 1;
	disk.fail_reads_after_flush = 1;
	CHECK(JOURNAL_COMMIT(&journal, 10, payload, 2) == EIO);
	CHECK(journal.poisoned == 1);
	disk.fail_reads = 0;
	CHECK(JOURNAL_COMMIT(&journal, 10, payload, 2) == EIO);
...
test_snapshot(void)
{
	...
	/* Exhaust the bounded store while retaining every published before-image. */
	CHECK(SNAPSHOT_CREATE(&reopened) == 0);
	for (record = 0; record < reopened.max_records; record++)
		CHECK(SNAPSHOT_PRESERVE(&reopened, 60 + record, 1) == 0);
	CHECK(SNAPSHOT_PRESERVE(&reopened, 80, 1) == ENOSPC);
	CHECK(reopened.next_record == reopened.max_records);
	CHECK(SNAPSHOT_PRESERVE(&reopened, 60, 1) == 0);
	CHECK(SNAPSHOT_DELETE(&reopened) == 0);

	/* Fail each preserve flush before publishing in-memory record ownership. */
	for (boundary = 1; boundary <= 3; boundary++) {
		CHECK(SNAPSHOT_CREATE(&reopened) == 0);
		disk.fail_flush = disk.flushes + boundary;
		CHECK(SNAPSHOT_PRESERVE(&reopened, 60, 1) == EIO);
		CHECK(reopened.next_record == 0);
		CHECK(memcmp(disk.sector[60], replacement, sizeof(replacement)) == 0);
		disk.fail_flush = 0;
		CHECK(SNAPSHOT_PRESERVE(&reopened, 60, 1) == 0);
		CHECK(reopened.next_record == 1);
		CHECK(SNAPSHOT_READ(&reopened, 60, 1, result) == 0);
		CHECK(memcmp(result, replacement, sizeof(result)) == 0);
		CHECK(SNAPSHOT_DELETE(&reopened) == 0);
	}
