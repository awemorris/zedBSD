...
/*
 * The target UFS generator's geometry, corruption, and I/O fault fixtures.
 */
	...
	char **argv)
{
	...
	printf("UFS formatter: %u checks PASS\n", checks);
	return 0;
}
	...
	uint64_t bytes)
{
	...
	error = ufs_format_verify(fd, bytes);
	...
	int fd)
{
	static const uint64_t rejected[] = {
		0U, 1024U, UFS_FORMAT_MIN_BYTES - 1024U,
		UFS_FORMAT_MIN_BYTES + 1U, UFS_FORMAT_MAX_BYTES + 1024U,
		UINT64_MAX
	};
	size_t index;
	int error;
	...
	for (index = 0; index < sizeof(rejected) / sizeof(rejected[0]); index++) {
		error = ufs_format_validate_size(rejected[index]);
		require_check(error == EINVAL, "reject unsupported geometry");
		error = ufs_format_write(fd, rejected[index]);
		require_check(error == EINVAL, "reject unsupported write size");
		error = ufs_format_verify(fd, rejected[index]);
	...
	build_image(fd, UFS_FORMAT_MIN_BYTES);
	build_image(fd, UFS_FORMAT_MIN_BYTES + 1024U);
	build_image(fd, UFS_FORMAT_MAX_BYTES);
}
	...
	int fd)
{
	...
	for (kind = FAULT_ERROR; kind <= FAULT_SHORT; kind++) {
		...
		for (index = 0; index < 4U; index++) {
			...
			write_kind = kind;
			error = ufs_format_write(fd, UFS_FORMAT_MIN_BYTES);
			...
			if (positions[index] != 0) {
				count = pread(fd, &magic, sizeof(magic), 65536U + 1372U);
	...
	for (kind = FAULT_ERROR; kind <= FAULT_SHORT; kind++) {
		...
		for (index = 0; index < 4U; index++) {
			...
			read_kind = kind;
			error = ufs_format_verify(fd, UFS_FORMAT_MIN_BYTES);
	...
	const char *reference_path)
{
	...
	require_check(written_bytes <= 768U * 1024U, "format writes bounded metadata only");
	require_check(read_bytes <= 768U * 1024U, "verify reads bounded metadata only");
	...
	for (offset = 0; offset < DEFAULT_BYTES; offset += sizeof(actual)) {
		...
		if (offset < 440U * 1024U ||
		    (offset >= second_group && offset < second_group + 144U * 1024U)) {
	...
	int fd)
{
	static const uint64_t offsets[] = {
		65536U + 1372U, 72U * 1024U + 200U,
		80U * 1024U + 2U * 256U, 144U * 1024U + 508U,
		272U * 1024U, 280U * 1024U, 424U * 1024U,
		(UINT64_C(2048) + 64U) * 1024U
	};
	size_t index;
	uint8_t original;
	uint8_t changed;
