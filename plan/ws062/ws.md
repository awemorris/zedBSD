...
Queue: q456（finished）
Resume point: p003 cleared（amd64 の既定は native、root・swap 各 1 GiB の 2 GiB の image、CI は gzip）。p004（規約）は問題が出たときか WS062 を閉じるとき
...
| [ws062-p003](phase003/phase.md) | amd64 の既定と試験の道具（guest.py・boot-test・guest-batches）を native にする。文書 | cleared（q456-i01。既定・menu・CI・道具・文書を native に。root・swap 各 1 GiB、CI は gzip（84 MB）。CI の再現で見つけた 2 点（toolchain の smoke、clang の libc.so の依存）を直した） | p002 |
