<!-- awesome-plan project=zedbsd record=ws003-p034 -->
...
2026-09-12、[ws027-p002](https://github.com/awemorris/zedBSD/issues/376)（[ws027](https://github.com/awemorris/zedBSD/issues/374)）へ計画を移管。旧Phaseは取消・終了し再利用しない。実行・完了を意味しない。
...
# ws003-p034: zedboot.cfg・FAT読み取り・PPC ELF handoff

## 作業

OF block I/Oを下層としてFATファイルを読み、/zedboot.cfgのkernel=で同じFATのvmunixを選ぶ。既存構文を再利用し、PPC ELF32 BEのmachine/class/endian、範囲/overflow、BSS、entry、予約メモリとの重なりを検査する。ブートパラメータ・デバイス情報をhandoffする。

## 受け入れ

選択されたvmunixのPPCテスト入口まで到達し、設定・boot identity・メモリ情報が一致する。異なるkernelパスで選択が変わること、不正設定/不正ELFが明確なエラーになることを確認する。rootやinitは条件にしない。
