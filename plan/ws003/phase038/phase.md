<!-- awesome-plan project=zedbsd record=ws003-p038 -->
...
2026-09-12、[ws027-p006](https://github.com/awemorris/zedBSD/issues/380)（[ws027](https://github.com/awemorris/zedBSD/issues/374)）へ計画を移管。旧Phaseは取消・終了し再利用しない。実行・完了を意味しない。
...
# ws003-p038: PPC USB boot・rootfs.img/data.img統合

## 作業

mac99のPCI/OHCI割込み・DMA/cache/endianを検証し、IDEからloader/kernel＋USB上root、次にOFがUSBからloader/cfg/kernelを読む順で分離する。カーネルが同じFATをboot0として再発見し、rootfs.img/data.imgを既存loop/overlay経路へ渡す。実装前に正確な設定とroot/dataの役割を固定する。

## 受け入れ

QEMUでUSBだけをブート元にして、同じFAT上のcfg/vmunix/rootfs.img/data.imgからinit/シェルへ進む。dataへの書込みが再起動後も保持される。OFロード元とカーネルroot選択の一致を記録。実機成功は主張しない。

## 依存

[ws003-p035](https://github.com/awemorris/zedBSD/issues/369), [ws003-p036](https://github.com/awemorris/zedBSD/issues/370), [ws003-p037](https://github.com/awemorris/zedBSD/issues/371)
