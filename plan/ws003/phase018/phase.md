...
Status: planned。Phase disposition: normal。Queue: none。
今回のゴールはLatitude 5320でインストールが行えること。本Phaseは既存FATを使う候補経路として再利用する。native方式は[ws003-p019](../phase019/phase.md)の候補であり、今回のゴールだけで両方式を必須にしない。
実行前に対象disk/ESP/payloadと保持データ、text/graphic、方式を決める。既存のno-format/no-GPT/no-NVRAMという本経路の境界を維持する。現行エントリは `/sbin/zedinst` と `/sbin/zedinst-graphic`。[ws019](../../ws019/ws.md)の実装・受け入れを利用し、古い「WS019実装待ち」を解除する一方、必要な現行source/artifactとの整合は確認する。
[ws004-p025](../../ws004/phase025/phase.md)の対象ストレージ読取り確認、[ws013-p002](../../ws013/phase002/phase.md)/[ws013-p003](../../ws013/phase003/phase.md)のloader/config成果を確認してから書込みを具体化する。以前の完了済みWSを再開しない。今回の通常インストールと対象からの起動を成果にし、古い反復回数を新しい機種Goalの必須条件へ無条件に持ち込まない。
実機成果物のhash・設定・対象識別子・実際の画面/ログを保存し、[ws003-p032](../phase032/phase.md)の最終変更レビューへ渡す。新しい開発・実機書込みは未承認。
