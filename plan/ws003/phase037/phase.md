<!-- awesome-plan project=zedbsd record=ws003-p037 -->
...
2026-09-12、[ws027-p005](https://github.com/awemorris/zedBSD/issues/379)（[ws027](https://github.com/awemorris/zedBSD/issues/374)）へ計画を移管。旧Phaseは取消・終了し再利用しない。実行・完了を意味しない。
...
# ws003-p037: PPCユーザーABI・libcとinit到達

## 作業

PowerPC syscall/return、ユーザーモード・例外、ELF実行、stack/TLS、libcのsyscall/context/setjmp等、image内のendiannessを監査し、初期init/シェルに必要な部分を実装する。ディスクに依存しないRAM rootの実装可否を先に確認し、追加が大きければp036後のUSB root検証と結合する判断を記録する。

## 受け入れ

PPCユーザープログラム、init/シェルを実行し、syscallとfault境界を観測する。RAM rootが未提供なのに実装済みと仮定しない。USB統合へ渡すユーザーランド/image構成と未対応ABIを明示する。
