...
build/NoctLang/build-static/noct --path=tools/build \
  plan/ws003/tests/pc98-ipl-contract.noct .
make check-disk-image
```

BR-T55 builds and checks the separately named diagnostic image, then boots a
disposable copy through both entry markers and `login:`:

```sh
make -f Makefile \
  -f plan/ws003/tests/pc98-ipl-diagnostic.mk -j16 \
  ws003-p024-diagnostic-image
build/NoctLang/build-static/noct --path=tools/build \
  plan/ws003/tests/pc98-ipl-diagnostic-contract.noct .
build/NoctLang/build-static/noct --path=tools/build \
  plan/ws003/tests/pc98-ipl-diagnostic-qemu.noct . EVIDENCE
```

The maintained BR-T46 `pc98/default` cell supplies the positive VRAM/login
gate. Disposable corruption evidence lives below
`plan/ws003/temp/pc98-v13-ipl/` and remains untracked.
...
cc -std=c11 -I. -Iinclude -Wall -Wextra -Werror \
  ...
  plan/ws003/tests/acpi-window-test.c \
...
cc -std=c11 -I. -Wall -Wextra -Werror \
  ...
  plan/ws003/tests/uefi-memory-map-test.c \
...
cc -std=c11 -Iinclude -I. -Wall -Wextra -Werror \
  src/drivers/pci.c \
  plan/ws003/tests/xhci-capability-mmio-test.c \
...
cc -std=c11 -Iinclude -I. -Wall -Wextra -Werror \
  src/drivers/usb.c \
  plan/ws003/tests/usb-hcd-unregister-test.c \
...
```sh
plan/ws003/tests/xhci-hotplug-lifecycle.sh \
...
```sh
plan/ws003/tests/legacy-xhci-usb-boot.sh \
...
```sh
cc -std=c11 -Iinclude -I. -Wall -Wextra -Werror \
  plan/ws003/tests/usb-storage-flush-test.c \
...
cc -std=c11 -Iinclude -Iinclude/uapi -I. -Wall -Wextra -Werror \
  ...
  plan/ws003/tests/boot-parameters-test.c \
...
cc -std=c11 -Iinclude -I. -Wall -Wextra -Werror \
  ...
  plan/ws003/tests/x86-parameter-handoff-test.c \
...
```sh
cc -std=c11 -Iinclude -Iinclude/uapi -I. -Wall -Wextra -Werror \
  src/kern/boot.c plan/ws003/tests/boot-source-test.c \
...
cc -std=c11 -Iinclude -Iinclude/uapi -I. -Wall -Wextra -Werror \
  ...
  src/kern/swap.c src/kern/swap-format.c \
  src/kern/swap-source.c src/kern/backing-claim.c \
  plan/ws003/tests/swap-source-test.c \
...
```sh
plan/ws003/tests/boot-parameter-qemu-acceptance.sh --list
plan/ws003/tests/boot-parameter-qemu-acceptance.sh --dry-run \
...
```sh
plan/ws003/tests/boot-parameter-qemu-acceptance.sh \
  plan/ws003/temp/q015-br-t46
```
...
| --- | --- | --- |
| amd64 UEFI file swap, repeat 002 | `plan/ws003/temp/q015-br-t46-reserve-file-focused-002-uefi` | 1767 / 3533 |
| amd64 UEFI file swap, repeat 003 | `plan/ws003/temp/q015-br-t46-reserve-file-focused-003-uefi` | 1767 / 3533 |
| amd64 BIOS raw swap | `plan/ws003/temp/q015-br-t46-reserve-raw-focused-001-bios` | 2755 / 5509 |
| amd64 UEFI raw swap | `plan/ws003/temp/q015-br-t46-reserve-raw-focused-001-uefi` | 1748 / 3495 |
| amd64 BIOS mixed file/raw swap | `plan/ws003/temp/q015-br-t46-reserve-mixed-focused-001-bios` | 2799 / 5597 |
| amd64 UEFI mixed file/raw swap | `plan/ws003/temp/q015-br-t46-reserve-mixed-focused-001-uefi` | 1792 / 3583 |
...
`plan/ws003/temp/q015-br-t46-final-007`: PC/AT 7/7, PC-98 6/6, amd64
...
**PASS 31/31** at
`plan/ws003/temp/q023-p016-br-t46-authoritative.9rIQrm`: PC/AT 7/7,
...
PC-98 at `plan/ws003/temp/q023-p016-audit-final.8oHUQf`. The affected
WS016 runtime-swap runner also passed file, mixed, and native cells 3/3 at
`plan/ws016/temp/q023-p016-runtime-swap.UGNqkG`.
...
make -j16 toolchain
build/NoctLang/build-static/noct --path=tools/build \
  plan/ws003/tests/boot-parameter-source-audit.noct \
  "$PWD" plan/ws003/temp/br-t47-source-audit
```
...
```sh
plan/ws003/tests/run-cf-sv7-early-init-host-test.sh
```
...
```sh
plan/ws003/tests/cf-sv7-pit-timeout-qemu.sh \
  build/amd64/hdd-image.img plan/ws003/temp/br-t52-pit-off
```
...
```sh
plan/ws003/tests/cf-sv7-early-idt-fault-qemu.sh \
  build/amd64/hdd-image.img plan/ws003/temp/br-t52-early-idt
```
...
```sh
plan/ws003/tests/bounded-gpt-larger-usb-boot.sh \
  build/amd64/hdd-image.img plan/ws003/temp/br-t53
```
