...
cc -std=c11 -D_POSIX_C_SOURCE=200809L -I. -Wall -Wextra -Werror \
  ...
  plan/ws011/tests/netconf-parser-test.c \
...
```sh
sh plan/ws011/tests/net-console-test.sh
...
cc -std=c11 -D_POSIX_C_SOURCE=200809L -I. -Wall -Wextra -Werror \
  -DNETCONF_LOCK_PATH='"/tmp/ws011-net.conf.lock"' \
  userland/base/net/netconf.c \
  plan/ws011/tests/netconf-persistence-test.c \
...
```sh
sh plan/ws011/tests/net-boot-test.sh
```
...
cc -std=c11 -D_POSIX_C_SOURCE=200809L -D_DEFAULT_SOURCE -I. \
  ...
  plan/ws011/tests/netconf-reconcile-test.c \
...
cc -std=c11 -D_POSIX_C_SOURCE=200809L -D_DEFAULT_SOURCE -I. \
  ...
  plan/ws011/tests/confirmed-commit-model-test.c \
...
```sh
sh plan/ws011/tests/confirmed-commit-test.sh
```
...
```sh
sh plan/ws011/tests/run-netconf-atomic-writer-host-test.sh
NCOM_SANITIZE=1 sh plan/ws011/tests/run-netconf-atomic-writer-host-test.sh
```
...
```sh
timeout 45s make -f plan/ws011/tests/overlay-publication-host-test.mk run
timeout 45s make -f plan/ws011/tests/overlay-publication-host-test.mk sanitize
```
...
```sh
sh plan/ws011/tests/run-fat-write-cost-host-test.sh
NCOM_SANITIZE=1 sh plan/ws011/tests/run-fat-write-cost-host-test.sh
timeout 180s sh plan/ws018/tests/run-fat-native-vfs-host-test.sh
```
...
```sh
sh plan/ws011/tests/run-fat-write-cursor-host-test.sh
NCOM_SANITIZE=1 sh plan/ws011/tests/run-fat-write-cursor-host-test.sh
```
...
differences in the [p009 ten-cell matrix](../phase009/phase.md).

```sh
NCOM_CELL_SELECTION=t021 NCOM_CAPTURE_FAILURE=1 NCOM_VARIANT=baseline \
  bash plan/ws011/tests/run-confirmed-commit-qemu.sh \
  plan/ws011/temp/q075-normal-01
```
