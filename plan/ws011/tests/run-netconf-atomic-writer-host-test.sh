...
test_root=$(mktemp -d plan/ws011/temp/atomic-writer.XXXXXX)
...
timeout --kill-after=5s 60s "${CC:-cc}" -std=c11 \
  ...
  plan/ws011/tests/netconf-atomic-writer-host-test.c \
...
timeout --kill-after=5s 20s "$test_root/netconf-atomic-writer-host-test" \
  plan/ws011/tests/confirmed-commit-qemu-net.conf \
  "$test_root/fixture"
