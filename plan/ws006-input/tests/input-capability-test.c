	    ...
	    size_t expected_size, const unsigned *codes, size_t code_count)
{
	...
	assert(drv_input_capability_bits(state, type, &actual, &actual_size) == 0);
	assert(actual != NULL);
...
test_bitsets_and_queries(void)
{
	...
	assert(drv_input_capability_state_init(&state, valid_capabilities,
	...
	assert(drv_input_capability_bits(&state, EV_MSC, &pointer, &size) ==
	    EINVAL);
	assert(pointer == (const uint8_t *)(uintptr_t)1U && size == 123U);
	assert(drv_input_capability_bits(NULL, EV_KEY, &pointer, &size) == EINVAL);
	assert(drv_input_capability_bits(&state, EV_KEY, NULL, &size) == EINVAL);
	assert(drv_input_capability_bits(&state, EV_KEY, &pointer, NULL) == EINVAL);

	memset(&info, 0xa5, sizeof(info));
	assert(drv_input_capability_abs_info(&state, ABS_Y, &info) == ENOENT);
	for (size_t index = 0; index < sizeof(info); index++)
		assert(((const uint8_t *)&info)[index] == 0xa5U);
	assert(drv_input_capability_abs_info(NULL, ABS_X, &info) == EINVAL);
	assert(drv_input_capability_abs_info(&state, ABS_X, NULL) == EINVAL);
	assert(drv_input_capability_abs_info(&state, ABS_MAX + 1U, &info) ==
	    EINVAL);
}
...
test_copy_windows(void)
{
	...
	assert(drv_input_capability_copy(source, sizeof(source), 0,
	    destination + 2, 2) == 0);
	...
	memset(destination, 0xa5, sizeof(destination));
	assert(drv_input_capability_copy(source, sizeof(source), 2,
	    destination + 2, 4) == 0);
	...
	memset(destination, 0xa5, sizeof(destination));
	assert(drv_input_capability_copy(NULL, 0, 0, destination + 2, 3) == 0);
	assert(destination[2] == 0 && destination[3] == 0 &&
	    destination[4] == 0);
	assert(destination[1] == 0xa5 && destination[5] == 0xa5);

	memset(destination, 0xa5, sizeof(destination));
	assert(drv_input_capability_copy(NULL, 1, 0, destination, 1) == EINVAL);
	assert(drv_input_capability_copy(source, sizeof(source), 0, NULL, 1) ==
	    EINVAL);
	assert(drv_input_capability_copy(source, sizeof(source), SIZE_MAX,
	    destination, 2) == EINVAL);
	for (size_t index = 0; index < sizeof(destination); index++)
		assert(destination[index] == 0xa5U);
	assert(drv_input_capability_copy(NULL, 0, SIZE_MAX, NULL, 0) == 0);
}
...
test_key_state(void)
{
	...
	assert(drv_input_capability_event(&state, EV_KEY, KEY_A, 1) == 1);
	assert(bit_is_set(bits, size, KEY_A));
	assert(drv_input_capability_event(&state, EV_KEY, KEY_A, 2) == 1);
	assert(bit_is_set(bits, size, KEY_A));
	assert(drv_input_capability_event(&state, EV_KEY, KEY_A, 99) == 0);
	assert(bit_is_set(bits, size, KEY_A));
	assert(drv_input_capability_event(&state, EV_KEY, KEY_A, 0) == 1);
	assert(!bit_is_set(bits, size, KEY_A));

	assert(drv_input_capability_event(&state, EV_KEY, KEY_MAX, 2) == 1);
	assert(bit_is_set(bits, size, KEY_MAX));
	assert(drv_input_capability_event(&state, EV_KEY, KEY_MAX, 0) == 1);
	assert(!bit_is_set(bits, size, KEY_MAX));

	assert(drv_input_capability_event(&state, EV_KEY, KEY_B, 1) == 0);
	assert(!bit_is_set(bits, size, KEY_B));
	assert(drv_input_capability_event(&state, EV_KEY, KEY_MAX + 1U, 1) == 0);
	assert(drv_input_capability_event(&state, EV_REL, REL_X, 1) == 1);
	assert(drv_input_capability_event(&state, EV_SYN, SYN_REPORT, 0) == 1);
	assert(drv_input_capability_event(&state, EV_SYN, SYN_DROPPED, 0) == 0);
	assert(drv_input_capability_event(&state, EV_SYN, SYN_REPORT, 1) == 0);
	assert(drv_input_capability_event(NULL, EV_KEY, KEY_A, 1) == 0);
	for (size_t index = 0; index < size; index++)
		assert(bits[index] == 0);

	assert(drv_input_capability_key_state(NULL, &bits, &size) == EINVAL);
	assert(drv_input_capability_key_state(&state, NULL, &size) == EINVAL);
	assert(drv_input_capability_key_state(&state, &bits, NULL) == EINVAL);
}
...
test_absolute_state(void)
{
	...
	assert(drv_input_capability_state_init(&state, valid_capabilities,
	    ARRAY_COUNT(valid_capabilities), valid_axes,
	    ARRAY_COUNT(valid_axes)) == 0);
	assert(drv_input_capability_abs_info(&state, ABS_X, &info) == 0);
	expect_abs_equal(&info, &expected);

	assert(drv_input_capability_event(&state, EV_ABS, ABS_X, 511) == 1);
	expected.value = 511;
	assert(drv_input_capability_abs_info(&state, ABS_X, &info) == 0);
	expect_abs_equal(&info, &expected);

	assert(drv_input_capability_event(&state, EV_ABS, ABS_Y, 9) == 0);
	assert(drv_input_capability_event(&state, EV_ABS, ABS_MAX + 1U, 9) == 0);
	assert(drv_input_capability_event(&state, EV_KEY, ABS_X, 1) == 0);
	assert(drv_input_capability_abs_info(&state, ABS_X, &info) == 0);
	expect_abs_equal(&info, &expected);

	assert(drv_input_capability_abs_info(&state, ABS_MT_SLOT, &info) == 0);
	expect_abs_equal(&info, &valid_axes[1].info);
	assert(drv_input_capability_abs_info(&state, ABS_MAX, &info) == 0);
...
test_registration_validation(void)
{
	...
	assert(drv_input_capability_state_init(NULL, valid_capabilities,
	    ARRAY_COUNT(valid_capabilities), valid_axes,
	    ARRAY_COUNT(valid_axes)) == EINVAL);
	assert(drv_input_capability_state_init(&state, NULL, 1, NULL, 0) ==
	    EINVAL);
	assert(drv_input_capability_state_init(&state, NULL, 0, NULL, 1) ==
	    EINVAL);
	assert(drv_input_capability_state_init(&state, NULL, 0, NULL, 0) == EINVAL);
	assert(drv_input_capability_state_init(&state, only_key_a,
	    ARRAY_COUNT(only_key_a), NULL, 0) == EINVAL);
	assert(drv_input_capability_state_init(&state, only_sync,
	    ARRAY_COUNT(only_sync), NULL, 0) == 0);
	assert(drv_input_capability_state_init(&state, duplicate_capabilities,
	    ARRAY_COUNT(duplicate_capabilities), NULL, 0) == EINVAL);
	assert(drv_input_capability_state_init(&state, duplicate_sync,
	    ARRAY_COUNT(duplicate_sync), NULL, 0) == EINVAL);
	assert(drv_input_capability_state_init(&state, invalid_sync,
	    ARRAY_COUNT(invalid_sync), NULL, 0) == EINVAL);
	assert(drv_input_capability_state_init(&state, invalid_type,
	    ARRAY_COUNT(invalid_type), NULL, 0) == EINVAL);
	assert(drv_input_capability_state_init(&state, invalid_key,
	    ARRAY_COUNT(invalid_key), NULL, 0) == EINVAL);
	assert(drv_input_capability_state_init(&state, invalid_rel,
	    ARRAY_COUNT(invalid_rel), NULL, 0) == EINVAL);
	assert(drv_input_capability_state_init(&state, invalid_abs,
	    ARRAY_COUNT(invalid_abs), NULL, 0) == EINVAL);
	assert(drv_input_capability_state_init(&state, only_abs_x,
	...
	assert(drv_input_capability_state_init(&state, only_abs_x, 1,
	    reversed_range, ABS_MAX + 2U) == EINVAL);
	assert(drv_input_capability_state_init(&state, only_abs_x, 1, NULL, 0) ==
	    EINVAL);
}
