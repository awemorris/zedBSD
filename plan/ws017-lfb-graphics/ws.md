...
Resume point: select `ws017-p001` for a finite Queue after execution approval.
...
- Forked mappings remain shared and retain the same ownership lease;
  `mprotect` may remove permissions but may not add write or execute rights.
...
| --- | --- | --- | --- |
| `ws017-p001` | [Device-mmap and graphics LFB UAPI](phase001-device-mmap-uapi/phase.md) | Planned; Queue-ready | Optional cdev mapping, lease lifetime, query validation, 8/16/24/32 layouts, and fallback pass host tests |
