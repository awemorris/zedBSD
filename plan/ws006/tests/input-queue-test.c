...
main(void)
{
	...
	drv_input_queue_reader_init(&queue, &early);
	for (index = 0; index < 3; index++) {
		item = event(index);
		drv_input_queue_push(&queue, &item);
	}
	drv_input_queue_reader_init(&queue, &late);
	count = drv_input_queue_read(&queue, &early, output, 2);
	if (count != 2 || output[0].value != 0 || output[1].value != 1)
		return 1;
	if (drv_input_queue_readable(&queue, &late))
		return 2;
	item = event(3);
	drv_input_queue_push(&queue, &item);
	count = drv_input_queue_read(&queue, &late, output, 4);
	...
	count = drv_input_queue_read(&queue, &early, output, 4);
	if (count != 4 || output[0].type != EV_SYN ||
	    output[0].code != SYN_DROPPED || output[1].value != 8)
		return 4;
	drv_input_queue_detach(&queue);
	if (!queue.detached)
		return 5;
	return 0;
}
