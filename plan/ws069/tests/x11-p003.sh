...
stop_all='for p in $(ps -A -o pid,args | grep -E "[z]wl|[X]zed|[z]term" | awk "{print \$1}"); do kill $p; done; i=0; while ps -A -o args | grep -qE "[z]wl|[X]zed|[z]term" && [ $i -lt 50 ]; do sleep 0.2; i=$((i+1)); done'
status=0
...
guest 'cat /tmp/xzed.log; grep -E "ZWL (MAP|TITLE)" /tmp/zwl.log' | tee "$out/start.txt"
