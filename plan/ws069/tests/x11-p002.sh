...
#  1. xzed.png: Xzed --wayland runs zterm (an X client) in a zdesktop --glass window; its shell's prompt shows.
...
stop_all='for p in $(ps -A -o pid,args | grep -E "[z]desktop( |$)|[X]zed|[z]term|[w]lshm|[m]view|[e]gltest|[z]desktop-terminal" | awk "{print \$1}"); do kill $p; done; i=0; while ps -A -o args | grep -qE "[z]desktop( |$)|[X]zed|[z]term" && [ $i -lt 50 ]; do sleep 0.2; i=$((i+1)); done'
status=0
...
guest 'cat /tmp/xzed.log; grep "ZWL MAP" /tmp/zdesktop.log' | tee "$out/start.txt"
