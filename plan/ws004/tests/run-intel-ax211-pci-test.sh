#!/bin/sh
set -eu
python3 "$(CDPATH= cd -- "$(dirname -- "$0")/../../.." && pwd)/plan/ws025/tests/prepare-driver-fragments.py" --source src/drivers/wifi/intel-ax211/intel-ax211.c --source src/drivers/wifi/intel-ax211/intel-ax211.c
...
includes="-I$repository/plan/ws004/tests/host-include -I$repository/include -I$repository/include/uapi"
fixture="$repository/plan/ws004/tests/intel-ax211-pci-test.c"
...
$compiler $warnings $target_includes -m64 -fsyntax-only \
	"$repository/plan/ws025/temp/p031-driver-fragments/src/drivers/pci-intel-ax211.c"
# shellcheck disable=SC2086
