...
[`ws004-p031`](../phase031/phase.md) owns a
...
TMPDIR="$PWD/build/q047-tmp" \
  ...
  src/drivers/pci.c \
  plan/ws004/tests/pci-shared-intx-test.c \
...
[`ws004-p032`](../phase032/phase.md) owns the
...
mkdir -p build/q047-tmp
TMPDIR="$PWD/build/q047-tmp" \
  plan/ws004/tests/run-usb-recovery-contract-test.sh
```
...
```sh
TMPDIR="$PWD/build/q047-tmp" \
  plan/ws004/tests/run-amd64-console-output-host-test.sh
```
...
TMPDIR="$PWD/build/q045-tmp" \
  plan/ws004/tests/run-xhci-superspeed-interrupt-context-test.sh
```
...
```sh
plan/ws004/tests/run-rtl8822b-table-import-test.sh
RTL8822B_FIRMWARE_TEST_BLOB=build/sources/firmware/rtl8822b/2f56219d20e4becccd718963fc3bcc671c543ce5/rtw8822b_fw.bin \
  plan/ws004/tests/run-rtl8822b-core-test.sh
plan/ws004/tests/run-usb-rtl8822bu-driver-test.sh
```
...
TMPDIR="$PWD/build/q058-tmp" \
  plan/ws004/tests/run-wlan-wpa2-ccmp-l2-test.sh
```
...
| --- | --- |
| `plan/ws004/tests/run-wlan-crypto-test.sh` | SHA-1, HMAC-SHA1, PBKDF2, AES-128, RFC 3394, bounds, erasure, sanitizer/analyzer, and amd64/i386 syntax |
| `plan/ws004/tests/run-wlan-wpa2-codec-test.sh` | Strict RSN, Authentication, Association, and EAPOL-Key wire encoding/decoding plus amd64/i386 syntax |
| `plan/ws004/tests/run-wlan-wpa2-engine-test.sh` | Asynchronous four-way handshake, response filtering, deadlines, retransmission/replay, key rollback, authorization, and secret lifetime |
| `plan/ws004/tests/run-wlan-l2-test.sh` | Ethernet/802.11/LLC-SNAP conversion, CCMP metadata, key ID/generation, PN replay, MIC trailer, and malformed-frame rejection |
| `plan/ws004/tests/run-rtl8822b-security-test.sh` | RTL8822B CAM word encoding, bounded register operations, programming, invalidation, and failure handling |
| `plan/ws004/tests/run-wlan-ccmp-reference-test.sh` | Independent RFC 3610 AES-CCM vector and production 802.11 CCMP nonce/AAD/PN/Key-ID/MIC cross-check |
| `plan/ws004/tests/run-wlan-common-core-test.sh` | Public ABI, scan/connect state, common WPA2/L2 integration, controlled-port/carrier ordering, cancellation, detach, and secret erasure |
| `plan/ws004/tests/run-usb-rtl8822bu-driver-test.sh` | RTL8822BU USB management/EAPOL/data descriptors, RX security classification, CAM generations, TX reports, errors, and teardown |
| `plan/ws004/tests/run-hal-printf-format-host-test.sh` | Production amd64 `hal_printf` conversions through a captured console, including the `l`/`ll` length modifiers behind the q070 RTL8822BU connect diagnostics |
...
TMPDIR="$PWD/build/q030-tmp" \
  plan/ws004/tests/run-gpt-host-test.sh
TMPDIR="$PWD/build/q030-tmp" \
  plan/ws004/tests/run-partition-publication-test.sh
```
...
```sh
TMPDIR="$PWD/build/q030-tmp" \
  plan/ws004/tests/qemu-nvme-gpt.sh
```
...
```sh
TMPDIR="$PWD/build/q030-tmp" \
  plan/ws004/tests/run-nvme-io-test.sh
TMPDIR="$PWD/build/q030-tmp" \
  plan/ws004/tests/run-nvme-io-lifecycle-test.sh
TMPDIR="$PWD/build/q030-tmp" \
  plan/ws004/tests/run-devfs-block-range-test.sh
```
...
TMPDIR="$PWD/build/q030-tmp" \
  plan/ws004/tests/qemu-nvme-io.sh
```
...
cc -std=c11 -Iinclude -Wall -Wextra -Werror \
  src/drivers/dma.c plan/ws004/tests/dma-constraints-test.c \
  -o /tmp/ws004-dma-test
/tmp/ws004-dma-test

cc -std=c11 -Iinclude -Wall -Wextra -Werror \
  src/drivers/pci.c plan/ws004/tests/pci-rescan-test.c \
  -o /tmp/ws004-pci-test
/tmp/ws004-pci-test

cc -std=c11 -Iinclude -Wall -Wextra -Werror \
  src/drivers/pci.c plan/ws004/tests/pcie-capability-test.c \
...
cc -std=c11 -Wall -Wextra -Werror \
  plan/ws004/tests/pci-hcd-irq-teardown-test.c \
...
TMPDIR="$PWD/build/q041-tmp" \
  plan/ws004/tests/run-legacy-hcd-retirement-test.sh
```
...
TMPDIR="$PWD/build/q041-tmp" make -j16 \
  ZEDBSD_CONFIG=plan/ws004/tests/config-amd64-legacy-hcd.mk \
  disk-image
TMPDIR="$PWD/build/q041-tmp" \
  plan/ws004/tests/run-legacy-hcd-qemu.sh \
...
```sh
cc -std=c11 -Iinclude -Wall -Wextra -Werror \
  plan/ws004/tests/xhci-model-test.c \
...
```sh
cc -std=c11 -Iinclude -Wall -Wextra -Werror \
  plan/ws004/tests/usb-storage-scsi-test.c \
...
```sh
TMPDIR="$PWD/build/q047-tmp" \
  plan/ws004/tests/run-usb-storage-no-media-test.sh
```
...
cc -std=c11 -Wall -Wextra -Werror -pthread \
  plan/ws004/tests/usb-urb-publication-test.c \
...
cc -std=c11 -Iinclude -Iinclude/uapi -Wall -Wextra -Werror \
  -pthread \
  src/drivers/usb.c \
  plan/ws004/tests/usb-function-model-test.c \
...
TMPDIR="$PWD/build/q027-tmp" \
  plan/ws004/tests/run-xhci-concurrent-urbs-test.sh
```
...
```sh
plan/ws004/tests/run-net-device-hotplug-test.sh
plan/ws004/tests/run-usb-cdc-ncm-wire-test.sh
plan/ws004/tests/run-usb-cdc-ncm-driver-test.sh
```
...
```sh
plan/ws004/tests/run-usb-cdc-ecm-driver-test.sh
```
...
```sh
plan/ws004/tests/run-usb-hcd-zero-packet-test.sh
```
...
build/NoctLang/build-static/noct --path=tools/build \
  plan/ws004/tests/qemu-usb-cdc-ecm.noct \
  "$PWD" /tmp/ws004-p019-evidence
```

## HW-T28 CDC ECM asynchronous TX accounting

The planned [`ws004-p034`](../phase034/phase.md)
...
The deferred [`ws004-p035`](../phase035/phase.md)
...
[`ws004-p036`](../phase036/phase.md) owns this
...
```sh
plan/ws004/tests/run-rtl8822b-firmware-package-test.sh
RTL8822B_FIRMWARE_TEST_BLOB=build/sources/firmware/rtl8822b/2f56219d20e4becccd718963fc3bcc671c543ce5/rtw8822b_fw.bin \
  plan/ws004/tests/run-rtl8822b-core-test.sh
```
...
```sh
plan/ws004/tests/run-intelax211-firmware-package-test.sh
plan/ws004/tests/run-intel-ax211-driver-license-test.sh
```
...
```sh
plan/ws004/tests/run-intel-ax211-core-test.sh
```
...
```sh
plan/ws004/tests/run-intel-ax211-firmware-loader-test.sh
plan/ws004/tests/run-intel-ax211-dma-test.sh
plan/ws004/tests/run-intel-ax211-mmio-test.sh
plan/ws004/tests/run-intel-ax211-pci-mmio-test.sh
plan/ws004/tests/run-intel-ax211-protocol-test.sh
plan/ws004/tests/run-intel-ax211-init-test.sh
plan/ws004/tests/run-intel-ax211-command-test.sh
plan/ws004/tests/run-intel-ax211-transport-backend-test.sh
plan/ws004/tests/run-intel-ax211-transport-test.sh
plan/ws004/tests/run-intel-ax211-boot-test.sh
plan/ws004/tests/run-intel-ax211-runtime-start-test.sh
plan/ws004/tests/run-intel-ax211-runtime-scan-test.sh
plan/ws004/tests/run-intel-ax211-scan-session-test.sh
plan/ws004/tests/run-intel-ax211-bss-test.sh
plan/ws004/tests/run-intel-ax211-assoc-test.sh
plan/ws004/tests/run-intel-ax211-key-test.sh
plan/ws004/tests/run-intel-ax211-tx-ring-test.sh
plan/ws004/tests/run-intel-ax211-tx-test.sh
plan/ws004/tests/run-intel-ax211-rx-test.sh
```
...
```sh
plan/ws004/tests/run-intel-ax211-pci-test.sh
plan/ws004/tests/run-intel-ax211-wlan-common-integration-test.sh
```
...
```sh
plan/ws004/tests/run-wlan-l2-test.sh
```
...
```sh
plan/ws004/tests/run-nvme-admin-test.sh
plan/ws004/tests/run-nvme-lifecycle-test.sh
```
...
TMPDIR="$PWD/build/q030-tmp" \
  plan/ws004/tests/run-nvme-io-lifecycle-test.sh
TMPDIR="$PWD/build/q030-tmp" \
  plan/ws004/tests/run-nvme-shutdown-lifecycle-test.sh
```
...
```sh
cc -std=c11 -Iinclude -I. -Wall -Wextra -Werror \
  src/drivers/pci.c plan/ws004/tests/pci-msi-test.c \
...
```sh
plan/ws004/tests/qemu-nvme-admin.sh \
...
[`ws004-p041`](../phase041/phase.md) owns the checked
...
[`ws004-p044`](../phase044/phase.md) owns the
...
[run-wifi-command-test.sh](../../ws005/tests/run-wifi-command-test.sh).
The targeted
[run-networkd-retire-stage-test.sh](../../ws005/tests/run-networkd-retire-stage-test.sh)
