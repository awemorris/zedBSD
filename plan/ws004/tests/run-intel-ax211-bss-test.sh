...
build_dir=$(mktemp -d "${TMPDIR:-/tmp}/zedbsd-intel-ax211-bss.XXXXXX")
trap 'rm -rf "$build_dir"' EXIT HUP INT TERM

cc=${CC:-cc}
warnings="-std=c11 -Wall -Wextra -Werror"
sources="$repo_root/src/drivers/wifi/intel-ax211/intel-ax211-bss.c"
fixture="$test_dir/intel-ax211-bss-test.c"
...
"$build_dir/analyzer"
# AX211 is amd64-only, but its private parser/cache is ILP32-clean.
# shellcheck disable=SC2086
$cc $warnings -m64 -nostdinc $abi_includes -fsyntax-only $sources
# shellcheck disable=SC2086
$cc $warnings -m32 -nostdinc $abi_includes -fsyntax-only $sources

echo "intel ax211 BSS metadata: ordinary, ASan/UBSan, analyzer, amd64/i386 syntax PASS"
