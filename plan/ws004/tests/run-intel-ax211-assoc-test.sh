...
build_dir=$(mktemp -d "${TMPDIR:-/tmp}/zedbsd-intel-ax211-assoc.XXXXXX")
trap 'rm -rf "$build_dir"' EXIT HUP INT TERM

cc=${CC:-cc}
warnings="-std=c11 -Wall -Wextra -Werror"
sources="$repo_root/src/drivers/wifi/intel-ax211/intel-ax211-protocol.c"
sources="$sources $repo_root/src/drivers/wifi/intel-ax211/intel-ax211-assoc.c"
fixture="$test_dir/intel-ax211-assoc-test.c"
...
# AX211 is amd64-only, but its private wire/state codec is ILP32-clean.
# shellcheck disable=SC2086
$cc $warnings -m64 -nostdinc $abi_includes -fsyntax-only $sources
# shellcheck disable=SC2086
$cc $warnings -m32 -nostdinc $abi_includes -fsyntax-only $sources

echo "intel ax211 association: ordinary, real API89, ASan/UBSan, analyzer, amd64/i386 syntax PASS"
