...
Parent: [p013](phase.md). Execution authority and limits: [q077](../../history/queue-q077.md).
...
```sh
python3 plan/ws018/tests/audit-legacy-disk-mount.py
sh plan/ws018/tests/run-legacy-disk-mount-test.sh
git diff --check
```
...
4. Only after host/build prerequisites pass and remaining q077 budget is
   ...
   ```sh
   python3 plan/ws019/tests/run-storage-qemu.py combined plan/ws019/temp/q077-resume-01 --mount-protection
   ```
