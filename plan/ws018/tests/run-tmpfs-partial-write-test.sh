...
temporary=$(mktemp -d "$repo/plan/ws018/temp/tmpfs-write.XXXXXX")
...
for mode in ordinary sanitize; do
 ...
 ${HOSTCC:-cc} -std=c11 -O1 -g -Wall -Wextra -Werror -ffunction-sections -fdata-sections \
  -DKERN_USER_ABI_LP64 $extra -I"$repo/include" -I"$repo/include/uapi" \
  -I"$repo/src" -I"$repo/libc/include" \
  "$repo/plan/ws018/tests/tmpfs-partial-write-test.c" \
