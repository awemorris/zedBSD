...
try:
    for chunk, imod in [(512,4000),(4096,4000),(512,0),(4096,0)]:
        ...
        with (out / (name + "-guest.log")).open("w") as log:
            subprocess.run(["python3","plan/ws018/tests/run-storage-native.py",
    ...
    with (out / "restore-build.log").open("w") as log:
        subprocess.run([*base,"-W","src/kern/syscall.c","-W","plan/ws025/temp/p031-driver-fragments/src/drivers/pci-xhci.c",
