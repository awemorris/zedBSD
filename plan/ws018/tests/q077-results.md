...
Date: 2026-09-05. Parent: [Queue](../../history/queue-q077.md).
...
| Command / gate | Result |
| --- | --- |
| `python3 plan/ws018/tests/audit-legacy-disk-mount.py` | KA-T120 PASS, 1,406 active files and six manifests |
| `sh plan/ws018/tests/run-legacy-disk-mount-test.sh` | KA-T121 1,303 checks each normal/ASan/UBSan; 21 admission schedules; covered rmdir EBUSY with no callback |
| `sh plan/ws018/tests/run-ufs-metadata-audit.sh` | UFS1 6,010 / UFS2 6,595 allocation/truncate/xattr/admission checks each normal/sanitized, plus shared-block update and directory bounds |
| `sh plan/ws018/tests/run-tmpfs-partial-write-test.sh` | 79 checks each normal/sanitized |
| `make -f plan/ws001/tests/credential-vfs-overlay-fault-host-test.mk run sanitize` | 3,204 checks each; real overlay preparation/copy-up/remove/rename routines |
| `make -f plan/ws001/tests/credential-vfs-ufs-socket-fault-host-test.mk run sanitize` | 78 checks per UFS version/mode |
| `make -f plan/ws001/tests/directory-fsync-host-test.mk run run-sanitize` | Retained fsync cells pass; UFS namespace rollback 79/83 checks normal/sanitized |
| `sh plan/ws018/tests/run-fat-native-vfs-host-test.sh` | 441,782 checks each normal/sanitized |
| `sh plan/ws019/tests/run-storage-foundation-test.sh` | 20,376 checks each normal/sanitized; amd64/i386 ABI PASS |
| `sh plan/ws016/tests/run-backing-claim-test.sh` | PASS |
| `sh plan/ws018/tests/run-filesystem-identity-host-test.sh` | 110 checks and ownership audit PASS |
| `sh plan/ws018/tests/run-ufs2-consistency-host-test.sh` | 45 checks and UFS2 ownership PASS |
...
Full logs are ignored under `plan/ws018/temp/q077-*.log`.
...
```sh
make -j16
make -j16 ZEDBSD_CONFIG=plan/ws021/tests/config-pcat.mk
make -j16 ZEDBSD_CONFIG=plan/ws021/tests/config-pc98.mk
make -j16 -f Makefile -f plan/ws019/tests/storage-qemu.mk ws019-storage-qemu-fixture
```
...
1. `plan/ws019/temp/q077-resume-01`: PASS, including reboot.
...
2. `plan/ws019/temp/q077-resume-02`: PASS on the final source,
