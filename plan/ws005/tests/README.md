...
```sh
sh plan/ws005/tests/run-wifi-conf-store-test.sh
```
...
build/NoctLang/build-static/noct --path=tools/build \
  plan/ws005/tests/wifi-credential-native-qemu.noct \
...
```sh
plan/ws005/tests/run-networkd-auth-test.sh
plan/ws005/tests/run-peercred-native-qemu.sh
plan/ws005/tests/run-inet-ioctl-authorization-test.sh
```
...
project-local `plan/ws005/temp` directory and honors `TMPDIR` when
...
```sh
plan/ws005/tests/run-wifi-command-test.sh
```
