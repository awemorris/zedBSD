...
| --- | --- | --- |
| [`ws007-p001`](phase001/phase.md) | Complete | PATH script lookup repaired; production `startx` launches the four-program session |
| [`ws007-p002`](phase002/phase.md) | Carried forward | Relative tracking/bounds pass; reported mismatch not reproduced and absolute input unavailable |
| [`ws007-p003`](phase003/phase.md) | Complete (`q039`) | PIC cascade lifecycle fixture passes; production qemu-pc98 moves Xzed exactly `(320,240) -> (420,290)` through evdev without manual PIC repair |
| [`ws007-p004`](phase004/phase.md) | Uncleared (`q043`) | Final image/QEMU/hash-recorded headless and focused host gates pass; maintained QEMU has no interactive backend, so resume only from the user's exact failing GUI environment |
