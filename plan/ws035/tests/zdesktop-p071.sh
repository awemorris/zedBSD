...
# /etc/zdesktop/apps.conf gets 30 applications (2 pages of 24); zwl --glass runs with a wl_shm window.
...
stop_all='ps -A -o pid,comm | awk "{ n = \$2; sub(\".*/\", \"\", n) } n == \"zwl\" || n == \"wlshm\" || n == \"Xzed\" {print \$1}" | while read p; do kill $p; done; sleep 1'
status=0
...
icon() {
	for try in 1 2 3; do
		found=$(guest "grep 'ZWL HOME icon name=\"$1\"' /tmp/zwl.log | tail -1" | sed -n 's/.* x=\([-0-9]*\) y=\([0-9]*\).*/\1 \2/p')
		[ -n "$found" ] && break
		sleep 1
	done
	echo "$found"
