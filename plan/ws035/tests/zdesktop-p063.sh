...
#  2. A longer drag opens it: peek.png on the way, then open.png; zwl logs
...
frames=$(guest "grep -c 'WISEVIEW frame progress=0\.' /tmp/zwl.log")
...
set -- $(guest "grep 'ZWL MAP client=2 ' /tmp/zwl.log" | sed -n 's/.* x=\([-0-9]*\) y=\([-0-9]*\).*/\1 \2/p')
...
# Nothing failed.
guest 'grep -E "ERROR|FAILED" /tmp/zwl.log /tmp/a.log /tmp/s.log /tmp/b.log /tmp/m.log' | tee "$out/errors.txt"
[ -s "$out/errors.txt" ] && status=1
guest 'grep -E "WISEVIEW (opening|open|tile|cancel|close|select|closed)" /tmp/zwl.log' > "$out/log.txt"
guest "$stop_all" >/dev/null
