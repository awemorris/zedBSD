...
#
# zwl runs at 1280x800.  Client a is a red 400x300 window, client b a green
...
guest 'for p in $(ps -A -o pid,args | grep -E "[z]wl|[w]ltest" | awk "{print \$1}"); do kill $p; done; sleep 1; rm -f /tmp/wayland-0' >/dev/null
guest '/bin/zwl --timeout=400 --width=1280 --height=800 --log-frames > /tmp/zwl.log 2>&1 </dev/null & sleep 3
...
wait_for 'grep "MODE fullscreen" /tmp/zwl.log' || status=1
sleep 2
...
wait_for 'grep "MODE window" /tmp/zwl.log | sed -n 2p' || status=1
sleep 2
...
guest 'echo "direct presents: $(grep -c "direct=1" /tmp/zwl.log)"
echo "vulkan imports: $(grep -c VULKAN_IMPORT /tmp/zwl.log)"
echo "compose frames: $(grep -c "ZWL COMPOSE" /tmp/zwl.log)"
grep -E "MODE|OUTPUT|ERROR" /tmp/zwl.log
grep "PERF compose" /tmp/zwl.log | tail -5' | tee "$out/log-summary.txt"
guest 'for p in $(ps -A -o pid,args | grep -E "[z]wl|[w]ltest" | awk "{print \$1}"); do kill $p; done' >/dev/null
