# Automatically generated by the zedBSD build menu.  Do not edit.
ZEDBSD_MENU_VERSION := 3
ZEDBSD_PLATFORM := sun4u
ZEDBSD_ARCHITECTURE := sparcv9
ZEDBSD_BOARD := sun4u
ZEDBSD_VARIANT := default

CONFIG_KERNEL_TEST_CHECKPOINTS := n
CONFIG_BUF_CACHE_KIB := 0
CONFIG_GPU_JOB_RESERVATION_MS := 10000
CONFIG_GPU_JOB_EXECUTION_MS := 60000
CONFIG_GPU_JOB_STOP_MS := 10000
CONFIG_GPU_CONTROL_MS := 10000
CONFIG_DRIVER_NE2000 := n
CONFIG_DRIVER_PCI_UHCI := n
CONFIG_DRIVER_PCI_EHCI := n
CONFIG_DRIVER_PCI_XHCI := n
CONFIG_DRIVER_PCI_NVME := n
CONFIG_DRIVER_PCI_INTEL_AX211 := n
CONFIG_DRIVER_PCI_VENUS := n
CONFIG_DRIVER_PCI_I915 := n
CONFIG_DRIVER_USB_STORAGE := n
CONFIG_DRIVER_USB_CDC_NCM := n
CONFIG_DRIVER_USB_CDC_ECM := n
CONFIG_DRIVER_USB_HID := n
CONFIG_DRIVER_USB_RTL8822BU := n
CONFIG_DRIVER_GRAPHICS := n
CONFIG_DRIVER_LGY98 := n

ZEDBSD_USER_PROGRAMS := admin ar at awk basename batch bc blkid cal cat cflow chgrp chmod chown cksum cmp comm compress cp crontab csplit curses cut cxref date dd delta df dhcpc diff dirname diskpart dmesg du ed expand expr fetch file find fmt fold fuser gencat get getconf gettext grep halt head host hostname iconv id ifconfig infocmp ipcrm ipcs join kill less link ln locale localedef logger login logname lp lpr ls m4 mailx mesg mkdir mkfifo mkfs mkswap more msgfmt mv newgrp ngettext nice nl nm nohup nslookup ntpdate od paste patch pathchk pax ping poweroff pr prs ps readlink realpath reboot renice rm rmdel rmdir route sact sccs sed service shutdown sleep sort split stat strings stty swapoff swapon sync tabs tail talk tee terminfo tic time timeout top touch tput tr truncate tsort tty uname uncompress unexpand unget uniq unlink uudecode uuencode val wc what who wifi write xargs zcat
