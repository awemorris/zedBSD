...
for setup in gpu gpu3 shm shmband; do
	...
	while [ $run -le "$runs" ]; do
		...
		guest "export XDG_RUNTIME_DIR=/tmp; rm -f /tmp/wayland-0; /bin/zwl --timeout=60 --width=1280 --height=800 > /tmp/perf.log 2>&1 </dev/null & sleep 3; $others sleep 2; /bin/wltest --windowed --size=200x150 --color=0000ff --frames=3600 --token=g > /tmp/g.log 2>&1 </dev/null & sleep 5; a=\$(grep -c 'WLTEST FRAME' /tmp/g.log); sleep 30; b=\$(grep -c 'WLTEST FRAME' /tmp/g.log); c=\$(grep 'PERF compose' /tmp/perf.log | tail -1 | sed 's/.*frame_ms=//'); d=\$(grep 'PERF shm' /tmp/perf.log | tail -1 | sed 's/.*copy_ms=//'); echo \"$setup run=$run gpu_frames_30s=\$((b-a)) compose_frame_ms=\$c shm_copy_ms=\${d:-none}\""
		run=$((run + 1))
	done
done
guest "$stop_all" >/dev/null
