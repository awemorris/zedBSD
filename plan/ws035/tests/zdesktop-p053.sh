...
#
# zdesktop runs at 1280x800; the pointer is moved with QMP (usb-tablet, absolute).
...
guest 'echo "copies: $(grep -c SHM_COPY /tmp/zdesktop.log)"; grep SHM_COPY /tmp/zdesktop.log | sed -n "1,2p;10,12p"' | tee "$out/shm-copies.txt"
guest 'grep SHM_COPY /tmp/zdesktop.log | sed -n 10p' | grep -q 'rows=[0-9]*-[0-9]*' || status=1
