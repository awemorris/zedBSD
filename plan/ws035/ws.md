...
Resume point: fg010: p052・p053 cleared（2 つのモードの核、`wl_shm` と cursor）。p054 は実装・試験済みで受け入れ 3 の読み替えの判断待ち（uncleared）。→ p011 → p055 … → p058。元の続き: タスクバー・タイトル描画
...
| [ws035-p054](phase054/phase.md) | （2026-09-25 承認）acquire fence の request（`zed_gpu_buffer_v1` の拡張） | **uncleared**（q459-i01。受け入れ 1・2・4 は満たした。3 の「present が速くなる」は測って速くならず（present は元から完了を待たない）、判断待ち） | p052 | 同上、libwayland の WSI |
