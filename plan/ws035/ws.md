...
Queue: secondary queue sq001（[queue-secondary.md](../queue-secondary.md)、サブエージェントが実行）
Resume point: fg010: sq001 の順（p052 → … → p058）。元の続き: とタスクバー・タイトル描画
...
| [ws035-p052](phase052/phase.md) | （2026-09-25 承認）2 つのモードの核: 全画面モード（swapchain を破棄してそのまま scanout）とウィンドウモード（背景→窓を下から順に Vulkan で合成、`VK_KHR_display` の swapchain）の切替、quad の pipeline、GPU 画像の OPAQUE_FD import（buffer ごとに 1 回）、sampling の fence と release、効果を載せる枠 | in-progress（sq001-i01） | p051 | `userland/base/zwl` |
