...
状態: 2026-09-07 に方針承認済み。独立した [WS025](ws025-io-memory-cache/ws.md) に詳細設計と Phase を登録。実装 Queue は未選択。

承認時に追加された amd64 の 1 GiB 制限解除、loader の全 RAM 報告、DMA 制約付き allocation は [WS025 の memory 設計](ws025-io-memory-cache/memory-design.md) に記録する。本書は承認時の方針として保持し、細部は同 WS の P-book で具体化する。
...
この段階は [WS024 の単一 64-bit UFS 方針](ws024-unified-ufs/ws.md) に接続する。現 UFS1 で初期 batching と回帰を整えつつ、長期の journal/metadata write-back は統合先 UFS の責任とする。既存 UFS2 の journal/snapshot を評価し、足りない commit/replay/barrier を補う。UFS1 専用の大きな新 journal を作ってから捨てる順序にはしない。
