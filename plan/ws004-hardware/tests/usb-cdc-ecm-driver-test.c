/* WS004 p019 production CDC ECM driver fixture. SPDX-License-Identifier: Zlib */
...
#include "../../../src/drivers/usb/usb-cdc-ecm.c"
...
static uint8_t received_frame[ECM_FRAME_SIZE];
static size_t received_length;
static unsigned submitted_flags;
static uint8_t submitted_endpoint;
...
function_init(struct drv_usb_device *device, unsigned capabilities)
{
	...
	configuration->iads[0].function_class = ECM_COMMUNICATION_CLASS;
	configuration->iads[0].function_subclass = ECM_COMMUNICATION_SUBCLASS;
	configuration->iads[0].function_protocol = ECM_COMMUNICATION_PROTOCOL;
	...
	alternate->descriptor.interface_class = ECM_COMMUNICATION_CLASS;
	alternate->descriptor.interface_subclass = ECM_COMMUNICATION_SUBCLASS;
	alternate->descriptor.interface_protocol = ECM_COMMUNICATION_PROTOCOL;
	...
	alternate->endpoints[0].max_packet = ECM_NOTIFICATION_SIZE;
	...
	alternate->descriptor.interface_class = ECM_DATA_CLASS;
	alternate->descriptor.interface_subclass = ECM_DATA_SUBCLASS;
	alternate->descriptor.interface_protocol = ECM_DATA_PROTOCOL;
	...
	unsigned setting)
{
	...
	if (setting == 1U) {
		...
		struct ecm_adapter *adapter = control->driver_data;
	...
	size_t length, unsigned timeout, size_t *actual)
{
	...
	if (request == ECM_SET_ETHERNET_PACKET_FILTER) {
...
drv_usb_urb_submit(struct drv_usb_urb *urb)
{
	...
	urb->hcd_owned = 1;
	submitted_flags = urb->flags;
	submitted_endpoint = urb->endpoint == NULL ? 0 : urb->endpoint->address;
	submitted_length = urb->length;
	if (urb->endpoint != NULL && urb->endpoint->interface ==
	    &urb->device->configuration.interfaces[1])
		urb->device->data_pending_urbs++;
	pending_urbs++;
	if (urb->device->auto_complete_notification &&
		...
		memset(bytes, 0, 8);
		bytes[0] = 0xa1;
		bytes[1] = ECM_NOTIFICATION_NETWORK_CONNECTION;
		bytes[2] = 1;
...
core_attach_attempt(struct drv_usb_device *device)
{
	...
	int error = ecm_driver.attach(control, &ecm_ids[0]);

	if (error != 0 && ecm_driver.detach(control,
...
{
	int error = ecm_driver.detach(control, flags);
...
static struct ecm_adapter *
attach_function(struct drv_usb_device *device)
{
	struct drv_usb_interface *control = &device->configuration.interfaces[0];

	CHECK(ecm_driver.match(control, &ecm_ids[0]) == 80);
...
test_binding_and_attach(void)
{
	...
	CHECK(ecm_driver.match(&device.configuration.interfaces[0],
	    &ecm_ids[0]) == 0);
	...
	CHECK(device.control_count == 1 && device.controls[0] == 0x101U);
	CHECK(device.configuration.interfaces[1].claimed_by ==
	    &device.configuration.interfaces[0]);
	CHECK(adapter->bulk_out_max_packet_size == 64U &&
	    published_device->mtu == ECM_MTU);
...
test_optional_iad_binding(void)
{
	...
	/* RTL8156's ECM configuration has a CDC Union but no IAD. */
	function_init(&device, DRV_USB_HCD_CAP_CONCURRENT_URBS);
...
test_strict_binding(void)
{
	...
	alternate->extra_count = 2;
	CHECK(ecm_driver.match(control, &ecm_ids[0]) == 0);
	function_init(&device, DRV_USB_HCD_CAP_CONCURRENT_URBS);
	...
	CHECK(ecm_driver.match(control, &ecm_ids[0]) == 0);
	function_init(&device, DRV_USB_HCD_CAP_CONCURRENT_URBS);
	device.configuration.interfaces[0].alternates[0].extras[2] =
	    (const uint8_t[]){13, 0x24, 0x0f, 3, 0, 0, 0, 0,
	    0xe9, 0x05, 0, 0, 0};
	CHECK(ecm_driver.match(&device.configuration.interfaces[0],
	    &ecm_ids[0]) == 0);
	function_init(&device, DRV_USB_HCD_CAP_CONCURRENT_URBS);
	device.configuration.interfaces[0].alternates[0].endpoints[0].max_packet =
	    7U;
	CHECK(ecm_driver.match(&device.configuration.interfaces[0],
	    &ecm_ids[0]) == 0);
	function_init(&device, DRV_USB_HCD_CAP_CONCURRENT_URBS);
	device.configuration.interfaces[1].alternates[1].endpoints[1].max_packet =
	    0U;
	CHECK(ecm_driver.match(&device.configuration.interfaces[0],
	    &ecm_ids[0]) == 0);
	function_init(&device, DRV_USB_HCD_CAP_CONCURRENT_URBS);
	device.configuration.interfaces[0].alternates[0].endpoints[0].type =
	    DRV_USB_TRANSFER_BULK;
	CHECK(ecm_driver.match(&device.configuration.interfaces[0],
	    &ecm_ids[0]) == 0);
	function_init(&device, DRV_USB_HCD_CAP_CONCURRENT_URBS);
	data = &device.configuration.interfaces[1];
	data->alternates[0].descriptor.endpoint_count = 1U;
	data->alternates[0].endpoint_count = 1U;
	CHECK(ecm_driver.match(&device.configuration.interfaces[0],
	    &ecm_ids[0]) == 0);
	function_init(&device, DRV_USB_HCD_CAP_CONCURRENT_URBS);
	data = &device.configuration.interfaces[1];
	data->alternates[1].endpoints[1].address = 0x85U;
	CHECK(ecm_driver.match(&device.configuration.interfaces[0],
	    &ecm_ids[0]) == 0);
	function_init(&device, DRV_USB_HCD_CAP_CONCURRENT_URBS);
	data = &device.configuration.interfaces[1];
	data->alternates[0].descriptor.interface_protocol = 1U;
	CHECK(ecm_driver.match(&device.configuration.interfaces[0],
	    &ecm_ids[0]) == 0);
	function_init(&device, DRV_USB_HCD_CAP_CONCURRENT_URBS);
	...
	CHECK(hal_live == 0);
	function_init(&device, DRV_USB_HCD_CAP_CONCURRENT_URBS);
	strcpy(device.mac, "010000000001");
	CHECK(core_attach_attempt(&device) == EINVAL);
	CHECK(device.configuration.interfaces[1].claimed_by == NULL &&
	    hal_live == 0);
	function_init(&device, DRV_USB_HCD_CAP_CONCURRENT_URBS);
	strcpy(device.mac, "000000000000");
	CHECK(core_attach_attempt(&device) == EINVAL);
	CHECK(device.configuration.interfaces[1].claimed_by == NULL &&
	    hal_live == 0);
	function_init(&device, DRV_USB_HCD_CAP_CONCURRENT_URBS);
...
static void
test_rndis_first_ecm_second_match(void)
{
	struct drv_usb_device rndis, ecm;
	struct drv_usb_interface *control;

	function_init(&rndis, DRV_USB_HCD_CAP_CONCURRENT_URBS);
	control = &rndis.configuration.interfaces[0];
	control->descriptor.interface_class = 0xe0U;
	control->descriptor.interface_subclass = 0x01U;
	control->descriptor.interface_protocol = 0x03U;
	control->alternates[0].descriptor = control->descriptor;
	CHECK(ecm_driver.match(control, &ecm_ids[0]) == 0);
	function_init(&ecm, DRV_USB_HCD_CAP_CONCURRENT_URBS);
	CHECK(ecm_driver.match(&ecm.configuration.interfaces[0],
	    &ecm_ids[0]) == 80);
}
...
test_io_and_lifetime(void)
{
	...
	uint8_t frame[65], storage_buffer[64];
	uint64_t rx_errors;
	unsigned before_frees, received_before, iteration;
	...
	adapter->notification_buffer[1] = ECM_NOTIFICATION_NETWORK_CONNECTION;
	...
	memcpy(adapter->rx_buffer, frame, 64U);
	fake_complete(adapter->rx_urb, DRV_USB_URB_COMPLETE, 64U);
	CHECK(net_device_poll(published_device, 1) == 1);
	CHECK(received_packets == received_before + 1U && received_length == 64U);
	CHECK(!memcmp(received_frame, frame, 64U));
	CHECK(adapter->rx_urb->status == DRV_USB_URB_PENDING);
	rx_errors = published_device->rx_errors;
	fake_complete(adapter->rx_urb, DRV_USB_URB_COMPLETE,
	    ECM_ETHERNET_HEADER_SIZE - 1U);
	CHECK(net_device_poll(published_device, 1) == 1);
	CHECK(published_device->rx_errors == rx_errors + 1U &&
	    received_packets == received_before + 1U);
	memset(adapter->notification_buffer, 0, 16);
	adapter->notification_buffer[0] = 0xa1;
	adapter->notification_buffer[1] = ECM_NOTIFICATION_SPEED_CHANGE;
	put_le16(adapter->notification_buffer + 4U, 1U);
	...
	CHECK(packet != NULL && packet_buf_append(packet, 64U) != NULL);
	memcpy(packet->data, frame, 64U);
	...
	CHECK(packet_frees == before_frees + 1U && adapter->tx_busy &&
	    submitted_endpoint == 0x05U && submitted_length == 64U &&
	    submitted_flags == DRV_USB_URB_ZERO_PACKET &&
	    !memcmp(adapter->tx_buffer, frame, 64U));
	packet = packet_buf_alloc(0);
	CHECK(packet != NULL && packet_buf_append(packet, 64U) != NULL);
	...
	CHECK(packet != NULL && packet_buf_append(packet,
	    ECM_FRAME_SIZE + 1U) != NULL);
	CHECK(net_device_transmit(published_device, packet) == EMSGSIZE);
	packet = packet_buf_alloc(0);
	CHECK(packet != NULL && packet_buf_append(packet,
	    ECM_ETHERNET_HEADER_SIZE - 1U) != NULL);
	CHECK(net_device_transmit(published_device, packet) == EMSGSIZE);
	packet = packet_buf_alloc(0);
	CHECK(packet != NULL && packet_buf_append(packet, 65U) != NULL);
	memcpy(packet->data, frame, 65U);
	...
	memcpy(packet->data, frame, 65U);
	CHECK(net_device_transmit(published_device, packet) == 0);
	CHECK(submitted_endpoint == 0x05U && submitted_length == 65U &&
	    submitted_flags == 0U && !memcmp(adapter->tx_buffer, frame, 65U));
	fake_complete(adapter->tx_urb, DRV_USB_URB_COMPLETE,
	    adapter->tx_urb->length);
	CHECK(net_device_poll(published_device, 8) == 1);
	net_device_close(published_device);
...
test_poll_budget_and_completion_storm(void)
{
	...
	CHECK(net_device_transmit(published_device, packet) == 0);
	memcpy(adapter->rx_buffer, frame, sizeof(frame));
	memset(adapter->notification_buffer, 0, 8);
	adapter->notification_buffer[0] = 0xa1;
	adapter->notification_buffer[1] = ECM_NOTIFICATION_NETWORK_CONNECTION;
	adapter->notification_buffer[2] = 1;
	fake_complete(adapter->notification_urb, DRV_USB_URB_COMPLETE, 8);
	fake_complete(adapter->rx_urb, DRV_USB_URB_COMPLETE, sizeof(frame));
	fake_complete(adapter->tx_urb, DRV_USB_URB_COMPLETE,
	    adapter->tx_urb->length);
	received_before = received_packets;
	...
	CHECK(adapter->notification_ready == 0 && adapter->rx_ready != 0 &&
	    ...
	    published_device->poll_scheduled != 0);
	CHECK(net_device_poll(published_device, 1) == 1);
	CHECK(adapter->rx_ready == 0 && adapter->rx_rearm == 0 &&
	    received_packets == received_before + 1U && pending_urbs == 2 &&
	    received_length == sizeof(frame));

	/* Terminal transfer failures each consume one unit and rearm exactly once. */
...
test_rearm_retry_and_quarantine(void)
{
	...
	device.submit_error_count = ECM_REARM_RETRY_MAX + 1U;
	for (unsigned attempt = 0; attempt < ECM_REARM_RETRY_MAX; attempt++) {
		CHECK(net_device_poll(published_device, 8) == 1);
...
test_shutdown(void)
{
	...
	ecm_driver.shutdown(&device.configuration.interfaces[0]);
...
test_registry_ninth_bind(void)
{
	...
	struct ecm_adapter *adapters[NET_DEVICE_MAX];
...
test_independent_storage_during_detach(void)
{
	...
	/* Normal class unbind retires only the ECM function.  An accepted request
...
main(void)
{
	CHECK(drv_usb_cdc_ecm_driver_register() == 0);
	CHECK(registered_driver == &ecm_driver);
	test_binding_and_attach();
	test_optional_iad_binding();
	test_strict_binding();
	test_rndis_first_ecm_second_match();
	...
	printf("usb-cdc-ecm driver fixture: PASS (%u checks)\n", checks);
	return 0;
}
