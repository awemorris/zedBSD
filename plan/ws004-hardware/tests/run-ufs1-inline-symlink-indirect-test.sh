...
temporary=$(mktemp -d "${TMPDIR:-/tmp}/zedbsd-ufs-symlink-indirect.XXXXXX")
...
# Twenty full filesystem blocks plus a tail forces the UFS single-indirect
...
PYTHONPATH="$repo/tools/build" python3 \
	"$repo/tools/build/check-ufs-image.py" "$image"
...
fs = checker.UFS(image.read_bytes())
...
assert fs.u64(raw_inode, 16) == payload.stat().st_size
assert all(fs.u64(raw_inode, 112 + index * 8) for index in range(12))
assert fs.u64(raw_inode, 208) != 0
assert fs.u64(raw_inode, 216) == 0
assert fs.u64(raw_inode, 224) == 0
...
echo 'UFS inline symlink and single-indirect image: PASS'
