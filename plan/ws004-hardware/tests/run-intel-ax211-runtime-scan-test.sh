...
build_dir=$(mktemp -d "${TMPDIR:-/tmp}/zedbsd-intel-ax211-runtime-scan.XXXXXX")
trap 'rm -rf "$build_dir"' EXIT HUP INT TERM

cc=${CC:-cc}
warnings="-std=c11 -Wall -Wextra -Werror"
sources="$repo_root/src/drivers/wifi/intel-ax211/intel-ax211-protocol.c"
sources="$sources $repo_root/src/drivers/wifi/intel-ax211/intel-ax211-runtime.c"
sources="$sources $repo_root/src/drivers/wifi/intel-ax211/intel-ax211-scan.c"
fixture="$test_dir/intel-ax211-runtime-scan-test.c"
...
# AX211 is amd64-only, but its private wire/state codecs remain ILP32-clean.
# shellcheck disable=SC2086
$cc $warnings -m64 -nostdinc $abi_includes -fsyntax-only $sources
# shellcheck disable=SC2086
$cc $warnings -m32 -nostdinc $abi_includes -fsyntax-only $sources

echo "intel ax211 runtime/scan: ordinary, real API89, ASan/UBSan, analyzer, amd64/i386 syntax PASS"
