...
| --- | --- | --- | --- | --- |
| 1 | `ws012-p003` | [WS012](../ws012/ws.md), [Phase](../ws012/phase003/phase.md), [tests](../ws012/tests/README.md) | completed | PID 1 and every installed service/shutdown client use bounded, terminated ZSV1 service and system-action records with no display-text parsing or unversioned fallback |
| 2 | `ws012-p004` | [WS012](../ws012/ws.md), [Phase](../ws012/phase004/phase.md), [tests](../ws012/tests/README.md) | completed | The argv CLI has fixed grammar/output/exit status, separates runtime control from locked persistent policy, and uses typed ZSV1 state |
| 3 | `ws012-p005` | [WS012](../ws012/ws.md), [Phase](../ws012/phase005/phase.md), [tests](../ws012/tests/README.md) | completed | Argument-free service enters the bounded native console and reuses the p004 dispatcher without candidate/save state |
| 4 | `ws012-p006` | [WS012](../ws012/ws.md), [Phase](../ws012/phase006/phase.md), [tests](../ws012/tests/README.md) | completed | Focused fixtures, production amd64 QEMU behavior, persistence/failure/concurrency cases, fatal-log scan, and public documentation prove the complete WS012 contract |
...
1. Execute [p003](../ws012/phase003/phase.md):
...
2. Execute [p004](../ws012/phase004/phase.md):
...
3. Execute [p005](../ws012/phase005/phase.md):
...
4. Execute [p006](../ws012/phase006/phase.md):
...
[the q018 p003 QEMU evidence](../ws012/tests/q018-p003-qemu-evidence.md).
...
used. The following p004-p006 items also completed before q018 closure.
...
`make check` was not run and `.internal/` was not used. The following p005-p006
items also completed before q018 closure.
...
`ws012-p006` completed on 2026-08-28 and closed WS012. The consolidated host
runner passed the model, persistence, protocol, client, server, shutdown-argv,
command, and console fixtures under both strict C17 and ASan/UBSan. The
production `make -j16` build passed.

The production QEMU runner passed initial YAML boot, LIST/SHOW detail,
runtime-only actions, the complete argument-free console and error recovery,
policy-only changes, malformed reload preservation, persistence across one
ZSV1 reboot, explicit start, and final reboot/halt/poweroff handoff. Guest and
QEMU/HMP fatal scans were empty; the original image and `config.mk` hashes were
unchanged. Exact evidence is in the
[p006 result matrix](../ws012/tests/q018-p006-results.tsv) and
[integration record](../ws012/tests/q018-p006-qemu-evidence.md).

Integration repaired deterministic image data modes and the `/bin/sh` race in
which a direct foreground external could read the TTY before foreground
ownership and stop on `SIGTTIN`. The general foreground-pipeline and `fg`
ordering residual is isolated as
[`ws001-p014`](../ws001/phase014/phase.md). Formatting,
DOC-T00, and `git diff --check` passed. `make check` was not run and
