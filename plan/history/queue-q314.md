...
# Queue q314: WS029 i915ネイティブGPU driver

<!-- awesome-plan-current:start -->
Status: finished
Active Queue: q314
Executor: q314-root
Item: q314-i01-i07 cleared; Queue finished
Last Queue: q313 finished / ws014-p010 cleared
<!-- awesome-plan-current:end -->

Authorization (planning): current user: WS029のi915 GPUドライバ作成に移りたいです。…まずはドライバの移植を行って、ビルドを通し、静的レビューを完了させたあと、このテストループ環境を作って、動くところまで持って行ってください。これを遂行するための計画を、ws029にphaseとして展開してください。まず計画だけ行います。…計画だけ行い、queueを作成してください。
Prepared UTC: 2026-09-13T18:44:19.735602+00:00
Start UTC: 2026-09-14T01:09:03.693295+00:00
Authorization (start): current user: ファームウェアが必要な場合は userland/firmware/ に機種ごとに追加します。では、実装してください。（2026-09-14、q314 計画の提示後）
Timebox: 1440 active minutes estimate; review every 120 active minutes. fixture 120秒、build/転送 1200秒、VM 300秒、host attach/restore 各 120秒。同条件無変更 retry 3回まで。
Stop conditions: 参照ファイルに非MITが混じる、host の i915/GDM 復旧が失敗、HAL変更が必要になる、ユーザー承認が必要な操作（reboot等）に達する。

| Order | Attempt | Phase | Status | Scope |
| --- | --- | --- | --- | --- |
| 1 | q314-i01 | [ws029-p001](https://github.com/awemorris/zedBSD/issues/398) | cleared | 対象確定・ライセンス境界・移植方針の固定（120 分） |
| 2 | q314-i02 | [ws029-p002](https://github.com/awemorris/zedBSD/issues/399) | cleared | driver骨格: PCI attach、MMIO/forcewake、GGTT、割込み（240 分） |
| 3 | q314-i03 | [ws029-p003](https://github.com/awemorris/zedBSD/issues/400) | cleared | メモリ: GEM object、48-bit PPGTT、CPU view（180 分） |
| 4 | q314-i04 | [ws029-p004](https://github.com/awemorris/zedBSD/issues/401) | cleared | 実行: engine/LRC/execlists、request/seqno、engine reset（300 分） |
| 5 | q314-i05 | [ws029-p005](https://github.com/awemorris/zedBSD/issues/402) | cleared | drv_gpu統合、native stream、生UAPI試験クライアント（240 分） |
| 6 | q314-i06 | [ws029-p006](https://github.com/awemorris/zedBSD/issues/403) | cleared | VFIO passthroughテストループ（host手順・harness・初回起動）（180 分） |
| 7 | q314-i07 | [ws029-p007](https://github.com/awemorris/zedBSD/issues/404) | cleared | 実機での描画確認、静的レビュー、規約全文確認（180 分） |

## 依存と実行境界

ws014-p010 cleared → q314-i01（p001 承認・監査）→ i02 骨格 → i03 メモリ → i04 実行 → i05 統合/試験client → i06 VFIO loop → i07 実機受入・レビュー。各 item は前 item の cleared を前提とし、順序を飛ばさない。p004（WS014）と表示/scanout は含めない。

q313履歴は plan/history/queue-q313.md。[Guardrail](https://github.com/awemorris/zedBSD/issues/363)、plan/coding-style.md全文、make -j16対象buildを使用。HAL/UAPI不変、private host転送の既存許可、本指示による host GDM停止/i915 unbind/VFIO passthrough の許可、GitHub同期の既存承認。git公開はユーザー担当。
