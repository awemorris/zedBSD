...
copies, sockets, and serial logs belong under `plan/ws008/temp/` and stay
...
make download
make -j16 toolchain
plan/ws008/tests/noct-host-path-contract.sh
plan/ws008/tests/run-zedbuild-byte-primitives.sh
plan/ws008/tests/qemu-noct-smoke.sh
plan/ws008/tests/qemu-noct-jit.sh
plan/ws008/tests/qemu-beui-zedbsd.sh
```
...
```sh
plan/ws008/tests/noct-p001-host-acceptance.sh --mode all
```
...
```sh
plan/ws008/tests/noct-p001-host-acceptance.sh \
...
new directory below `plan/ws008/temp/` by default. Override the canonical
...
`plan/ws008/temp/q020-p002-beui.XXXXXX/` directory and may be deleted; the
...
`plan/ws008/temp/q020-p003-jit.XXXXXX/` directory and may be deleted; the
...
```sh
plan/ws008/tests/noct-p004-review.sh
```
...
```sh
plan/ws008/tests/noct-p005-backends.sh
```
...
[the Phase execution evidence](../phase006/evidence.md).
...
```sh
bash plan/ws008/tests/noct-host-path-contract.sh
```

The runner retains a bounded result directory under
`plan/ws008/temp/`. `NOCT-T083` remains the existing
`plan/ws010/tests/toolchain-smoke.noct`; `NOCT-T084` is the separate
