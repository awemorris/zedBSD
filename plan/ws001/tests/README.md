...
```sh
plan/ws001/tests/shell-job-control-test.sh
```
...
is written below the ignored `plan/ws001/temp/` directory. The test does
...
```sh
plan/ws001/tests/qemu-shell-job-control.sh
```

Its optional output-directory argument must name a new path. The default is a
new ignored directory below `plan/ws001/temp/`; only a disposable image
copy is writable.
