...
sources=['src/kern/writeback.c','plan/ws025-io-memory-cache/tests/writeback-credit-host.c']
(out/'source.json').write_text(json.dumps({s:hashlib.sha256((repo/s).read_bytes()).hexdigest() for s in [*sources,"include/kern/writeback.h"]},indent=2)+'\n')
commands=[]
for variant in ('ordinary','sanitize'):
 ...
 build=['cc','-std=c11','-O1','-g','-Wall','-Wextra','-Werror','-DZEDBSD_USER_ABI_LP64','-DZEDBSD_STORAGE_HOST_TEST','-ffunction-sections','-fdata-sections','-I.','-Iinclude','-Iinclude/uapi','-Isrc',*extra,*sources,bridge,'-pthread','-Wl,--gc-sections','-o',binary]
