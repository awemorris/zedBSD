...
for mode in ('ordinary','sanitize'):
    ...
    binary=str(out/mode)
    run(mode+'-build',['cc','-std=c11','-O1','-g','-Wall','-Wextra','-Werror','-DZEDBSD_USER_ABI_LP64','-ffunction-sections','-fdata-sections','-I.','-Iinclude','-Iinclude/uapi','-Isrc','-Ilibc/include','-I'+str(out),*extra,'plan/ws025-io-memory-cache/tests/cache-buffer-host.c', 'src/kern/cache-memory.c', 'src/kern/io-stats.c',bridge,'-pthread','-Wl,--gc-sections','-o',binary])
