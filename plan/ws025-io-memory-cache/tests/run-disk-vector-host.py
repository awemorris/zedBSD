...
for mode in ('ordinary','sanitize'):
 ...
 binary=out/mode
 args=['cc','-std=c11','-O1','-g','-Wall','-Wextra','-Werror','-ffunction-sections','-fdata-sections','-DZEDBSD_USER_ABI_LP64','-DZEDBSD_STORAGE_HOST_TEST','-I.','-Iinclude','-Iinclude/uapi','-Isrc','-Ilibc/include',*flags,'plan/ws025-io-memory-cache/tests/disk-vector-host.c','src/drivers/disklabel/mbr.c','src/kern/io-stats.c','src/kern/io-error.c','-Wl,--gc-sections','-o',str(binary)]
