...
spec = importlib.util.spec_from_file_location('storage', REPO/'plan/ws019-installation/tests/run-storage-qemu.py')
...
def main():
    out = Path(sys.argv[1]).resolve(); out.relative_to(REPO/'plan/ws025-io-memory-cache/temp')
    ...
    try:
        for depth in depths:
            ...
            command=['make','-j16','-f','Makefile','-f','plan/ws025-io-memory-cache/tests/nvme-pipeline.mk','ZEDBSD_CONFIG=config/ci/config-amd64.mk',f'BUILD={private}', f'ARCH_IMAGE_DIR={arches}', f'ZEDBSD_TEST_CPPFLAGS=-DNVME_IO_PIPELINE_DEPTH={depth}', str(private/'vmunix'), 'ws025-nvme-fixture']
