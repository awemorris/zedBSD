#!/usr/bin/env python3
"""Production policy and dirty ownership with controlled worker scheduling."""
...
(out/'writeback-sysctl.inc').write_text(control[start:])
commands=[]
for variant in ('ordinary','sanitize'):
 ...
 bridges=[]
 for name,path in [('thread','plan/ws018-kernel-architecture/tests/mount-thread-host.c'),('exit','plan/ws025-io-memory-cache/tests/async-thread-host.c')]:
 ...
 build=['cc','-std=c11','-O1','-g','-Wall','-Wextra','-Werror','-DZEDBSD_USER_ABI_LP64','-ffunction-sections','-fdata-sections','-I.','-Iinclude','-Iinclude/uapi','-Isrc','-Ilibc/include','-I'+str(out),*extra,*sources,*bridges,'-pthread','-Wl,--gc-sections','-o',binary]
