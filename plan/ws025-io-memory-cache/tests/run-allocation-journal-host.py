...
for variant in ('sanitize','ordinary'):
 ...
 binary=out/variant
 subprocess.run(['cc','-DZEDBSD_USER_ABI_LP64',*extra,'-std=c11','-O1','-g','-Wall','-Wextra','-Werror','-ffunction-sections','-fdata-sections','-I.','-Iinclude','-Iinclude/uapi','-Isrc','-Ilibc/include','-Iplan/ws018-kernel-architecture/tests','-Isrc/drivers/fs/ufs','plan/ws025-io-memory-cache/tests/'+fixture,'src/drivers/fs/ufs/ufs-endian.c','src/drivers/fs/ufs/ufs-journal.c','src/kern/quota.c','src/kern/io-stats.c',*extra_sources,str(bridge),str(jump),'-pthread','-Wl,--gc-sections','-o',str(binary)],cwd=repo,check=True)
