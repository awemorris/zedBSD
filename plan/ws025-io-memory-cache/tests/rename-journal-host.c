...
static unsigned rename_number(uint64_t fragment,const char *name)
{
 ...
 while(pos<512) {
  length=drv_ufs_get16(p,pos+4,0);REQUIRE(length>=8 && !(length&3) && length<=512-pos);
  if(drv_ufs_get32(p,pos,0) && p[pos+7]==strlen(name) && !memcmp(p+pos+8,name,strlen(name)))return drv_ufs_get32(p,pos,0);
  pos+=length;
 }
 return 0;
}
static void rename_write_check(uint64_t first,uint32_t count,const void *buffer)
{
 ...
 if(first==176 || first==184 || first==160 || first==8 || first==16) {
  ...
  REQUIRE(drv_ufs_get32(namespace_journal->image,16,0)==rename_extents);
...
static void rename_scenario(unsigned cross,unsigned directory,unsigned replacing,unsigned shared,unsigned write_fail,unsigned flush_fail,unsigned landed,unsigned stop)
{
 ...
 old_links=drv_ufs_get16(rename_raw(&old.inode),UFS_DI_NLINK,0);
 new_links=drv_ufs_get16(rename_raw(rename_new),UFS_DI_NLINK,0);
 ...
 if(replacing){target_links=drv_ufs_get16(rename_raw(&target.inode),UFS_DI_NLINK,0);REQUIRE(target_links==(committed?0:(directory?2U:1U)));}
 REQUIRE(drv_ufs_get16(rename_raw(&node.inode),UFS_DI_NLINK,0)==(directory?2:1));
 REQUIRE(drv_ufs_get64(rename_raw(&node.inode),UFS_DI_DB,0)==160);
...
static void rename_refusals(void)
{
 ...
 drv_ufs_put16(block,4,16,0);drv_ufs_put16(block,20,4,0);
 REQUIRE(directory_image_change(&old.inode,block,&victim,2,0,0)==EIO);
 REQUIRE(drv_ufs_get32(block,0,0)==2);
