...
int __real_hal_pmem_alloc(const struct hal_pmem_request *,struct hal_pmem *);
int __real_io_scratch_alloc(size_t,int,struct io_scratch *);
int __wrap_hal_pmem_alloc(const struct hal_pmem_request *request,struct hal_pmem *memory)
{
 if(force_fragmentation && request->size>=65536)return HAL_ERR_NOMEM;
 return __real_hal_pmem_alloc(request,memory);
}
...
{
 struct hal_vmap *map;struct hal_memory_stats before,after;
 ...
 for(table_failure=1;table_failure<=2;table_failure++){
  hal_memory_get_stats(&before);REQUIRE(hal_vmap_reserve(65536,&map)==HAL_OK);
  table_calls=0;allocating=1;REQUIRE(hal_vmap_populate(map,0,UINT64_MAX)==HAL_ERR_NOMEM);allocating=0;
  REQUIRE(hal_vmap_release(map)==HAL_OK);
  hal_memory_get_stats(&after);REQUIRE(before.physical_free==after.physical_free);
  hal_printf("VMAP TABLE ROLLBACK PASS step=%u\n",table_failure);
 }
 table_failure=0;
 first=hal_mem_create_space();second=hal_mem_create_space();REQUIRE(first && second);
 for(round=0;round<3;round++){
  hal_memory_get_stats(&before);
  REQUIRE(hal_vmap_reserve(65536,&map)==HAL_OK);
  hal_memory_get_stats(&after);REQUIRE(before.physical_free==after.physical_free);
  allocating=1;REQUIRE(hal_vmap_populate(map,0,UINT64_MAX)==HAL_OK);allocating=0;
  REQUIRE(hal_vmap_pin(map,&address)==HAL_OK);
  REQUIRE(hal_vmap_release(map)==HAL_ERR_BUSY);
  previous=0;
  for(index=0;index<16;index++){
   REQUIRE(hal_kernel_page_lookup((char *)address+index*4096,&pa)==HAL_OK);
  ...
  hal_page_switch_space(first);REQUIRE(probe_address[0]==probe_value);
  hal_page_switch_space(second);REQUIRE(probe_address[65535]==probe_value);
  hal_page_switch_space(HAL_SPACE_SYS);
  new_space=hal_mem_create_space();REQUIRE(new_space!=NULL);
  hal_page_switch_space(new_space);REQUIRE(probe_address[4096]==probe_value);
  hal_page_switch_space(HAL_SPACE_SYS);hal_page_destroy_space(new_space);
  ...
  hal_vmap_unpin(map);REQUIRE(hal_vmap_release(map)==HAL_OK);
  REQUIRE(hal_kernel_page_lookup(address,&pa)==HAL_ERR_INVALID);
  hal_memory_get_stats(&after);REQUIRE(before.physical_free==after.physical_free);
 ...
 hal_page_destroy_space(first);hal_page_destroy_space(second);
 __real_io_pool_init();
}
