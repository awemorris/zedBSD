...
既存の [BR-T24 high-memory boot](../ws003-bringup/tests/uefi-high-memory-usb-boot.sh) は 4/8/16 GiB 設定の boot と高位 RSDP を確認する。allocator がその RAM を使った証拠ではないため、本 WS は別に高位 PFN の実使用を検証する。
