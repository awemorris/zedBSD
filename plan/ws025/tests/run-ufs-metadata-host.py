...
for variant in ("ordinary", "sanitize"):
    ...
    run("counter-build-" + variant, [*flags, "-pthread",
        "plan/ws025/tests/io-stats-host.c", "src/kern/io-stats.c",
        "-o", str(binary)])
    ...
    for version in (2,):
        ...
        run(f"ufs{version}-build-{variant}", [*flags,
            ...
            *([] if variant == "ordinary" else ["--param", "asan-globals=0"]),
            "plan/ws025/tests/ufs-metadata-host.c",
            "plan/ws025/temp/p031-driver-fragments/src/drivers/fs/ufs/ufs-endian.c",
...
print("WS025 UFS metadata host gates PASS")
