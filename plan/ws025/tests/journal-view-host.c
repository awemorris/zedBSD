...
#include "plan/ws018/tests/mount-thread-host.h"
...
{ checkpoint_result=drv_ufs_journal_checkpoint(context); }
static void check_copy(struct ufs_journal_view *view)
{
 uint8_t out[1536];unsigned n;
 memset(out,0xcc,sizeof(out));
 REQUIRE(drv_ufs_journal_view_copy(view,7,2,out)==ENOENT);
 for(n=0;n<sizeof(out);n++)REQUIRE(out[n]==0xcc);
 REQUIRE(drv_ufs_journal_view_copy(view,8,3,out)==0);
 for(n=0;n<1024;n++)REQUIRE(out[n]==0x31);
 for(;n<sizeof(out);n++)REQUIRE(out[n]==0x72);
 REQUIRE(drv_ufs_journal_view_copy(view,38,2,out)==EINVAL);
}
static void racing_reader(void *context)
{
 ...
 while(!__atomic_load_n(&stop_readers,__ATOMIC_ACQUIRE)) {
  error=drv_ufs_journal_view_acquire(&journal,&view);
  REQUIRE(error==0 || error==ENOENT);
  if(!error){check_copy(&view);drv_ufs_journal_view_release(&view);}
...
static void scenario(unsigned failure)
{
 ...
 REQUIRE(drv_ufs_journal_init(&journal,&io,40,12,39)==0);
 REQUIRE(drv_ufs_journal_bind_image(&journal,image,sizeof(image))==0);
 REQUIRE(drv_ufs_journal_view_acquire(&journal,&pinned)==ENOENT);
 REQUIRE(drv_ufs_journal_publishv(&journal,extents,2)==0);
 REQUIRE(drv_ufs_journal_view_acquire(&journal,&pinned)==0 && pinned.sequence==1);
 REQUIRE(drv_ufs_journal_view_acquire(&journal,&pinned)==EBUSY);
 REQUIRE(drv_ufs_journal_views_busy(&journal));check_copy(&pinned);
 ...
 REQUIRE(drv_ufs_journal_view_acquire(&journal,&other)==0);check_copy(&other);
 REQUIRE(reads==saved_reads && writes==saved_writes && flushes==saved_flushes);
 drv_ufs_journal_view_release(&other);
 ...
 if(failure)REQUIRE(drv_ufs_journal_replay(&journal)==0);
 ...
 REQUIRE(drv_ufs_journal_view_acquire(&journal,&other)==ENOENT);
 saved_reads=reads;saved_writes=writes;saved_flushes=flushes;
 REQUIRE(drv_ufs_journal_bind_image(&journal,NULL,0)==EBUSY);
 REQUIRE(drv_ufs_journal_publishv(&journal,extents,2)==EBUSY);
 REQUIRE(drv_ufs_journal_replay(&journal)==EBUSY);
 REQUIRE(reads==saved_reads && writes==saved_writes && flushes==saved_flushes);
 check_copy(&pinned);drv_ufs_journal_view_release(&pinned);drv_ufs_journal_view_release(&pinned);
 REQUIRE(!drv_ufs_journal_views_busy(&journal));
 REQUIRE(drv_ufs_journal_publishv(&journal,extents,2)==0);
 REQUIRE(drv_ufs_journal_view_acquire(&journal,&other)==0 && other.sequence==2);
 REQUIRE(drv_ufs_journal_view_copy(&other,8,2,a)==0 && a[0]==0x99);
 drv_ufs_journal_view_release(&other);REQUIRE(drv_ufs_journal_checkpoint(&journal)==0);
 REQUIRE(drv_ufs_journal_bind_image(&journal,NULL,0)==0);
}
int main(void)
{
