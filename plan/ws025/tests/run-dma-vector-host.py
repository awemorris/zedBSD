...
for variant in ('ordinary','sanitize'):
 ...
 binary=out/variant
 args=['cc','-std=c11','-g','-Dtid_t=int32_t','-ffunction-sections','-fdata-sections','-Wl,--gc-sections','-Wall','-Wextra','-Werror','-Iinclude','-Iinclude/uapi','-I.',*flags,'plan/ws025/tests/dma-vector-host.c','src/drivers/generic/dma.c','src/hal/amd64/pmem-range.c','src/kern/cache.c','src/kern/io.c','-pthread','-o',str(binary)]
