...
sources=['src/drivers/disklabel/mbr.c','src/kern/io-stats.c','src/kern/io-error.c','plan/ws025/tests/flush-frontier-host.c']
(out/'source.json').write_text(json.dumps({s:hashlib.sha256((repo/s).read_bytes()).hexdigest() for s in [*sources,"src/kern/disk.c","src/kern/mount.c","include/kern/disk.h","include/kern/mount.h","include/kern/io-epoch.h","plan/ws019/tests/storage-foundation-test.c"]},indent=2)+'\n')
commands=[]
for variant in ('ordinary','sanitize'):
 ...
 build=['cc','-std=c11','-O1','-g','-Wall','-Wextra','-Werror','-DKERN_USER_ABI_LP64','-DZEDBSD_STORAGE_HOST_TEST','-ffunction-sections','-fdata-sections','-I.','-Iinclude','-Iinclude/uapi','-Isrc','-Ilibc/include',*extra,*sources,bridge,'-pthread','-Wl,--gc-sections','-o',binary]
