/* Final creation name and child/parent links with real redo and name cache. */
...
static struct inode *creation_target;
static unsigned creation_shared,creation_snapshot;
static int creation_result;
static unsigned creation_directory;
static struct ufs_transaction_outcome creation_outcome;
static void creation_write_check(uint64_t first,uint32_t count,const void *buffer)
{
 unsigned expected=creation_shared?12:268;
...
{
 mutex_lock(&((struct ufs_mount_state *)((struct inode *)argument)->i_mount->m_data)->namespace_lock);
 creation_result=creation_group(argument,&added,creation_target,&creation_outcome);
 mutex_unlock(&((struct ufs_mount_state *)((struct inode *)argument)->i_mount->m_data)->namespace_lock);
 if(creation_result==0)inode_dir_changed(argument);
}
...
static void creation_scenario(unsigned shared,unsigned layout,unsigned write_fail,unsigned flush_fail,unsigned landed,unsigned stop)
{
 ...
 node.inode.i_ino=3;node.inode.i_linkcount=0;
 if(creation_directory){node.inode.i_type=INODE_DIR;node.inode.i_mode=S_IFDIR|0700;}
 REQUIRE(persist_inode(&node.inode)==0);
 memset(&directory,0,sizeof(directory));directory.inode.i_mount=&mountp;
 directory.inode.i_ino=shared?4:18;directory.inode.i_type=INODE_DIR;directory.inode.i_mode=S_IFDIR|0755;
 ...
 crash_cut=stop;crash_ops=crashed=0;creation_result=EIO;
 (void)host_crash_run(perform_link,&directory.inode);
 if(crashed)for(n=0;n<32;n++)if(owned[n])kern_free(owned[n]);
 REQUIRE(creation_result==0 || creation_result==EIO || (media_override_error && creation_result==media_override_error));
 if(!crashed) {
  REQUIRE(fs.journal_io.context==NULL && fs.snapshot_io.context==NULL);
  REQUIRE(node.inode.i_linkcount==(fs.journal.committed_sequence?(creation_directory?2:1):0));
  REQUIRE(directory.inode.i_linkcount==2U+(creation_directory && fs.journal.committed_sequence!=0));
...
int main(void)
{
 ...
 for(creation_directory=0;creation_directory<2;creation_directory++)for(shared=0;shared<2;shared++)for(layout=0;layout<4;layout++) {
  creation_scenario(shared,layout,0,0,0,0);
  for(mode=0;mode<2;mode++)for(n=1;n<=20;n++)creation_scenario(shared,layout,n,0,mode,0);
  for(n=1;n<=12;n++)creation_scenario(shared,layout,0,n,0,0);
  for(n=1;n<=20;n++){second_failure=n+1;creation_scenario(shared,layout,n,0,0,0);}second_failure=0;
  for(mode=0;mode<2;mode++){crash_mode=mode;for(n=1;n<=28;n++)creation_scenario(shared,layout,0,0,0,n);}
  crash_mode=0;creation_snapshot=1;snapshot_fail=0;creation_scenario(shared,layout,0,0,0,0);
  for(n=1;n<=(shared?2U:3U);n++){snapshot_fail=n;creation_scenario(shared,layout,0,0,0,0);}creation_snapshot=snapshot_fail=0;
  media_override_error=EOPNOTSUPP;creation_scenario(shared,layout,0,1,0,0);media_override_error=0;
 }
 printf("UFS grouped creation name/child/parent recovery: PASS (%u checks)\n",functional_checks);return 0;
}
