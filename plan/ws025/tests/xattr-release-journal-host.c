...
static void xattr_write_check(uint64_t first,uint32_t count,const void *buffer)
{
 ...
 if(first==160 || first==32 || first==224 || first==UFS_SBLOCK_OFFSET/512 || first==8) {
  ...
  REQUIRE(drv_ufs_get32(xattr_journal->image,16,0)==xattr_extents);
...
static void xattr_scenario(unsigned layout,unsigned write_fail,unsigned flush_fail,unsigned landed,unsigned stop)
{
 ...
 if(layout==2) {
  ...
  drv_ufs_put32(cg,UFS_CG_NDBLK,192,0);memcpy(second,cg,4096);drv_ufs_put32(second,UFS_CG_CGX,1,0);
 }
 area=storage+160*512;memset(area,0,4096);drv_ufs_put32(area,0,count*4096,0);area[4]=UFS_EXTATTR_NAMESPACE_USER;area[6]=1;area[7]='x';
 ...
 REQUIRE(drv_ufs_journal_bind_image(&recovered,redo,sizeof(redo))==0);xattr_journal=&recovered;
 REQUIRE(drv_ufs_journal_replay(&recovered)==0);
 raw=storage+8*512+2*UFS_DINODE_SIZE;committed=drv_ufs_get32(raw,UFS_DI_EXTSIZE,0)==(xattr_keep?16U:0U);
 REQUIRE(drv_ufs_get32(raw,UFS_DI_EXTSIZE,0)==(committed?(xattr_keep?16U:0U):count*4096));
 REQUIRE(drv_ufs_get64(raw,UFS_DI_EXTB,0)==(committed && !xattr_keep?0:160));
 REQUIRE(drv_ufs_get64(raw,UFS_DI_EXTB+8,0)==(committed || count==1?0:second_fragment));
 REQUIRE(drv_ufs_get64(raw,UFS_DI_BLOCKS,0)==(committed?8+xattr_keep*8:8+count*8));
 REQUIRE(drv_ufs_get64(raw,UFS_DI_DB,0)==176 && !bit_test(cg+264,176));
 for(n=0;n<8;n++) {
 ...
 REQUIRE(drv_ufs_get32(cg,UFS_CG_NBFREE,0)==(committed?(layout==2?1-xattr_keep:count-xattr_keep):0));
 if(layout==2)REQUIRE(drv_ufs_get32(second,UFS_CG_NBFREE,0)==committed);
 REQUIRE(drv_ufs_get64(storage+UFS_SBLOCK_OFFSET,UFS_FS_CSTOTAL_NBFREE,0)==(committed?count-xattr_keep:0));
 REQUIRE(drv_ufs_get32(storage+160*512,0,0)==(committed && xattr_keep?16U:count*4096));
...
static void xattr_refusals(void)
{
 ...
 REQUIRE(persist_inode(&node.inode)==0);REQUIRE(write_super_summaries(&mountp)==0);REQUIRE(disk_sync(&disk)==0);
 io.context=&disk;io.read=media_read;io.write=media_write;io.flush=media_flush;
 REQUIRE(drv_ufs_journal_init(&fs.journal,&io,380,34,379)==0);REQUIRE(drv_ufs_journal_bind_image(&fs.journal,redo,sizeof(redo))==0);fs.journal_enabled=1;
