...
static void orphan_scenario(unsigned directory,unsigned write_fail,unsigned flush_fail,unsigned landed,unsigned stop)
{
 ...
 drv_ufs_put32(cg,UFS_CG_NIFREE,27,0);drv_ufs_put32(cg,UFS_CG_NDIR,1+directory,0);
 fs.super.cstotal_nifree=27;fs.super.cstotal_ndir=1+directory;
 attr=storage+168*512;drv_ufs_put32(attr,0,16,0);attr[4]=UFS_EXTATTR_NAMESPACE_USER;attr[6]=1;attr[7]='x';memcpy(attr+8,"retained",8);
 ...
 REQUIRE(drv_ufs_journal_init(&recovered,&io,380,130,379)==0);REQUIRE(drv_ufs_journal_bind_image(&recovered,redo,sizeof(redo))==0);
 orphan_journal=&recovered;REQUIRE(drv_ufs_journal_replay(&recovered)==0);
 raw=storage+8*512+3*UFS_DINODE_SIZE;
 allocated=bit_test(cg+256,3)!=0;data_owned=drv_ufs_get64(raw,UFS_DI_DB,0)!=0;attr_owned=drv_ufs_get64(raw,UFS_DI_EXTB,0)!=0;
 links=drv_ufs_get16(raw,UFS_DI_NLINK,0);
 REQUIRE(links==0);
 if(links!=0)REQUIRE(allocated && data_owned && attr_owned);
 REQUIRE((drv_ufs_get16(raw,UFS_DI_MODE,0)!=0)==allocated);
 REQUIRE(drv_ufs_get64(raw,UFS_DI_DB,0)==(data_owned?160:0));
 REQUIRE(drv_ufs_get64(raw,UFS_DI_EXTB,0)==(attr_owned?168:0));
 REQUIRE(drv_ufs_get32(raw,UFS_DI_EXTSIZE,0)==(attr_owned?16:0));
 REQUIRE(drv_ufs_get64(raw,UFS_DI_BLOCKS,0)==8*(data_owned+attr_owned));
 for(n=0;n<8;n++){REQUIRE((unsigned)bit_test(cg+264,160+n)==!data_owned);REQUIRE((unsigned)bit_test(cg+264,168+n)==!attr_owned);}
 REQUIRE(drv_ufs_get32(cg,UFS_CG_NBFREE,0)==2-data_owned-attr_owned);
 REQUIRE(drv_ufs_get64(storage+UFS_SBLOCK_OFFSET,UFS_FS_CSTOTAL_NBFREE,0)==2-data_owned-attr_owned);
 REQUIRE(drv_ufs_get32(cg,UFS_CG_NIFREE,0)==28-allocated);
 REQUIRE(drv_ufs_get64(storage+UFS_SBLOCK_OFFSET,UFS_FS_CSTOTAL_NIFREE,0)==28-allocated);
 REQUIRE(drv_ufs_get32(cg,UFS_CG_NDIR,0)==1+directory*allocated);
...
static void orphan_boundaries(void)
{
 ...
 cg=storage+32*512;drv_ufs_put32(cg,UFS_CG_NDBLK,192,0);
 memset(cg+256,0,8);for(n=0;n<4;n++)bit_set(cg+256,n);
 drv_ufs_put32(cg,UFS_CG_NIFREE,28,0);drv_ufs_put32(cg,UFS_CG_NDIR,1,0);
 memcpy(storage+224*512,cg,4096);cg=storage+224*512;
 drv_ufs_put32(cg,UFS_CG_CGX,1,0);memset(cg+256,0,8);bit_set(cg+256,0);bit_set(cg+256,1);
 drv_ufs_put32(cg,UFS_CG_NIFREE,30,0);
 ...
 raw=storage+8*512+3*UFS_DINODE_SIZE;drv_ufs_put64(raw,UFS_DI_DB,379,0);
 REQUIRE(orphan_recover(&mountp)==EIO && !fs.writable);fs.writable=1;drv_ufs_put64(raw,UFS_DI_DB,160,0);
 drv_ufs_put16(raw,UFS_DI_MODE,0,0);
 REQUIRE(orphan_recover(&mountp)==EOPNOTSUPP && !fs.writable);fs.writable=1;drv_ufs_put16(raw,UFS_DI_MODE,S_IFREG|0600,0);
 ...
 REQUIRE(drv_ufs_get64(storage+200*512+UFS_DINODE_SIZE,UFS_DI_DB,0)==360);
 REQUIRE(drv_ufs_get16(storage+200*512+UFS_DINODE_SIZE,UFS_DI_NLINK,0)==1);
