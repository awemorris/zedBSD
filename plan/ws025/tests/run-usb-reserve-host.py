...
for mode in ('ordinary','sanitize'):
 ...
 for name in (sys.argv[2:] or ['usb','xhci','storage']):
  ...
  run(name+'-'+mode+'-build',['cc','-std=c11','-O1','-g','-Wall','-Wextra','-Werror','-ffunction-sections','-fdata-sections','-I.','-Iinclude','-Iinclude/uapi','-Isrc',*extra,'plan/ws025/tests/'+name+'-reserve-host.c','src/kern/io.c','-pthread','-Wl,--gc-sections','-o',binary])
