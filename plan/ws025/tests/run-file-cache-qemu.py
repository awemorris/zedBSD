...
def main():
    ...
    root = REPO / "build/arch-images/amd64-ws025-cache.ufs"
    ...
    try:
        guest.login()
        at = len(guest.text())
        guest.send("storage-exit file-cache")
        result, _ = guest.wait(r"root@[^\s]*:[^\n]*\$ ?$", at, 240)
        assert "FILE CACHE PASS" in result and "storage-result-0" in result, result[-4000:]
        assert "FILE CACHE FAIL" not in result
        assert "CACHE WARM ufs_content_reads=0 usb_reads=0" in result
        assert "CACHE BUDGET managed=" in result and "worker=69632" in result
        print(result, flush=True)
        at = len(guest.text())
        guest.send("storage-exit sysctl vfs.cache_memory.stats")
        stats_result = guest.prompt(at)
        assert "storage-result-0" in stats_result and "worker resident=69632 pending=0" in stats_result
        at = len(guest.text())
        guest.send("storage-exit sysctl vfs.cache_memory.target_bytes=8388608")
        control_result = guest.prompt(at)
        assert "storage-result-0" in control_result
        assert "vfs.cache_memory.target_bytes: 8388608" in control_result
        (output / "results.json").write_text(json.dumps({"cache": "PASS", "budget": "PASS",
            "sysctl": "PASS", "warm_ufs_reads": 0, "warm_usb_reads": 0}) + "\n")
    finally:
        guest.stop()
    assert base.digest(source) == original
    (output / "source.sha256").write_text(original + "  build/amd64/hdd-image.img\n")
    print("WS025 native file cache PASS")
