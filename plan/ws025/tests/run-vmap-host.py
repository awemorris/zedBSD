...
for variant in ('ordinary','sanitize'):
 ...
 args=['cc','-std=c11','-g','-ffunction-sections','-fdata-sections','-Wl,--gc-sections','-Wall','-Wextra','-Werror','-Iinclude','-Iinclude/uapi','-I.','-I'+str(out),*flags,'plan/ws025/tests/vmap-host.c','src/kern/io.c',str(out/'vm-kernel-map-extracted.c'),'-pthread','-o',str(binary)]
