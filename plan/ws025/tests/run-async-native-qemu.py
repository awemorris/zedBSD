...
def main():
    ...
    root = REPO / "build/arch-images/amd64-ws025-async.ufs"
    ...
    try:
        guest.login()
        at = len(guest.text())
        guest.send("storage-exit async-native")
        result, _ = guest.wait(r"root@[^\s]*:[^\n]*\$ ?$", at, 240)
        assert "ASYNC NATIVE PASS" in result and "storage-result-0" in result, result[-4000:]
        assert "ASYNC NATIVE KERNEL PASS" in result
        print(result, flush=True)
        (output / "results.json").write_text(json.dumps({"async_scheduler": "PASS", "async_usb": "PASS"}) + "\n")

    finally:
        guest.stop()
    assert base.digest(source) == original
    (output / "source.sha256").write_text(original + "  build/amd64/hdd-image.img\n")
    print("WS025 native asynchronous BIO PASS")
