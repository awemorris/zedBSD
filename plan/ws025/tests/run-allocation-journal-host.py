...
for variant in ('sanitize','ordinary'):
 ...
 subprocess.run(['cc','-DKERN_USER_ABI_LP64',*extra,'-O1','-g','-pthread','-c','plan/ws018/tests/mount-thread-host.c','-o',str(bridge)],cwd=repo,check=True)
 jump=out/(variant+'-crash.o')
 subprocess.run(['cc',*extra,'-O1','-g','-c','plan/ws025/tests/allocation-crash-bridge.c','-o',str(jump)],cwd=repo,check=True)
 ...
 binary=out/variant
 subprocess.run(['cc','-DKERN_USER_ABI_LP64',*extra,'-std=c11','-O1','-g','-Wall','-Wextra','-Werror','-ffunction-sections','-fdata-sections','-I.','-Iinclude','-Iinclude/uapi','-Isrc','-Ilibc/include','-Iplan/ws018/tests','-Iplan/ws025/temp/p031-driver-fragments/src/drivers/fs/ufs','plan/ws025/tests/'+fixture,'plan/ws025/temp/p031-driver-fragments/src/drivers/fs/ufs/ufs-endian.c','plan/ws025/temp/p031-driver-fragments/src/drivers/fs/ufs/ufs-journal.c','src/kern/quota.c','src/kern/io-stats.c',*extra_sources,str(bridge),str(jump),'-pthread','-Wl,--gc-sections','-o',str(binary)],cwd=repo,check=True)
