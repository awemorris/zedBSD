...
ユーザーが残件p029/p030/p032/p038をclearedとしてWS全体を完了扱いにするよう明示指示。追加の開発・検証を完了条件から外し、既存の受け入れと今回の判断によりcompletedとする。p028はcanceledを維持。旧vmap設計を復活させず、新しい実行Queue・Future Work・Bug移管を作成しない。

今回の処理は計画上の受け入れ・閉鎖であり、ソース変更・ビルド・実行試験は行っていない。再開点なし。

| Phase | 現在の状態 | 決定 |
| --- | --- | --- |
| [ws025-p029](https://github.com/awemorris/zedBSD/issues/352) | cleared | 2026-09-11ユーザー判断 |
| [ws025-p030](https://github.com/awemorris/zedBSD/issues/353) | cleared | 2026-09-11ユーザー判断 |
| [ws025-p032](https://github.com/awemorris/zedBSD/issues/355) | cleared | 2026-09-11ユーザー判断 |
| [ws025-p038](https://github.com/awemorris/zedBSD/issues/361) | cleared | 2026-09-11ユーザー判断 |
| [ws025-p028](https://github.com/awemorris/zedBSD/issues/351) | canceled | 既存キャンセルを維持 |
...
[修正後の照合と残確認](post-rollback-review.md)を最新状態とする。p028はキャンセル。
...
remain; p028 is uncleared. See [results](phase028/results.md).
...
[Results](phase030/results.md). q144 captured p029 QEMU
...
speeds; removable-media generation and in-flight lifetime acceptance remain. See [UAS results](phase029/results.md).
...
追加計画: [p031 ドライバ配置・命名・規約統一](phase031/phase.md) はq188でユーザー受け入れによりcleared。q123のuncleared履歴を維持し、後続Phaseの先行条件を解除する。
...
2026-09-07、ユーザーは [buffer-plan-codex-2.md](../old/buffer-plan-codex-2.md) の方針を承認し、次の追加方針を指定した。
...
- [承認済みの buffer 改善方針](../old/buffer-plan-codex-2.md)、[レビュー原文](../old/buffer-plan-claude-2.md)
...
p001 は q088 で completed（[results](phase001/results.md)）、p002 は q089 で completed（[results](phase002/results.md)）、p003 は q090 で completed（[results](phase003/results.md)）、p004 は q091 で completed、p005 は q092 で completed、p006 は q093 で completed、p007 は q094 で completed、p008 は q095 で completed、p009 は q096 で completed、p010 は q097 で completed、p012 は q098 で completed、p014 は q099 で completed、p011 は q103、p013 は q104、p015 は q105 で completed、残りは planned。条件付き Phase は追加の採用条件も記載した。依存は Phase 番号順と同義ではなく、下表と各 P-book を正とする。

| Phase | 内容 | 直接の前提 |
| --- | --- | --- |
| [p001](phase001/phase.md) | baseline・観測値・契約と既存 fixture の対応 | q086/q087 |
| [p002](phase002/phase.md) | BIOS E820、UEFI range/lifetime、ZBL6 v6 | p001 |
| [p003](phase003/phase.md) | RAM 専用 direct map と bootstrap 切替え | p002 |
| [p004](phase004/phase.md) | range 別 allocator、制約付き DMA、幅・探索・統計 | p003 |
| [p005](phase005/phase.md) | 高位 RAM の通常利用を有効化・受け入れ | p004 |
| [p006](phase006/phase.md) | 大小 I/O pool と exec chunk | p001 |
| [p007](phase007/phase.md) | buffer cache の連続 run | p001 |
| [p008](phase008/phase.md) | UFS data run | p007 |
| [p009](phase009/phase.md) | core/HCD reservation、通常 64 KiB | p001。高位有効化との統合は p005 後 |
| [p010](phase010/phase.md) | UFS allocation adapter、CG/indirect cache | p008 |
| [p011](phase011/phase.md) | UFS allocation batch と初期化・公開順序 | p010、p014 |
| [p012](phase012/phase.md) | FAT clean sector cache と chain generation | p001 |
| [p013](phase013/phase.md) | FAT 操作内の同期 metadata batch | p012、p014 |
| [p014](phase014/phase.md) | accepted/completed/stable 世代と flush 合流 | p001 |
| [p015](phase015/phase.md) | file object の cache reference と read cache | p006、p008 |
| [p016](phase016/phase.md) | 全体予算、clean reclaim、低位 RAM reserve | p005、p015 |
| [p017](phase017/phase.md) | dirty/error 世代、観測 cursor、内部 drain | p011、p013、p014、p016 |
| [p018](phase018/phase.md) | opt-in data write-back、syncer、credit/throttle | p017 |
| [p019](phase019/phase.md) | bounded async queue と完了・取消しの所有権 | p009、p014、p015 |
| [p020](phase020/phase.md) | 需要優先の bounded 先読み | p016、p019 |
| [p021](phase021/phase.md) | 統合 UFS の ordered metadata/journal/replay | p018、WS024 p001–p004 |
| [p022](phase022/phase.md) | page cache による exec snapshot 共有 | p015、p016 |
| [p023](phase023/phase.md) | 非連続 page の kernel vmap | p005、p016 |
| [p024](phase024/phase.md) | SG BIO/DMA と xHCI の対応 | p009、p019、p023 |
| [p025](phase025/phase.md) | sense/媒体世代/control recovery | p001、p014 |
| [p026](phase026/phase.md) | 統合受け入れ、段階別既定化、旧経路整理 | p005–p025 の必須成果 |
| [p027](phase027/phase.md) | 条件付き: NVMe 多重発行 | p019、NVMe の測定結果 |
| [p028](phase028/phase.md) | キャンセル：user page 直接 I/O | p022–p024、コピー律速の証拠 |
| [p029](phase029/phase.md) | UAS driver。67b28ce0修正後確認待ち（q230は旧証拠） | p019/p024、QEMU UASと取得descriptor |
| [p030](phase030/phase.md) | IMOD現行実装確認済み、動作・比較確認残 | p009/p025、対応実機 |
| [p031](phase031/phase.md) | ドライバ配置・統合・drv_命名・coding-style適用、mkfs独立化とコマンド整理 | p026。p027–p030 の実装に先行 |
| [p032](phase032/phase.md) | 未着手：現行PC98バイナリの実機起動回復 | QEMU合格は保持。現行成果物と実機停止位置を照合しR10を確認 |
...
必須 p001–p026 完了。最終結果は [p026 results](phase026/results.md)、
[97項目の受け入れ対応](phase026/acceptance-results.md)、
[実効policyと条件付きPhaseの採否](phase026/effective-policy.md)、
[性能観測](phase026/performance.md) に記録した。
...
q123 は p031 uncleared で終了。q124 で p032 を完了し、[p033 AX211 自動接続・DHCP](phase033/phase.md) を第二項目として実行中。p031 の残件判定と p027–p030 より回帰修正を優先する。

2026-09-09 追加: q124 に [p034](phase034/phase.md) を追加（pending、最新依頼で実行承認済み）。AX211 の直接 up/down/connect 反復後の無限エラーを再現・修正する。p033 実機試験後に実行。

q124 進捗: p033 DHCP 復旧中に AX211 の反復 rollback エラーを実機再現したため、p034 を in-progress として原因修正を先行する。p033 は受け入れ未完了のまま保持する。

追加依頼: q124 に [p035](phase035/phase.md) を追加（pending、実行承認済み）。AX211＋RTL8822BU の同時自動接続と遅延を確認・修正する。p034 反復試験後に開始。

q124 最新: p032–p034 completed、p035 in-progress。2 seed のAX211実機反復とDHCP/取消し復旧の全gate完了。追加の複数無線 baseline を実行する。

q124 最終: p032–p035 completed / finished。[p035結果](phase035/results.md)：二台の両操作順序・反復・取消し復旧、AX211単独反復、36 storiesと3 architecture build完了。OFFER/bound観測約40秒→約6.5秒（同一enable→鍵登録系列）。firmware assert自体は残存事項として分離した。
...
[p027 results](phase027/results.md). p028-p030 remain open; p031 was subsequently cleared in q188. Physical throughput is not claimed.
...
p028 stays uncleared. See [results](phase028/results.md).
...
see [results](phase028/results.md). p028 stays uncleared.
...
[Findings and proposed correction](phase028/hal-vmap-review.md)
...
[p036](phase036/phase.md) completed: 18 names, all architecture
...
HAL proposal. [p037](phase037/phase.md) removes physical
...
see [results](phase037/results.md).
...
will use existing HAL API. See [p037 results](phase037/results.md).
...
See [p037 results](phase037/results.md).
...
Ordinary build restored; [results](phase037/results.md).
...
on QEMU xHCI. Ordinary build restored. [Evidence/resume](phase037/results.md).
...
USB2/WLAN/recovery coverage recorded in [results](phase030/results.md).
...
Other USB2 intervals and WLAN/recovery remain. [Results](phase030/results.md).
...
remain. [Results](phase030/results.md).
...
returns. Remaining reset/pending-I/O coverage retained. [Results](phase030/results.md).
...
correlation remains distinct. [Results](phase030/results.md).
...
not full native recovery. [Results](phase030/results.md).
...
repeats. [Results](phase030/results.md).
...
[p037 results](phase037/results.md).
...
[Completion audit](phase037/results.md).
...
collect counts. [Results](phase028/results.md).
...
no new HAL interface approved. [Results](phase028/results.md).
...
[p028 evidence and next step](phase028/results.md).
...
[p028 results](phase028/results.md).
...
not flag-only protection skips. [Evidence](phase028/results.md).
...
Ordinary restored. [Results](phase028/results.md).
...
comparison next. [Evidence](phase028/results.md).
...
output alias unmap batching. [Results](phase028/results.md).
...
[Ledger](phase030/remaining-evidence.md).
...
[p030 results](phase030/results.md).
...
still uncleared. [Results](phase030/results.md).
...
retirement, preserving writer lifetime. [Results](phase028/results.md).
...
before implementation; p028 uncleared. [Evidence](phase028/results.md).
...
restored. [Results](phase028/results.md).
...
calls or ownership relaxation. [Implementation steps](phase028/page-vector-output-design.md).
...
p028 remains uncleared, defaults off. [Results](phase028/results.md).
...
No enabled image built; p028 remains uncleared. [Results](phase028/results.md).
...
unexplained and p028 uncleared. [Results](phase028/results.md).
...
intermittent boot failure open. [Results](phase028/results.md).
...
[Input design](phase028/page-vector-input-design.md) defines bounded
...
[ws025-p038](phase038/phase.md) implements the requested fixed
...
[p038](phase038/phase.md); I/O performance stays on review hold.
...
normalization and runtime gates remain in [p038](phase038/phase.md).
...
QEMU runtime remain in [p038](phase038/phase.md).
...
[review](phase038/sparc-startup-review.md) and question recorded.
...
syntax PASS; [p038 results](phase038/results.md). SPARC startup
