<!-- awesome-plan project=zedbsd record=ws001-p010 -->

既存計画の取り込み。本文はローカルの現行記録、リポジトリ資料へのリンクは基準コミットの履歴です。Issue作成・open状態は実行承認や未完了判定を意味しません。Awesome Planの状態同期導入前のため、実際の状態は本文を参照してください。

元資料: `plan/ws001/phase010/phase.md`

親: [ws001](https://github.com/awemorris/zedBSD/issues/2)

# ws001-p010: local reimplementation of bc, ed, and m4

WSID: `ws001`

Phase ID: `p010`

Status: complete at the local-replacement milestone
...
carried in [WS001](https://github.com/awemorris/zedBSD/issues/2); all three matrix rows remain
...
6. update the Phase 10 progress and affected component rows in
   [WS001](https://github.com/awemorris/zedBSD/issues/2). Do not leave notes that describe the
   removed port.
...
6. update the `P10-*` replacement gates plus every affected subsystem/API/libc
   row in [WS001](https://github.com/awemorris/zedBSD/issues/2) during the same work unit.
...
- [WS001](https://github.com/awemorris/zedBSD/issues/2) records the completed replacement gates and
...
## 11. Interruption and resumption record

The local-replacement milestone completed without interruption. Full `bc`,
`ed`, and `m4` conformance remains WS001 backlog and resumes only through a new
Phase selected from the ledger.

## 12. Completion conditions

- Imported production/generated code for `bc`, `ed`, and `m4` is absent from base.
- Each replacement builds solely from zedBSD-local source and installs independently.
- Replacement provenance, host, standalone, top-level build, and QEMU gates pass.
- Remaining semantic gaps fail safely and remain recorded in WS001.
