...
| `ws008` | Noct and BeUI | Resumed by user priority; q041 selects p008 host refresh, while target packages remain disabled until the target Phase passes | `ws008-p007` complete; p008 selected; p009 target refresh is human-blocked by the current upstream zedBSD build regression | Execute q041 p008 after WS005 p005. Resume p009 only from a corrected upstream revision or an explicit downstream-patch-overlay decision; never mutate the old dirty checkout | [WS008](ws008-noct/ws.md) |
