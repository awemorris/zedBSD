...
Active Queue: なし。直近: q459（ws035-p054 uncleared: acquire fence を実装・試験済み。受け入れ 3 の「present が速くなる」は測って速くならず、読み替えの判断待ち）。直近: q458（ws035-p053 cleared: `wl_shm` と cursor）。直近: q456（ws062-p003 cleared: amd64 の既定を native・2 GiB に、CI は gzip）。直近の終了: q453（ws065-p003 cleared: sh の builtin の bash 拡張）
