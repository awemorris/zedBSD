...
static struct ufs2_snapshot_entry *
map_find(struct ufs2_snapshot *snapshot,uint64_t sector,int insert)
{
	size_t start=hash_sector(sector,snapshot->map_count),slot=start;
	do {struct ufs2_snapshot_entry *entry=&snapshot->map[slot];
		if(entry->sector==sector)return entry;
		if(entry->sector==UFS2_SNAPSHOT_EMPTY)return insert?entry:NULL;
		slot=(slot+1U)%snapshot->map_count;
	} while(slot!=start);
	return NULL;
}
static void map_clear(struct ufs2_snapshot *snapshot)
{ size_t n;for(n=0;n<snapshot->map_count;n++){
		snapshot->map[n].sector=UFS2_SNAPSHOT_EMPTY;snapshot->map[n].record=0;} }

static int
write_control(struct ufs2_snapshot *snapshot,unsigned active,uint32_t next)
{
...
int
ufs2_snapshot_init(struct ufs2_snapshot *snapshot,const struct ufs2_journal_io *io,
	uint64_t volume,uint64_t first,uint32_t sectors,
	struct ufs2_snapshot_entry *map,size_t map_count)
{
	uint32_t records;
...
ufs2_snapshot_open(struct ufs2_snapshot *snapshot)
{
	...
	for(record=0;record<count;record++) {
		struct ufs2_snapshot_entry *entry;uint64_t target;
		...
		if(get32(header)!=RECORD_MAGIC||get32(header+4)!=SNAPSHOT_VERSION||
		    ...
		    entry->sector!=UFS2_SNAPSHOT_EMPTY)return EIO;
...
int
ufs2_snapshot_preserve(struct ufs2_snapshot *snapshot,uint64_t first,uint32_t count)
{
	...
	for(n=0;n<count;n++) {
		uint64_t target=first+n;uint32_t record;struct ufs2_snapshot_entry *entry;
...
int
ufs2_snapshot_read(struct ufs2_snapshot *snapshot,uint64_t first,uint32_t count,
	void *buffer)
{
	...
	if(snapshot==NULL||buffer==NULL||count==0||!snapshot->active||
		...
		return EINVAL;
	for(n=0;n<count;n++) {
		struct ufs2_snapshot_entry *entry=map_find(snapshot,first+n,0);
