...
journal_discover(struct mount *mountp,struct ufs2_mount_state *ms)
{
	...
	error=ufs2_journal_init(&ms->journal,&io,end+1U,sectors);
	if(error==0){ms->journal_enabled=1;error=ufs2_journal_replay(&ms->journal);}
	return error;
}
...
{
	struct ufs2_journal_io io;uint8_t locator[UFS2_SECTOR_SIZE];
	...
	io.context=mountp->m_disk;io.read=journal_read;io.write=journal_write;
	io.flush=journal_flush;
	error=ufs2_snapshot_init(&ms->snapshot,&io,end,cursor+1U,sectors,
	    ms->snapshot_map,map_count);
	if(error==0)error=ufs2_snapshot_open(&ms->snapshot);
...
load_cg_locked(struct mount *mountp, uint32_t cg)
{
	...
	ndir = ufs2_get32(ms->cg, UFS2_CG_NDIR, ms->super.swapped);
	nbfree = ufs2_get32(ms->cg, UFS2_CG_NBFREE, ms->super.swapped);
	nifree = ufs2_get32(ms->cg, UFS2_CG_NIFREE, ms->super.swapped);
	nffree = ufs2_get32(ms->cg, UFS2_CG_NFFREE, ms->super.swapped);
	ms->cg_iusedoff = ufs2_get32(ms->cg, UFS2_CG_IUSEDOFF,
	    ms->super.swapped);
	ms->cg_freeoff = ufs2_get32(ms->cg, UFS2_CG_FREEOFF,
	    ms->super.swapped);
	ms->cg_nextfreeoff = ufs2_get32(ms->cg, UFS2_CG_NEXTFREEOFF,
	    ms->super.swapped);
	if (ufs2_get32(ms->cg, UFS2_CG_MAGIC, ms->super.swapped) !=
	    UFS2_CG_MAGIC_VALUE ||
	    ufs2_get32(ms->cg, UFS2_CG_CGX, ms->super.swapped) != cg ||
	    ufs2_get32(ms->cg, UFS2_CG_NDBLK, ms->super.swapped) != ndblk ||
...
write_super_summaries(struct mount *mountp)
{
	...
	if(error==0) {
		ufs2_put64(buffer,UFS2_FS_CSTOTAL_NDIR,
		    ms->super.cstotal_ndir,
		    ms->super.swapped);
		ufs2_put64(buffer,UFS2_FS_CSTOTAL_NBFREE,
		    ms->super.cstotal_nbfree,
		    ms->super.swapped);
		ufs2_put64(buffer,UFS2_FS_CSTOTAL_NIFREE,
		    ms->super.cstotal_nifree,
		    ms->super.swapped);
		ufs2_put64(buffer,UFS2_FS_CSTOTAL_NFFREE,
...
allocate_block(struct mount *mountp,uid_t uid,gid_t gid,uint64_t *result)
{
	...
	for(fragment=(ms->super.dblkno+ms->super.frag-1U)&~(ms->super.frag-1U);
		...
		{ old_total=ms->super.cstotal_nbfree;
			uint32_t free=ufs2_get32(ms->cg,UFS2_CG_NBFREE,ms->super.swapped);
			if(free==0){for(n=0;n<ms->super.frag;n++)bit_set(map,fragment+n);break;}
			ufs2_put32(ms->cg,UFS2_CG_NBFREE,free-1U,ms->super.swapped);
		...
		if(error!=0){for(n=0;n<ms->super.frag;n++)bit_set(map,fragment+n);ufs2_put32(ms->cg,UFS2_CG_NBFREE,ufs2_get32(ms->cg,UFS2_CG_NBFREE,ms->super.swapped)+1U,ms->super.swapped);ms->super.cstotal_nbfree=old_total;error=write_cg_rollback(mountp,error);}else {*result=cgstart(&ms->super,cg)+fragment;ms->rotor_cg=cg;}
		break;
	}
...
free_block(struct mount *mountp,uint64_t fragment,uid_t uid,gid_t gid)
{
	...
	free=ufs2_get32(ms->cg,UFS2_CG_NBFREE,ms->super.swapped);
	...
	ms->super.cstotal_nbfree=old_total+1U;
	error=write_cg(mountp);
	if(error!=0){for(n=0;n<ms->super.frag;n++)bit_clear(map,local+n);ufs2_put32(ms->cg,UFS2_CG_NBFREE,free,ms->super.swapped);ms->super.cstotal_nbfree=old_total;error=write_cg_rollback(mountp,error);}
	mutex_unlock(&ms->lock);
...
allocate_inode_number(struct mount *mountp,uid_t uid,gid_t gid,uint32_t *number)
{
	...
	for(ino=cg==0?UFS2_ROOT_INO+1U:0U;ino<ms->super.ipg;ino++)if(!bit_test(map,ino)){
		uint32_t free=ufs2_get32(ms->cg,UFS2_CG_NIFREE,ms->super.swapped);
		if(free==0)break;
		old_total=ms->super.cstotal_nifree;bit_set(map,ino);
		ufs2_put32(ms->cg,UFS2_CG_NIFREE,free-1U,ms->super.swapped);
		ms->super.cstotal_nifree=old_total-1U;
		error=write_cg(mountp);if(error!=0){bit_clear(map,ino);ufs2_put32(ms->cg,UFS2_CG_NIFREE,free,ms->super.swapped);ms->super.cstotal_nifree=old_total;error=write_cg_rollback(mountp,error);}else {*number=cg*ms->super.ipg+ino;ms->rotor_cg=cg;}break;
	}
		if(error!=ENOSPC)break;
	}
	mutex_unlock(&ms->lock);
...
free_inode_number(struct mount *mountp,uint32_t number,uid_t uid,gid_t gid)
{
	...
	free=ufs2_get32(ms->cg,UFS2_CG_NIFREE,ms->super.swapped);
	if(free==UINT32_MAX){mutex_unlock(&ms->lock);return EIO;}
	old_total=ms->super.cstotal_nifree;bit_clear(map,local);
	ufs2_put32(ms->cg,UFS2_CG_NIFREE,free+1U,ms->super.swapped);
	ms->super.cstotal_nifree=old_total+1U;error=write_cg(mountp);
	if(error!=0){bit_set(map,local);ufs2_put32(ms->cg,UFS2_CG_NIFREE,free,ms->super.swapped);ms->super.cstotal_nifree=old_total;error=write_cg_rollback(mountp,error);}
	mutex_unlock(&ms->lock);
	...
	uint64_t *result)
{
	...
	if (error == 0)
		*result = ufs2_get64(block, (size_t)index * 8U, s->swapped);
	kern_free(block);
	return error;
}
...
bmap_ensure(struct inode *inode,uint64_t logical,uint64_t *result)
{
	...
	for(depth=level+1U;depth!=0;depth--) {
		...
		next=ufs2_get64(block,(size_t)index*8U,s->swapped);
		if(next==0) {
			uint64_t allocated=0;
			error=allocate_block(inode->i_mount,inode->i_uid,inode->i_gid,&next);
			if(error==0) {
				allocated=next;
				ufs2_put64(block,(size_t)index*8U,next,s->swapped);
...
persist_inode(struct inode *inode)
{
	...
	ufs2_put64(raw,UFS2_DI_ATIME,(uint64_t)inode->i_atime.tv_sec,
		ms->super.swapped);
	ufs2_put32(raw,UFS2_DI_ATIMENSEC,(uint32_t)inode->i_atime.tv_nsec,
		ms->super.swapped);
	ufs2_put64(raw,UFS2_DI_MTIME,(uint64_t)inode->i_mtime.tv_sec,
		ms->super.swapped);
	ufs2_put32(raw,UFS2_DI_MTIMENSEC,(uint32_t)inode->i_mtime.tv_nsec,
		ms->super.swapped);
	ufs2_put64(raw,UFS2_DI_CTIME,(uint64_t)inode->i_ctime.tv_sec,
		ms->super.swapped);
	ufs2_put32(raw,UFS2_DI_CTIMENSEC,(uint32_t)inode->i_ctime.tv_nsec,
		ms->super.swapped);
	ufs2_put32(raw,UFS2_DI_EXTSIZE,ui->extattr_size,ms->super.swapped);
	...
	if(inode->i_type==INODE_CHAR||inode->i_type==INODE_BLOCK) {
		...
		for(n=0;n<UFS2_NDADDR;n++)ufs2_put64(raw,UFS2_DI_DB+n*8U,ui->direct[n],ms->super.swapped);
		for(n=0;n<UFS2_NIADDR;n++)ufs2_put64(raw,UFS2_DI_IB+n*8U,ui->indirect[n],ms->super.swapped);
	}
...
load_inode(struct mount *mountp, uint32_t number, struct inode **result)
{
	...
	mode = ufs2_get16(raw, UFS2_DI_MODE, s->swapped);
	...
	inode->i_mode = mode; inode->i_linkcount = ufs2_get16(raw,UFS2_DI_NLINK,s->swapped);
	inode->i_size = (off_t)ufs2_get64(raw,UFS2_DI_SIZE,s->swapped);
	inode->i_uid = ufs2_get32(raw,UFS2_DI_UID,s->swapped);
	inode->i_gid = ufs2_get32(raw,UFS2_DI_GID,s->swapped);
	inode->i_atime.tv_sec = (time_t)ufs2_get64(raw,UFS2_DI_ATIME,s->swapped);
	inode->i_atime.tv_nsec = ufs2_get32(raw,UFS2_DI_ATIMENSEC,s->swapped);
	inode->i_mtime.tv_sec = (time_t)ufs2_get64(raw,UFS2_DI_MTIME,s->swapped);
	inode->i_mtime.tv_nsec = ufs2_get32(raw,UFS2_DI_MTIMENSEC,s->swapped);
	inode->i_ctime.tv_sec = (time_t)ufs2_get64(raw,UFS2_DI_CTIME,s->swapped);
	inode->i_ctime.tv_nsec = ufs2_get32(raw,UFS2_DI_CTIMENSEC,s->swapped);
	...
	if(inode->i_type==INODE_CHAR||inode->i_type==INODE_BLOCK) {
		...
		for (n=0;n<UFS2_NDADDR;n++)
			ui->direct[n]=ufs2_get64(raw,UFS2_DI_DB+n*8U,s->swapped);
		for (n=0;n<UFS2_NIADDR;n++)
			ui->indirect[n]=ufs2_get64(raw,UFS2_DI_IB+n*8U,s->swapped);
	}
	ui->disk_flags=ufs2_get32(raw,UFS2_DI_FLAGS,s->swapped);
	ui->blocks=ufs2_get64(raw,UFS2_DI_BLOCKS,s->swapped);
	ui->generation=ufs2_get32(raw,UFS2_DI_GEN,s->swapped);
	...
	uint8_t *type, char name[NAME_MAX+1U])
{
	...
	while (*cursor < directory->i_size) {
		...
		*number=ufs2_get32(head,0,state(directory->i_mount)->super.swapped);
		reclen=ufs2_get16(head,4,state(directory->i_mount)->super.swapped);
...
dir_remove(struct inode *directory,const struct componentname *name,uint32_t *number)
{
	...
	if(error==0) {
		memcpy(original,block,ms->super.bsize);
		uint16_t reclen=ufs2_get16(block,offset+4U,
			ms->super.swapped);
		if(previous!=UINT32_MAX &&
		    previous/UFS2_DIRBLKSIZ==offset/UFS2_DIRBLKSIZ) {
			uint16_t prior=ufs2_get16(block,previous+4U,
				ms->super.swapped);
			ufs2_put16(block,previous+4U,prior+reclen,
				ms->super.swapped);
		} else {
			ufs2_put32(block,offset,0,ms->super.swapped);
		}
	...
	uint32_t number,uint8_t type,uint32_t *old_number,uint8_t *old_type)
{
	...
	if(error==0) {
		...
		*old_type=block[offset+6U];
		ufs2_put32(block,offset,number,ms->super.swapped);
...
extattr_load(struct inode *inode,uint8_t **result,size_t *length)
{
	...
	*result=NULL;*length=ui->extattr_size;
	...
	while(offset<ui->extattr_size) {
		...
		record=ufs2_get32(area,offset,ms->super.swapped);
	...
	size_t *record_length,size_t *content_at,size_t *content_length)
{
	...
	while(offset<area_length) {
		uint32_t record=ufs2_get32(area,offset,ms->super.swapped);
	...
	unsigned flags)
{
	...
	ufs2_put32(updated,at,(uint32_t)new_record,ms->super.swapped);
...
ufs2_listxattr(struct inode *inode,char *list,size_t size)
{
	...
	mutex_lock(&inode->i_lock);error=extattr_load(inode,&area,&area_length);
	if(error!=0)goto out;
	while(offset<area_length){uint32_t record=ufs2_get32(area,offset,ms->super.swapped);
...
static char
ufs2_identity_hex(unsigned value)
{
	return (char)(value < 10U ? '0' + value : 'A' + value - 10U);
}

static void
ufs2_identity_hex32(char output[8], uint32_t value)
{
	unsigned index;

	for (index = 0; index < 8U; index++)
		output[index] = ufs2_identity_hex(
		    (value >> (28U - index * 4U)) & 15U);
}

static void
ufs2_identity_label(char *output, size_t capacity, const uint8_t *input,
		    size_t length)
{
	size_t end = length;
	size_t index;
...
int
ufs2_identify(struct disk *disk, struct block_identity *identity)
{
	struct ufs2_super super;
	uint8_t *buffer;
	...
	if (disk == NULL || identity == NULL)
		return EINVAL;
	if (disk->d_block_size != UFS2_SECTOR_SIZE)
		return EOPNOTSUPP;
	first_block = UFS2_SBLOCK_OFFSET / UFS2_SECTOR_SIZE;
	block_count = UFS2_SBLOCK_SIZE / UFS2_SECTOR_SIZE;
	if (disk->d_block_count < first_block + block_count)
		return EOPNOTSUPP;
	buffer = kern_malloc(UFS2_SBLOCK_SIZE);
	if (buffer == NULL)
		return ENOMEM;
	error = disk_read_direct(disk, first_block, (uint32_t)block_count,
	    buffer);
	if (error == 0)
		error = ufs2_super_decode(buffer, UFS2_SBLOCK_SIZE,
	...
	strcpy(identity->type, "ufs2");
	identity->flags |= ZEDBSD_BLKID_TYPE;
	first = ufs2_get32(buffer, UFS2_FS_ID, super.swapped);
	second = ufs2_get32(buffer, UFS2_FS_ID + 4U, super.swapped);
	if (first != 0U || second != 0U) {
		ufs2_identity_hex32(identity->uuid, first);
		ufs2_identity_hex32(identity->uuid + 8U, second);
		identity->uuid[16] = '\0';
		identity->flags |= ZEDBSD_BLKID_UUID;
	}
	ufs2_identity_label(identity->label, sizeof(identity->label),
	    buffer + UFS2_FS_VOLNAME, UFS2_FS_VOLNAME_SIZE);
...
ufs2_quota_rebuild(struct mount *mountp)
{
	...
	for(cg=0;cg<ms->super.ncg&&error==0;cg++) {
		...
		for(index=0;index<ms->super.ipg;index++) {
			...
			mode=ufs2_get16(raw,UFS2_DI_MODE,ms->super.swapped);
			if(mode==0)continue;
			blocks=ufs2_get64(raw,UFS2_DI_BLOCKS,ms->super.swapped);
			if(blocks%(ms->super.bsize/UFS2_SECTOR_SIZE)!=0){error=EIO;break;}
			error=quota_rebuild_add(&ms->quota,
			    ufs2_get32(raw,UFS2_DI_UID,ms->super.swapped),
			    ufs2_get32(raw,UFS2_DI_GID,ms->super.swapped),
...
snapshot_disk_submit(struct disk *disk,struct bio *bio)
{
	...
	else {mutex_lock(&ms->snapshot_lock);error=ufs2_snapshot_read(&ms->snapshot,
...
static int ufs2_mount_impl(struct mount *mountp)
{
	...
	for(cg=0;cg<ms->super.ncg;cg++) {
		...
		if(cg==0&&!bit_test(ms->cg+ms->cg_iusedoff,UFS2_ROOT_INO))
			{error=EINVAL;break;}
		total_ndir+=ufs2_get32(ms->cg,UFS2_CG_NDIR,ms->super.swapped);
		total_nbfree+=ufs2_get32(ms->cg,UFS2_CG_NBFREE,ms->super.swapped);
		total_nifree+=ufs2_get32(ms->cg,UFS2_CG_NIFREE,ms->super.swapped);
		total_nffree+=ufs2_get32(ms->cg,UFS2_CG_NFFREE,ms->super.swapped);
	}
...
ufs2_snapshotctl(struct mount *mountp,struct snapshot_control *request)
{
	...
	case ZEDBSD_SNAPSHOT_CREATE:
		...
		mutex_lock(&ms->snapshot_lock);error=ufs2_snapshot_create(&ms->snapshot);
		mutex_unlock(&ms->snapshot_lock);
		if(error==0)error=snapshot_disk_publish(ms);
		if(error!=0&&ms->snapshot.active){mutex_lock(&ms->snapshot_lock);
			(void)ufs2_snapshot_delete(&ms->snapshot);
			mutex_unlock(&ms->snapshot_lock);}
		break;
	case ZEDBSD_SNAPSHOT_DELETE:
		...
		mutex_lock(&ms->snapshot_lock);error=ufs2_snapshot_delete(&ms->snapshot);
		mutex_unlock(&ms->snapshot_lock);
...
const struct filesystem_type ufs2_filesystem_type={
	.fs_name="ufs2",.probe=ufs2_probe,.identify=ufs2_identify,
	.mount=ufs2_mount_impl,.sync=ufs2_sync,
