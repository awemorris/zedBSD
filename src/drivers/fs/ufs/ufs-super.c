...
#define GET64(field, offset) super->field = ufs_get64(buffer, offset, swapped)
	GET32(sblkno,UFS_FS_SBLKNO);GET32(cblkno,UFS_FS_CBLKNO);
	GET32(iblkno,UFS_FS_IBLKNO);GET32(dblkno,UFS_FS_DBLKNO);
	GET32(ncg,UFS_FS_NCG);GET32(bsize,UFS_FS_BSIZE);
	GET32(fsize,UFS_FS_FSIZE);GET32(frag,UFS_FS_FRAG);
	GET32(bshift,UFS_FS_BSHIFT);GET32(fshift,UFS_FS_FSHIFT);
	GET32(fragshift,UFS_FS_FRAGSHIFT);GET32(fsbtodb,UFS_FS_FSBTODB);
	GET32(sbsize,UFS_FS_SBSIZE);GET32(nindir,UFS_FS_NINDIR);
	GET32(inopb,UFS_FS_INOPB);GET32(cssize,UFS_FS_CSSIZE);
	GET32(cgsize,UFS_FS_CGSIZE);GET32(ipg,UFS_FS_IPG);
	GET32(fpg,UFS_FS_FPG);GET64(sblockloc,UFS_FS_SBLOCKLOC);
	...
	GET64(size,UFS_FS_SIZE);GET64(dsize,UFS_FS_DSIZE);
	GET64(csaddr,UFS_FS_CSADDR);GET32(flags,UFS_FS_FLAGS);
...
#undef GET64
	...
	if (!power2(super->bsize) || !power2(super->fsize) ||
	    ...
	    super->sblockloc != UFS_SBLOCK_OFFSET ||
