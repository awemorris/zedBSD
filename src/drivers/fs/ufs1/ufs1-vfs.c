	...
	if(logical<UFS1_NDADDR) {
		if(ui->direct[logical]==0) {
			...
			if(error!=0) {
				...
				if(rollback==0)rollback=free_block(inode->i_mount,allocated);
...
persist_inode(struct inode *inode)
{
	...
	mutex_lock(&ms->lock);
	error=read_block(inode->i_mount,fragment,block);
	if(error!=0){mutex_unlock(&ms->lock);kern_free(block);return error;}raw=block+(index%ms->super.inopb)*UFS1_DINODE_SIZE;
	...
	if(inode->i_type==INODE_CHAR||inode->i_type==INODE_BLOCK) {
		...
		ufs1_put32(raw,UFS1_DI_DB,(uint32_t)inode->i_rdev,
			ms->super.swapped);
		for(n=0;n<UFS1_NIADDR;n++)
			ufs1_put32(raw,UFS1_DI_IB+n*4U,0,ms->super.swapped);
...
static int
detach_inode_block(struct inode *inode, uint32_t *pointer)
{
	struct ufs1_mount_state *ms = state(inode->i_mount);
	struct ufs1_inode_info *ui = info(inode);
	uint32_t fragment = *pointer;
	unsigned sectors = ms->super.bsize / UFS1_SECTOR_SIZE;
	int error;
	...
	*pointer = 0;
	...
	if (error == 0)
		error = free_block(inode->i_mount, fragment);
	if (error != 0)
		ms->writable = 0;
	return error;
}
	...
	uint64_t base, uint64_t keep, int *empty)
{
	...
	unsigned index, sectors = ms->super.bsize / UFS1_SECTOR_SIZE;
	int error = 0;
	...
	*empty = 1;
	...
	for (index = 0; index < ms->super.nindir; index++) {
		...
		ufs1_put32(block, (size_t)index * 4U, 0, ms->super.swapped);
...
load_inode_locked(struct mount *mountp, uint32_t number, struct inode **result)
{
	...
	if(inode->i_type==INODE_CHAR||inode->i_type==INODE_BLOCK) {
		inode->i_rdev=ufs1_get32(raw,UFS1_DI_DB,s->swapped);
	...
	uint8_t *block,uint32_t *offset,uint32_t *previous,uint32_t *number)
{
	...
	if(directory->i_size<0 || (uint64_t)directory->i_size>ms->super.bsize ||
	    (uint64_t)directory->i_size%UFS1_DIRBLKSIZ!=0 ||
	    info(directory)->direct[0]==0)
		return EIO;
	error=read_block(directory->i_mount,info(directory)->direct[0],block);if(error)return error;
	while(pos<(uint32_t)directory->i_size){if((uint32_t)directory->i_size-pos<8U||pos%UFS1_DIRBLKSIZ>UFS1_DIRBLKSIZ-8U){return EIO;}uint32_t ino=ufs1_get32(block,pos,ms->super.swapped);uint16_t reclen=ufs1_get16(block,pos+4U,ms->super.swapped);uint8_t nlen=block[pos+7U];
...
dir_add(struct inode *directory,const struct componentname *name,uint32_t number,uint8_t type)
{
	...
	if(old_size<0 || (uint64_t)old_size>ms->super.bsize ||
	    (uint64_t)old_size%UFS1_DIRBLKSIZ!=0) {
		error=EIO;goto out;
	}
	...
	while(pos<(uint32_t)directory->i_size){if((uint32_t)directory->i_size-pos<8U||pos%UFS1_DIRBLKSIZ>UFS1_DIRBLKSIZ-8U){error=EIO;goto out;}uint16_t reclen=ufs1_get16(block,pos+4U,ms->super.swapped);uint8_t nlen=block[pos+7U];uint16_t minimum=dir_minimum(nlen);
		if(reclen<minimum||(reclen&3U)!=0||pos%UFS1_DIRBLKSIZ+reclen>UFS1_DIRBLKSIZ||reclen>(uint32_t)directory->i_size-pos){error=EIO;goto out;}
...
{
	/* inode_creation_prepare() borrows the endpoint supplied by pathname
	 * bind.  A failed create must never leave that endpoint reachable from a
	 * cached inode, especially when a failed directory rollback leaves the
	 * on-disk name behind and freezes the mount.  The socket caller retains
	 * ownership, so detaching does not destroy the endpoint.
	 */
	if(inode==NULL)
		return;
	mutex_lock(&inode->i_lock);
...
discard_new_inode(struct inode *inode,int directory_counted)
{
	struct ufs1_mount_state *ms=state(inode->i_mount);
	struct ufs1_inode_info *ui=info(inode);
	uint32_t block=ui->direct[0],number=(uint32_t)inode->i_ino;
	...
	uint32_t old_blocks=ui->blocks;
	unsigned n;
	int error=0,cleanup;

	detach_new_socket_special(inode);
	/* A just-created inode can own only the first directory block.  Refuse
	 * to guess how to tear down any other shape. */
	for(n=1;n<UFS1_NDADDR;n++)
	...
	if(block!=0) {
		cleanup=free_block(inode->i_mount,block);
		if(error==0&&cleanup!=0)error=cleanup;
	}
	cleanup=free_inode_number(inode->i_mount,number);
	if(error==0&&cleanup!=0)error=cleanup;
	if(error!=0)
		ms->writable=0;
	inode->i_ino=0;
	...
	int original_error)
{
	...
	/* A directory transaction whose rollback failed may still name this
	 * inode.  Keep its allocation intact and freeze the filesystem. */
	if(!ms->writable){
		detach_new_socket_special(inode);
		inode_release(inode);
		return original_error;
	...
	struct inode **result)
{
	...
	*result=NULL;
	...
	error=allocate_inode_number(mountp,&number);

	if(error)
		return error;
	inode=inode_alloc(mountp);
	if(inode==NULL) {
		error=free_inode_number(mountp,number);
		if(error!=0) {
	...
	inode->i_op=&ufs1_inode_ops;
	inode->i_fop=request->type==INODE_DIR?&ufs1_directory_ops:
		request->type==INODE_REG?&ufs1_regular_ops:
...
static int
ufs1_lookup_locked(struct inode *directory,const struct componentname *component,
	struct inode **result)
{
	...
	struct inode **result)
{
	...
	if (entered)
		mutex_lock(gate);
	error = ufs1_lookup_locked(directory, component, result);
	if (entered)
		mutex_unlock(gate);
	return error;
}
...
static int
ufs1_create(struct inode *directory,const struct componentname *name,
...
static int
ufs1_mkdir(struct inode *directory,const struct componentname *name,
	const struct inode_creation_request *request,struct inode **result)
{
...
static const struct file_ops ufs1_directory_ops={.readdir=ufs1_readdir,.fsync=ufs1_file_sync};
...
static char
ufs1_identity_hex(unsigned value)
{
	return (char)(value < 10U ? '0' + value : 'A' + value - 10U);
}

static void
ufs1_identity_hex32(char output[8], uint32_t value)
{
	unsigned index;

	for (index = 0; index < 8U; index++)
		output[index] = ufs1_identity_hex(
		    (value >> (28U - index * 4U)) & 15U);
}

static void
ufs1_identity_label(char *output, size_t capacity, const uint8_t *input,
		    size_t length)
{
	size_t end = length;
	size_t index;
...
int
ufs1_identify(struct disk *disk, struct block_identity *identity)
{
	struct ufs1_super super;
	uint8_t *buffer;
	...
	if (disk == NULL || identity == NULL)
		return EINVAL;
	if (disk->d_block_size != UFS1_SECTOR_SIZE)
		return EOPNOTSUPP;
	first_block = UFS1_SBLOCK_OFFSET / UFS1_SECTOR_SIZE;
	block_count = UFS1_SBLOCK_SIZE / UFS1_SECTOR_SIZE;
	if (disk->d_block_count < first_block + block_count)
		return EOPNOTSUPP;
	buffer = kern_malloc(UFS1_SBLOCK_SIZE);
	if (buffer == NULL)
		return ENOMEM;
	error = disk_read_direct(disk, first_block, (uint32_t)block_count,
	    buffer);
	if (error == 0)
		error = ufs1_super_decode(buffer, UFS1_SBLOCK_SIZE,
	...
	strcpy(identity->type, "ufs1");
	identity->flags |= ZEDBSD_BLKID_TYPE;
	first = ufs1_get32(buffer, UFS1_FS_ID, super.swapped);
	second = ufs1_get32(buffer, UFS1_FS_ID + 4U, super.swapped);
	if (first != 0U || second != 0U) {
		ufs1_identity_hex32(identity->uuid, first);
		ufs1_identity_hex32(identity->uuid + 8U, second);
		identity->uuid[16] = '\0';
		identity->flags |= ZEDBSD_BLKID_UUID;
	}
	ufs1_identity_label(identity->label, sizeof(identity->label),
	    buffer + UFS1_FS_VOLNAME, UFS1_FS_VOLNAME_SIZE);
...
const struct filesystem_type ufs1_filesystem_type={
	.fs_name="ufs1",.probe=ufs1_probe,.identify=ufs1_identify,
	.mount=ufs1_mount_impl,.sync=ufs1_sync,
