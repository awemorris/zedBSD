...
#define RTL8822B_FIRMWARE_PATH "/lib/firmware/rtw88/rtw8822b_fw.bin"
...
typedef int (*rtl8822b_firmware_chunk_fn)(
	void *context, const struct rtl8822b_firmware_chunk *chunk);
...
enum rtl8822b_rx_kind { RTL8822B_RX_FRAME = 1, RTL8822B_RX_C2H = 2 };
...
int drv_rtl8822b_sha256(const void *data, size_t length, uint8_t digest[32]);
int drv_rtl8822b_firmware_validate(const uint8_t *data, size_t length,
				   struct rtl8822b_firmware_view *view);
/* firmware must be zero initialized or owned by a previous successful load. */
int drv_rtl8822b_firmware_load(struct rtl8822b_firmware_blob *firmware);
void drv_rtl8822b_firmware_release(struct rtl8822b_firmware_blob *firmware);
int drv_rtl8822b_firmware_walk(const struct rtl8822b_firmware_view *view,
			       rtl8822b_firmware_chunk_fn callback,
			       void *context);
int drv_rtl8822b_firmware_tx_descriptor(uint8_t descriptor[48],
					size_t payload_length);

int drv_rtl8822b_efuse_decode(const uint8_t *physical, size_t physical_length,
			      uint8_t *logical, size_t logical_length);
int drv_rtl8822b_chip_identity_parse(uint32_t sys_cfg1,
				     struct rtl8822b_chip_identity *identity);
int drv_rtl8822bu_board_parse(const uint8_t *logical, size_t logical_length,
			      uint32_t sys_cfg1,
			      struct rtl8822bu_board_info *board);
int drv_rtl8822b_board_active_channel_allowed(
	const struct rtl8822bu_board_info *board, uint8_t channel);

int drv_rtl8822b_rx_packet_parse(const uint8_t *bytes, size_t length,
				 struct rtl8822b_rx_packet *packet);
int drv_rtl8822b_rx_aggregate_walk(const uint8_t *bytes, size_t length,
				   rtl8822b_rx_packet_fn callback,
				   void *context, size_t *packet_count);
 ...
 */
int
drv_rtl8822b_radio_power_on(struct rtl8822b_radio *radio,
			    const struct rtl8822b_radio_transport *transport,
			    const struct rtl8822bu_board_info *board,
			    uint64_t deadline_ticks);
int drv_rtl8822b_radio_start(struct rtl8822b_radio *radio,
			     uint64_t deadline_ticks);
int drv_rtl8822b_radio_set_channel(struct rtl8822b_radio *radio,
...
int drv_rtl8822b_radio_rx_generation_resume(struct rtl8822b_radio *radio,
					    uint64_t deadline_ticks);
int drv_rtl8822b_radio_stop(struct rtl8822b_radio *radio,
			    uint64_t deadline_ticks);
int drv_rtl8822b_radio_active_scan_allowed(const struct rtl8822b_radio *radio,
					   uint8_t channel);
int drv_rtl8822b_radio_management_frame_prepare(
	const struct rtl8822b_radio *radio, uint8_t *wire, size_t capacity,
	const uint8_t *frame, size_t frame_length, size_t *wire_length);
int drv_rtl8822b_radio_deauthentication_prepare(
	const struct rtl8822b_radio *radio, uint8_t *wire, size_t capacity,
 ...
 */
int drv_rtl8822b_tx_queues_empty(struct rtl8822b_radio *radio,
				 uint64_t deadline_ticks);
int drv_rtl8822b_security_set_association(struct rtl8822b_radio *radio,
					  const uint8_t bssid[6], uint16_t aid,
					  uint64_t deadline_ticks);
int drv_rtl8822b_security_clear_association(struct rtl8822b_radio *radio,
					    uint64_t deadline_ticks);
int drv_rtl8822b_cam_program_ccmp(struct rtl8822b_radio *radio, uint8_t slot,
				  uint8_t key_index, int group,
				  const uint8_t address[6],
				  const uint8_t key[16],
				  uint64_t deadline_ticks);
int drv_rtl8822b_cam_stage_ccmp(struct rtl8822b_radio *radio, uint8_t slot,
				uint8_t key_index, int group,
				const uint8_t address[6], const uint8_t key[16],
				uint64_t deadline_ticks);
int drv_rtl8822b_cam_activate_ccmp(struct rtl8822b_radio *radio, uint8_t slot,
				   uint8_t key_index, int group,
				   const uint8_t address[6],
				   uint64_t deadline_ticks);
int drv_rtl8822b_cam_clear(struct rtl8822b_radio *radio, uint8_t slot,
			   uint64_t deadline_ticks);
int drv_rtl8822b_data_frame_prepare(const struct rtl8822b_radio *radio,
				    uint8_t *wire, size_t capacity,
				    const uint8_t *frame, size_t frame_length,
				    int encrypted, uint8_t mac_id,
				    uint16_t cookie, size_t *wire_length);

#ifdef RTL8822B_TESTING
int drv_rtl8822b_test_firmware_segment(
	const struct rtl8822b_firmware_view *view, size_t length,
	rtl8822b_firmware_chunk_fn callback, void *context);
int drv_rtl8822b_test_firmware_validate(const uint8_t *data, size_t length,
					const uint8_t expected_digest[32],
					struct rtl8822b_firmware_view *view);
int drv_rtl8822b_test_firmware_walk(const struct rtl8822b_firmware_view *view,
				    const uint8_t expected_digest[32],
				    rtl8822b_firmware_chunk_fn callback,
				    void *context);
int drv_rtl8822b_test_firmware_blob_state(
	const struct rtl8822b_firmware_blob *firmware, int *owned);
int drv_rtl8822b_test_radio_table_apply(struct rtl8822b_radio *radio,
					uint8_t domain, uint8_t width,
					uint8_t rf_path, const uint32_t *words,
					size_t word_count,
					uint64_t deadline_ticks);
#endif
