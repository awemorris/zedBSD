...
void drv_pc98_glyph_default(struct pc98_glyph *backend,
			    struct pc98_display_ops *display,
			    pc98_in8_fn port_in8, pc98_out8_fn port_out8,
			    void *io_context);
int drv_pc98_glyph_make_hal(struct pc98_glyph_ops *hal,
			    struct pc98_glyph *backend);
int drv_pc98_glyph_get_bitmap(struct pc98_glyph *backend, uint32_t codepoint,
			      uint8_t font[32], unsigned *width,
			      unsigned *height);
uint16_t drv_pc98_unicode_to_jis(uint32_t codepoint);
