...
int drv_x68k_scsi_cdb10(uint8_t cdb[10], uint8_t opcode, uint32_t lba,
			uint32_t blocks);
void drv_x68k_scsi_inquiry_cdb(uint8_t cdb[6], uint8_t allocation_length);
void drv_x68k_scsi_request_sense_cdb(uint8_t cdb[6], uint8_t allocation_length);
void drv_x68k_scsi_simple_cdb10(uint8_t cdb[10], uint8_t opcode);
int drv_x68k_scsi_parse_capacity10(const uint8_t response[8], uint64_t *blocks,
				   uint32_t *block_size);
int drv_x68k_scsi_parse_sense(const uint8_t *response, size_t length,
			      struct x68k_scsi_sense *sense);
uint32_t drv_x68k_scsi_transfer_chunk(uint64_t lba, uint32_t blocks,
				      uint64_t capacity, uint32_t driver_limit);
int drv_x68k_spc_pio_init(const struct x68k_spc_bus *bus,
			  unsigned initiator_id);
int drv_x68k_spc_pio_command(const struct x68k_spc_bus *bus,
			     unsigned initiator_id, unsigned target_id,
			     unsigned lun, const uint8_t *cdb,
			     size_t cdb_length, void *data, size_t data_length,
			     enum x68k_spc_direction direction,
			     struct x68k_spc_result *result);
int drv_x68k_spc_pio_read10(const struct x68k_spc_bus *bus,
			    unsigned initiator_id, unsigned target_id,
			    unsigned lun, uint32_t lba, uint32_t blocks,
			    void *buffer, struct x68k_spc_result *result);
int drv_x68k_spc_pio_write10(const struct x68k_spc_bus *bus,
			     unsigned initiator_id, unsigned target_id,
			     unsigned lun, uint32_t lba, uint32_t blocks,
			     const void *buffer,
			     struct x68k_spc_result *result);
int drv_x68k_spc_pio_request_sense(const struct x68k_spc_bus *bus,
				   unsigned initiator_id, unsigned target_id,
				   unsigned lun, uint8_t response[18],
				   struct x68k_spc_result *result);
int drv_x68k_spc_pio_inquiry(const struct x68k_spc_bus *bus,
			     unsigned initiator_id, unsigned target_id,
			     unsigned lun, uint8_t response[36],
			     struct x68k_spc_result *result);
int drv_x68k_spc_pio_read_capacity10(const struct x68k_spc_bus *bus,
				     unsigned initiator_id, unsigned target_id,
				     unsigned lun, uint8_t response[8],
				     struct x68k_spc_result *result);
int drv_x68k_spc_pio_test_unit_ready(const struct x68k_spc_bus *bus,
				     unsigned initiator_id, unsigned target_id,
				     unsigned lun,
				     struct x68k_spc_result *result);
int drv_x68k_spc_pio_synchronize10(const struct x68k_spc_bus *bus,
				   unsigned initiator_id, unsigned target_id,
				   unsigned lun,
				   struct x68k_spc_result *result);
