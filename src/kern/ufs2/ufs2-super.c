	...
	struct ufs2_super *super)
{
	...
	if (buffer == NULL || super == NULL || length < UFS2_FS_STRUCT_SIZE)
		return EINVAL;
	magic = ufs1_get32(buffer, UFS2_FS_MAGIC, 0);
	if (magic == UFS2_MAGIC)
		swapped = 0;
	else if (ufs1_get32(buffer, UFS2_FS_MAGIC, 1) == UFS2_MAGIC)
		swapped = 1;
	else
		return EOPNOTSUPP;
	memset(super, 0, sizeof(*super));
#define GET32(field, offset) super->field = ufs1_get32(buffer, offset, swapped)
#define GET64(field, offset) super->field = ufs1_get64(buffer, offset, swapped)
