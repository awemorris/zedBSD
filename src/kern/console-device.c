			 ...
			 uintptr_t argument)
{
	...
	(void)file;
	switch (request) {
	case BOOTS_CONSOLE_GET_SIZE: {
		const struct boots_console_size size = {
			HAL_CONS_ROWS, HAL_CONS_COLUMNS
		};
		return copyout(&size, argument, sizeof(size));
	}
	case BOOTS_CONSOLE_CLEAR:
		hal_cons_clear();
		return 0;
	case BOOTS_CONSOLE_CLEAR_ROW: {
		struct boots_console_row row;
	...
	case BOOTS_CONSOLE_CLEAR_TO_EOL: {
		struct boots_console_position position;
		error = copyin(argument, &position, sizeof(position));
		if (error != 0) return error;
		return hal_cons_clear_to_eol_at(position.row, position.column) ? 0 : EINVAL;
	}
	case BOOTS_CONSOLE_GET_CURSOR: {
		struct boots_console_cursor cursor;
	...
	case BOOTS_CONSOLE_WRITE_AT:
		return console_write_at(argument);
	case BOOTS_CONSOLE_POLL_EVENT:
	case BOOTS_CONSOLE_READ_EVENT: {
		struct boots_console_event event;
		int value = request == BOOTS_CONSOLE_POLL_EVENT ?
