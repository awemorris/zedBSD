...
flockfile(FILE *stream)
{
	...
	/*
	 * The lock word is 0 when free, 1 when held and 2 when held with a
	 * thread waiting: only an unlock that finds 2 has anyone to wake.
	 */
	expected = 0;
	if (!__atomic_compare_exchange_n(&stream->lock, &expected, 1U, 0,
	    __ATOMIC_ACQUIRE, __ATOMIC_RELAXED)) {
		while (__atomic_exchange_n(&stream->lock, 2U,
		    __ATOMIC_ACQUIRE) != 0) {
...
funlockfile(FILE *stream)
{
	...
	if (--stream->lock_depth != 0)
		return;
	stream->lock_owner = 0;
	if (__atomic_exchange_n(&stream->lock, 0U, __ATOMIC_RELEASE) == 2U &&
	    __stdio_lock_wake != NULL)
