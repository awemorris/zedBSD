...
static int block_in_heap(const struct heap_allocator *heap, const struct heap_block *block);
static struct heap_block *pointer_block(const struct heap_allocator *heap, void *pointer);
static int extend_heap(struct heap_allocator *heap, size_t minimum);
...
/*
 * Returns whether a block header lies wholly inside the heap at an aligned
 * address, so that its fields may be read.
 */
static int
block_in_heap(
	const struct heap_allocator *heap,
	const struct heap_block *block)
{
	const uint8_t *bytes;
	size_t header;

	/* Measures a header against the address. */
	bytes = (const uint8_t *)block;
	header = block_header_size();

	/* Refuses an address before the heap or too near its end for a header. */
	if (bytes < heap->begin)
		return 0;
	if ((size_t)(heap->end - bytes) < header)
		return 0;

	/* Refuses an address no block could start at. */
	if ((uintptr_t)bytes % HEAP_ALIGNMENT != 0)
		return 0;

	/* A header fits here and may be read. */
	return 1;
}

/*
 * Returns the block whose payload a pointer is, or NULL for a pointer the
 * heap never returned.
 *
 * The header before the pointer must carry the magic, and its neighbours must
 * point back at it: the previous block's next and the next block's previous
 * are the block itself, and a block with no neighbour on a side is the first
 * or the last.  A block merged into another has lost its magic, so a stale
 * pointer is refused as well.  This asks the same question as a walk of the
 * whole chain did, without the walk: that made every free and realloc cost
 * time in proportion to the number of blocks (BUG-033).
 */
static struct heap_block *
pointer_block(
	const struct heap_allocator *heap,
	void *pointer)
{
	...
	/* Refuses a pointer when there is no heap to hold it. */
	if (heap == NULL || pointer == NULL || heap->begin == NULL)
		return NULL;

	/* Refuses a pointer that cannot follow a header inside the heap. */
	bytes = pointer;
	header = block_header_size();
	if (bytes < heap->begin + header || bytes >= heap->end)
		return NULL;

	/* Refuses a pointer whose header is not a block's. */
	block = (struct heap_block *)(bytes - header);
	inside = block_in_heap(heap, block);
	if (!inside)
		return NULL;
	if (block->magic != HEAP_MAGIC)
		return NULL;

	/* Refuses a block the previous one does not lead to. */
	previous = block->previous_physical;
	if (previous == NULL) {
		if (block != heap->first)
			return NULL;
	} else {
		inside = block_in_heap(heap, previous);
		if (!inside)
			return NULL;
		if (previous->next_physical != block)
			return NULL;
	}

	/* Refuses a block the next one does not lead back from. */
	next = block->next_physical;
	if (next == NULL) {
		if (block != heap->last)
			return NULL;
	} else {
		inside = block_in_heap(heap, next);
		if (!inside)
			return NULL;
		if (next->previous_physical != block)
			return NULL;
	}

	/* The block is in the chain. */
	return block;
}
...
/*
 * Grows the heap by at least a minimum at its end.
 *
 * The growth joins the last block when that is free, and otherwise becomes a
 * free block of its own after it.  It reports 1 when the heap grew.
 */
static int
extend_heap(
	...
	size_t minimum)
{
	...
	/* Refuses to grow a heap that has no way to grow. */
	if (heap->grow == NULL || heap->end == NULL)
		return 0;

	/* Asks for the memory after the heap's end. */
	added = heap->grow(heap->grow_context, heap->end, minimum);
	if (added < minimum || (added & (HEAP_ALIGNMENT - 1U)) != 0)
		return 0;
	if (added > UINTPTR_MAX - (uintptr_t)heap->end)
		return 0;
	if (heap->original_size > SIZE_MAX - added)
		return 0;

	/* Finds the block the growth follows. */
	last = heap->last;
	if (last == NULL)
		return 0;

	/* A free last block takes the growth into its own capacity. */
	if (last->state == HEAP_FREE) {
		last->capacity += added;
		heap->end += added;
		heap->original_size += added;

		/* The heap grew inside its last block. */
		return 1;
	}

	/* Refuses a growth too small to hold a block of its own. */
	header = block_header_size();
	if (added < header + HEAP_ALIGNMENT)
		return 0;

	/* Places a free block over the growth and links it after the last. */
	block = (struct heap_block *)heap->end;
	block->magic = HEAP_MAGIC;
	block->state = HEAP_FREE;
	block->capacity = added - header;
	block->used = 0;
	...
	/* Moves the heap's end past the new block. */
	heap->end += added;
	heap->original_size += added;

	/* The heap grew by a block. */
	return 1;
}
...
heap_allocator_validate(const struct heap_allocator *heap)
{
	...
	/* The block the chain ends at is the one growth follows. */
