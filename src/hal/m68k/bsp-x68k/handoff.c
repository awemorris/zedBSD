...
x68k_boot_handoff_valid(const struct x68k_boot_handoff *handoff)
{
	...
	if (handoff == NULL || handoff->common.magic != KERN_HANDOFF_MAGIC ||
	    ...
	    KERN_BOOT_PARTITION_LBA_UNKNOWN ||
	    ...
	    handoff->memory_region_count > KERN_X68K_MAX_MEMORY_REGIONS)
		return 0;
	for (index = 0; index < handoff->memory_region_count; index++) {
		const struct boot_memory_region32 *region =
			&handoff->memory_regions[index];
		if (region->size == 0 ||
		    (region->type != KERN_MEMORY_AVAILABLE &&
		     region->type != KERN_MEMORY_RESERVED) ||
