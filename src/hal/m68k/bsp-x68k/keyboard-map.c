...
#define X68K_KEY_CAPS  0x5dU
#define X68K_KEY_SHIFT 0x70U
#define X68K_KEY_CTRL  0x71U
#define X68K_KEY_OPT1  0x72U
...
static const uint16_t normal_map[128] = {
	...
	[0x34] = '_', [0x35] = ' ',
	[0x36] = HAL_KEY_HOME, [0x37] = HAL_KEY_DELETE,
	[0x38] = HAL_KEY_PAGE_UP, [0x39] = HAL_KEY_PAGE_DOWN,
	[0x3a] = HAL_KEY_END, [0x3b] = HAL_KEY_LEFT,
	[0x3c] = HAL_KEY_UP, [0x3d] = HAL_KEY_RIGHT,
	[0x3e] = HAL_KEY_DOWN,
	[0x40] = '/', [0x41] = '*', [0x42] = '-', [0x43] = '7',
	[0x44] = '8', [0x45] = '9', [0x46] = '+', [0x47] = '4',
	[0x48] = '5', [0x49] = '6', [0x4a] = '=', [0x4b] = '1',
	[0x4c] = '2', [0x4d] = '3', [0x4e] = HAL_KEY_ENTER,
	[0x4f] = '0', [0x50] = ',', [0x51] = '.',
	[0x63] = HAL_KEY_F1, [0x64] = HAL_KEY_F2,
	[0x65] = HAL_KEY_F3, [0x66] = HAL_KEY_F4,
	[0x67] = HAL_KEY_F5, [0x68] = HAL_KEY_F6,
	[0x69] = HAL_KEY_F7, [0x6a] = HAL_KEY_F8,
	[0x6b] = HAL_KEY_F9, [0x6c] = HAL_KEY_F10,
};
...
{
	unsigned scan;
	if (key == HAL_KEY_SHIFT) return X68K_KEY_SHIFT;
...
{
	return (scan_down(state, X68K_KEY_SHIFT) ? HAL_KEY_EVENT_SHIFT : 0U) |
	    (scan_down(state, X68K_KEY_CTRL) ? HAL_KEY_EVENT_CTRL : 0U) |
	    (scan_down(state, X68K_KEY_OPT1) ? HAL_KEY_EVENT_GRAPH : 0U);
}
...
x68k_keyboard_feed(struct x68k_keyboard_state *state, uint8_t raw)
{
	...
	if (released)
		...
		state->down[scan >> 3] |= (uint8_t)(1U << (scan & 7U));
	if (scan == X68K_KEY_CAPS && !released)
		state->caps_lock ^= 1U;
	if (released || scan == X68K_KEY_CAPS || scan == X68K_KEY_SHIFT ||
	    scan == X68K_KEY_CTRL || scan == X68K_KEY_OPT1)
		return -1;
	key = scan_down(state, X68K_KEY_SHIFT) && shift_map[scan] != 0 ?
		shift_map[scan] : normal_map[scan];
	if (key >= 'a' && key <= 'z') {
		if (scan_down(state, X68K_KEY_SHIFT) ^ state->caps_lock)
			key = (uint16_t)(key - 'a' + 'A');
		if (scan_down(state, X68K_KEY_CTRL))
