...
valid_user_pc_sp(hal_space_t space, uintptr_t pc, uintptr_t sp)
{
	...
	if (space == HAL_SPACE_SYS || pc < M68K030_PAGE_SIZE ||
		...
		return 0;
	stack_byte = sp - 1U;
	if (hal_page_query(space, (void *)(pc & ~M68K030_PAGE_MASK),
	    &flags) != HAL_PMEM_SUCCESS || (flags & HAL_PAGE_PRESENT) == 0)
		return 0;
	return hal_page_query(space,
		(void *)(stack_byte & ~M68K030_PAGE_MASK), &flags) ==
		HAL_PMEM_SUCCESS && (flags & HAL_PAGE_PRESENT) != 0;
}
		      ...
		      uintptr_t restorer, uint32_t token)
{
	...
	if (running_task == NULL || (frame = running_task->active_user_frame) ==
	    NULL || running_task->signal_depth != 0 || handler == 0 ||
	    stack == 0 || token == 0)
		return -1;
	...
	if (size <= M68K_FRAME_HARDWARE_OFFSET ||
	    size > sizeof(running_task->signal_frame))
		return -1;
	hal_memcpy(running_task->signal_frame, frame, size);
	running_task->signal_frame_size = size;
	running_task->signal_token = token;
	running_task->signal_depth = 1;
...
hal_task_signal_return(uint32_t token, intptr_t *return_value)
{
	struct m68k_saved_frame *frame;
	if (running_task == NULL || (frame = running_task->active_user_frame) ==
	    NULL || return_value == NULL || running_task->signal_depth != 1 ||
	    token == 0 || token != running_task->signal_token)
		return -1;
	hal_memcpy(frame, running_task->signal_frame,
		running_task->signal_frame_size);
