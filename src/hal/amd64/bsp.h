...
uint64_t bsp_mem_probe(void);
uint32_t bsp_mem_range_count(void);
int bsp_mem_range(uint32_t index, uint64_t *base, uint64_t *size, uint32_t *type);
int bsp_physical_range_mappable(uint64_t physical, size_t size);
