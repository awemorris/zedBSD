...
static int
snapshot_equal(
	const struct rtc_snapshot *a,
	const struct rtc_snapshot *b)
{
...
static int
snapshot_read(
	x86_cmos_read_fn read,
	void *context,
	struct rtc_snapshot *out)
{
	unsigned poll;
...
static int
snapshot_seconds(
	const struct rtc_snapshot *snapshot,
	uint64_t *result)
{
	...
	/* PC/AT HAL intentionally supports 2000..2099. */
	year += 2000U;
