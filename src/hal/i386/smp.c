...
{
	unsigned i,target=1;uint8_t bsp=i386_lapic_id();hal_memset(cpus,0,sizeof(cpus));cpus[0].apic_id=bsp;
...
int i386_smp_apic_id(hal_cpu_id_t cpu,uint8_t*id){if(cpu>=cpu_count||id==NULL)return HAL_ERR_INVALID;*id=cpus[cpu].apic_id;return HAL_OK;}

static int start_one(hal_cpu_id_t cpu)
{
	uint8_t*dst=(uint8_t*)(uintptr_t)(TRAMPOLINE_PHYS|0x80000000U);size_t size=(size_t)(i386_ap_trampoline_end-i386_ap_trampoline_start);unsigned timeout;
	...
	hal_memcpy(dst,i386_ap_trampoline_start,size);*(uint32_t*)(dst+off(i386_ap_trampoline_cr3))=asm_get_cr3();
	*(uint32_t*)(dst+off(i386_ap_trampoline_stack))=(uint32_t)(uintptr_t)cpus[cpu].stack+AP_STACK_SIZE;
	*(uint32_t*)(dst+off(i386_ap_trampoline_cpu))=cpu;*(uint32_t*)(dst+off(i386_ap_trampoline_target))=(uint32_t)(uintptr_t)i386_ap_high_entry;hal_wmb();
...
void i386_smp_ap_entry(uint32_t cpu)
{
	...
	ready_mask.bits[cpu/64U]|=(uint64_t)1U<<(cpu%64U);hal_wmb();kernel_secondary_entry(cpu);HAL_FATAL("secondary kernel entry returned");
}
...
int i386_smp_send_tlb(hal_cpu_id_t cpu){if(cpu>=cpu_count)return HAL_ERR_INVALID;if(cpu!=0&&!cpus[cpu].ready)return HAL_ERR_STATE;return i386_interrupt_uses_apic()?i386_lapic_send_fixed(cpus[cpu].apic_id,VECTOR_TLB):HAL_ERR_UNSUPPORTED;}
