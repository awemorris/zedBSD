...
uint64_t arm64_irq_save(void)
{
	uint64_t state;
	__asm__ volatile("mrs %0, daif\n\tmsr daifset, #2" : "=r"(state) :: "memory");
	return state;
}
void arm64_irq_restore(uint64_t state)
{
...
uint8_t hal_mmio_read8(const volatile void*p){uint8_t v=*(const volatile uint8_t*)p;hal_io_rmb();return v;}
uint16_t hal_mmio_read16(const volatile void*p){uint16_t v=*(const volatile uint16_t*)p;hal_io_rmb();return v;}
uint32_t hal_mmio_read32(const volatile void*p){uint32_t v=*(const volatile uint32_t*)p;hal_io_rmb();return v;}
uint64_t hal_mmio_read64(const volatile void*p){uint64_t v=*(const volatile uint64_t*)p;hal_io_rmb();return v;}
void hal_mmio_write8(volatile void*p,uint8_t v){*(volatile uint8_t*)p=v;hal_io_wmb();}
void hal_mmio_write16(volatile void*p,uint16_t v){*(volatile uint16_t*)p=v;hal_io_wmb();}
void hal_mmio_write32(volatile void*p,uint32_t v){*(volatile uint32_t*)p=v;hal_io_wmb();}
void hal_mmio_write64(volatile void*p,uint64_t v){*(volatile uint64_t*)p=v;hal_io_wmb();}
void hal_halt(void){arm64_wfi();}

static void port_io_fatal(void){HAL_FATAL("port I/O is unavailable on Raspberry Pi 4");}
uint8_t hal_io_inp8(uint16_t p){(void)p;port_io_fatal();return 0;}
uint16_t hal_io_inp16(uint16_t p){(void)p;port_io_fatal();return 0;}
uint32_t hal_io_inp32(uint16_t p){(void)p;port_io_fatal();return 0;}
void hal_io_outp8(uint16_t p,uint8_t v){(void)p;(void)v;port_io_fatal();}
void hal_io_outp16(uint16_t p,uint16_t v){(void)p;(void)v;port_io_fatal();}
void hal_io_outp32(uint16_t p,uint32_t v){(void)p;(void)v;port_io_fatal();}

static size_t cache_line_size(void)
{uint64_t ctr;__asm__ volatile("mrs %0,ctr_el0":"=r"(ctr));return(size_t)4U<<(ctr>>16&15U);}
...
void hal_reset(void)
{
	volatile uint8_t *pm=(volatile uint8_t *)(0xffff000000000000ULL+0xfe100000ULL);
