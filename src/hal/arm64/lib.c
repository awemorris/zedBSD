...
{ uint8 *d = p; while (n-- != 0) *d++ = (uint8)c; return p; }
void *hal_memset16(uint16 *p, uint16 c, size_t n)
{ uint16 *d = p; while (n-- != 0) *d++ = c; return p; }
void *hal_memset32(uint32 *p, uint32 c, size_t n)
{ uint32 *d = p; while (n-- != 0) *d++ = c; return p; }
void *hal_memcpy(void *d0, const void *s0, size_t n)
{ uint8 *d = d0; const uint8 *s = s0; while (n-- != 0) *d++ = *s++; return d0; }
static void *(*alloc_fn)(size_t);
static void (*free_fn)(void *);
void hal_set_allocator(void *(*a)(size_t), void (*f)(void *)) { alloc_fn = a; free_fn = f; }
...
int hal_printf(const char *fmt, ...)
{
	...
	while (*fmt != '\0') {
		...
		case 'u': put_u64(long_arg ? __builtin_va_arg(ap, uint64) : __builtin_va_arg(ap, uint32), 10, width); break;
		case 'x': put_u64(long_arg ? __builtin_va_arg(ap, uint64) : __builtin_va_arg(ap, uint32), 16, width); break;
