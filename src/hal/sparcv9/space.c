...
int hal_page_map(hal_space_t h,void*v,hal_physaddr_t p,size_t n,uint32 attr){struct sparcv9_space*s=h;uintptr_t a=(uintptr_t)v,o;if(!valid(s)||!valid_range(a,n)||(p&SPARCV9_PAGE_MASK)||p>=hal_pmem_get_total_size()||n>hal_pmem_get_total_size()-p||!(attr&(HAL_SPACE_READ|HAL_SPACE_WRITE|HAL_SPACE_EXEC))||((attr&HAL_SPACE_WRITE)&&(attr&HAL_SPACE_EXEC)))return HAL_ERR_INVALID;for(o=0;o<n;o+=SPARCV9_PAGE_SIZE)if(find(s,a+o))return HAL_ERR_INVALID;for(o=0;o<n;o+=SPARCV9_PAGE_SIZE){struct sparcv9_mapping*m=hal_malloc(sizeof(*m));if(!m){(void)hal_page_unmap(s,v,o);return HAL_ERR_NOMEM;}m->virtual_address=a+o;m->physical_address=p+o;m->attributes=attr;m->flags=HAL_PAGE_PRESENT;m->next=s->mappings;s->mappings=m;page_table_count++;}return 0;}
int hal_page_prot(hal_space_t h,void*v,size_t n,uint32 attr){struct sparcv9_space*s=h;uintptr_t a=(uintptr_t)v,o;if(!valid(s)||!valid_range(a,n)||!(attr&(HAL_SPACE_READ|HAL_SPACE_WRITE|HAL_SPACE_EXEC))||((attr&HAL_SPACE_WRITE)&&(attr&HAL_SPACE_EXEC)))return HAL_ERR_INVALID;for(o=0;o<n;o+=SPARCV9_PAGE_SIZE){struct sparcv9_mapping*m=find(s,a+o);if(!m)return HAL_ERR_INVALID;m->attributes=attr;}if(current_space==s)flush_unlocked();return 0;}
int hal_page_unmap(hal_space_t h,void*v,size_t n){struct sparcv9_space*s=h;uintptr_t a=(uintptr_t)v,o;if(!n)return 0;if(!valid(s)||!valid_range(a,n))return HAL_ERR_INVALID;for(o=0;o<n;o+=SPARCV9_PAGE_SIZE){struct sparcv9_mapping**link=&s->mappings;while(*link&&(*link)->virtual_address!=a+o)link=&(*link)->next;if(*link){struct sparcv9_mapping*m=*link;*link=m->next;hal_free(m);if(page_table_count)page_table_count--;}}if(current_space==s)flush_unlocked();return 0;}
int hal_page_query(hal_space_t h,void*v,uint32*f){struct sparcv9_space*s=h;struct sparcv9_mapping*m;if(!valid(s)||!f||!valid_range((uintptr_t)v,SPARCV9_PAGE_SIZE))return HAL_ERR_INVALID;m=find(s,(uintptr_t)v);*f=m?m->flags:0;return 0;}
int hal_page_clear_flags(hal_space_t h,void*v,uint32 f){struct sparcv9_space*s=h;struct sparcv9_mapping*m;if(!valid(s)||!valid_range((uintptr_t)v,SPARCV9_PAGE_SIZE)||(f&~(HAL_PAGE_ACCESSED|HAL_PAGE_DIRTY)))return HAL_ERR_INVALID;m=find(s,(uintptr_t)v);if(!m)return HAL_ERR_INVALID;m->flags&=~f;if(current_space==s)flush_unlocked();return 0;}
