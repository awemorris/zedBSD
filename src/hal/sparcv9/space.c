...
void hal_page_switch_space(hal_space_t h){struct sparcv9_space*s=h;if(s&&!valid(s))HAL_FATAL("invalid SPARC V9 space switch");if(s==current_space)return;current_space=s;sparcv9_set_primary_context(s?s->context:0);}
...
int sparcv9_resolve_miss(uintptr_t address,int instruction,int write)
{
	...
	tte=sparcv9_tte(m->physical_address,flags);m->flags|=HAL_PAGE_ACCESSED;if(write)m->flags|=HAL_PAGE_DIRTY;
	slot=(page>>10)&0x1f8U;
