#ifndef KERN_HAL_SPARCV9_TASK_H
#define KERN_HAL_SPARCV9_TASK_H

#include <hal/hal.h>
#include "trap.h"
#define SPARCV9_SYS_STACK_SIZE 32768U
#define SPARCV9_NWINDOWS 8U
struct sparcv9_window_context{uint64_t window[SPARCV9_NWINDOWS][16];uint64_t cwp,cansave,canrestore,otherwin,cleanwin,wstate;uint64_t global[8];};
struct sparcv9_task{struct sparcv9_task*next;hal_space_t space;int run_cpu;void*stack_allocation;void*stack;uintptr_t trap_sp;void*private_data;uintptr_t tls;uint64_t trap_level,trap_pc,trap_next_pc,trap_state,trap_type;struct sparcv9_window_context windows,trap_windows;struct sparcv9_user_trap_frame*active_user_frame;uint64_t active_pc,active_next_pc,active_tstate,active_trap_type;struct sparcv9_window_context signal_windows[HAL_SIGNAL_NEST_MAX];struct sparcv9_user_trap_frame signal_frame[HAL_SIGNAL_NEST_MAX];uint64_t signal_pc[HAL_SIGNAL_NEST_MAX],signal_next_pc[HAL_SIGNAL_NEST_MAX],signal_tstate[HAL_SIGNAL_NEST_MAX];uint32_t signal_token[HAL_SIGNAL_NEST_MAX],signal_depth;};
...
void sparcv9_task_enter_user_frame(struct sparcv9_user_trap_frame *,
	uintptr_t, uintptr_t, uint64_t, uint64_t);
