...
display_enter(void *context, struct noct_beui_display_info *info)
{
	...
	if (info == NULL)
		return 0;
	if (graphics_fd < 0)
		graphics_fd = open("/dev/graphics", O_RDWR);
	if (graphics_fd < 0) {
...
static int
display_poll(void *context)
{
	(void)context;
	return 1;
}
static void
rect_copy(struct graphics_rect *to, const struct noct_beui_rect *from)
{
	to->x = from->x;
	to->y = from->y;
	to->width = from->width;
...
static int
display_fill(void *context, const struct noct_beui_rect *rect, uint32_t color)
{
	struct graphics_fill request;
	(void)context;
	memset(&request, 0, sizeof(request));
	rect_copy(&request.rect, rect);
	request.color = color;
	return graphics_fd >= 0 &&
	       ioctl(graphics_fd, ZEDBSD_GRAPHICS_FILL_RECT, &request) == 0;
}
static int
display_line(void *context, unsigned x0, unsigned y0, unsigned x1, unsigned y1,
	     uint32_t color)
{
	struct graphics_line request = {x0, y0, x1, y1, color, 0};
	(void)context;
	return graphics_fd >= 0 &&
	       ioctl(graphics_fd, ZEDBSD_GRAPHICS_DRAW_LINE, &request) == 0;
}
static int
display_pattern(void *context, const struct noct_beui_rect *rect,
		uint32_t color, uint64_t pattern)
{
	...
	return graphics_fd >= 0 &&
	       ioctl(graphics_fd, ZEDBSD_GRAPHICS_PATTERN_FILL, &request) == 0;
}
static int
display_image_common(void *context, unsigned x, unsigned y,
		     ...
		     int patterned)
{
	...
	request.format = image->format == NOCT_BEUI_IMAGE_RGB24
			     ? ZEDBSD_GRAPHICS_FORMAT_RGB24
			     : ZEDBSD_GRAPHICS_FORMAT_INDEX8;
	...
	if (result != 0)
		fprintf(stderr,
			...
			errno, request.x, request.y, request.width,
			request.height, request.format, request.stride,
			request.pixels, request.palette, request.palette_count,
			display_info.width, display_info.height,
			display_info.bits_per_pixel);
	return result == 0;
}
static int
display_image(void *c, unsigned x, unsigned y, const struct noct_beui_image *i)
{
	return display_image_common(c, x, y, i, 0, 0);
}
static int
display_image_pattern(void *c, unsigned x, unsigned y,
		      const struct noct_beui_image *i, uint64_t p)
{
	return display_image_common(c, x, y, i, p, 1);
}
static int
display_flush(void *context, const struct noct_beui_rect *rectangles,
	      size_t count)
{
	struct graphics_rect converted[32];
	...
	if (count > 32U)
		return 0;
	for (i = 0; i < count; i++)
		rect_copy(&converted[i], &rectangles[i]);
	request.rectangles = count == 0 ? 0U : (uapi_ptr_t)(uintptr_t)converted;
	request.rectangle_count = (uint32_t)count;
	return graphics_fd >= 0 &&
	       ioctl(graphics_fd, ZEDBSD_GRAPHICS_FLUSH, &request) == 0;
}
static int
	      ...
	      unsigned *height)
{
	...
	request.bitmap = (uapi_ptr_t)(uintptr_t)bitmap;
	request.bitmap_capacity = sizeof(bitmap);
	if (graphics_fd < 0 ||
	    ioctl(graphics_fd, ZEDBSD_GRAPHICS_GET_GLYPH, &request) != 0)
		return 0;
	*width = request.width;
	   ...
	   uint32_t foreground, uint32_t background)
{
	...
	glyph.bitmap_capacity = sizeof(bitmap);
	if (graphics_fd < 0 ||
	    ioctl(graphics_fd, ZEDBSD_GRAPHICS_GET_GLYPH, &glyph) != 0)
		return 0;
...
static int
key_state(void *context, int key)
{
	struct console_key_state request = {(uint32_t)key, 0};
	(void)context;
	return get_console() >= 0 && ioctl(console_fd, ZEDBSD_CONSOLE_KEY_STATE,
					   &request) == 0
		   ? request.down
		   : -1;
}
...
input_drain(void *context)
{
	(void)context;
	if (get_console() >= 0)
		(void)ioctl(console_fd, ZEDBSD_CONSOLE_DRAIN_INPUT);
}

static struct noct_beui_hal beui = {
    .display = {NULL, display_enter, display_leave, display_poll, display_fill,
		display_line, display_pattern, display_image,
		display_image_pattern, display_flush},
