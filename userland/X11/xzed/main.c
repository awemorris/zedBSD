...
struct server {
	...
	uint32_t focus;
	int pointer_grab_owner;uint32_t pointer_grab_window;
};
...
{struct window*p;while(w&&w->parent!=ROOT_XID){p=find_window(s,w->parent);if(!p||p==w)break;w=p;}return w;}
static void raise_window(struct server*s,struct window*w)
{
	struct window saved;size_t index;if(!w||w==&s->windows[0])return;
	index=(size_t)(w-s->windows);if(index>=s->window_count||index+1U==s->window_count)return;
	mark_dirty(s,w->x,w->y,w->width,w->height);saved=*w;
...
{unsigned i;for(i=0;i<s->pixmap_count;i++)if(s->pixmaps[i].id==id)return &s->pixmaps[i];return NULL;}
...
static void mark_dirty(struct server*s,int x,int y,int w,int h)
{
	...
	if(w<=0||h<=0)return;
	if(!s->dirty){s->dirty=1;s->dirty_x0=x;s->dirty_y0=y;s->dirty_x1=x+w;s->dirty_y1=y+h;return;}
	if(x<s->dirty_x0)s->dirty_x0=x;
	if(y<s->dirty_y0)s->dirty_y0=y;
	if(x+w>s->dirty_x1)s->dirty_x1=x+w;
	if(y+h>s->dirty_y1)s->dirty_y1=y+h;
}
static struct window*top_at_parent(struct server*s,uint32_t parent,int x,int y)
{
	unsigned i=s->window_count;while(i-->1){struct window*w=&s->windows[i];if(w->mapped&&w->parent==parent&&x>=w->x&&y>=w->y&&x<w->x+w->width&&y<w->y+w->height){struct window*child=top_at_parent(s,w->id,x,y);return child?child:w;}}return NULL;
}
...
static int resize_cursor_black(uint16_t shape,int x,int y)
{
	...
	if(shape==XC_SB_H_DOUBLE_ARROW)
		return (ady<=1&&x>=2&&x<=13)||(x<=6&&adx+ady<=5)||
		    (x>=9&&adx+ady<=6);
	if(shape==XC_SB_V_DOUBLE_ARROW)
		return (adx<=1&&y>=2&&y<=13)||(y<=6&&adx+ady<=5)||
		    (y>=9&&adx+ady<=6);
	if(shape==XC_BOTTOM_LEFT_CORNER)
		return (abs(x+y-15)<=1&&x>=2&&x<=13&&y>=2&&y<=13)||
		    (x==2&&y>=8&&y<=13)||(y==13&&x>=2&&x<=7)||
		    (x==13&&y>=2&&y<=7)||(y==2&&x>=8&&x<=13);
	if(shape==XC_BOTTOM_RIGHT_CORNER)
		return (abs(x-y)<=1&&x>=2&&x<=13&&y>=2&&y<=13)||
		    (x==2&&y>=2&&y<=7)||(y==2&&x>=2&&x<=7)||
		    (x==13&&y>=8&&y<=13)||(y==13&&x>=8&&x<=13);
	return 0;
}
static int cursor_pixel(uint16_t shape,int x,int y,uint32_t*color)
{
	int nx,ny;
	if(shape==XC_LEFT_PTR){uint16_t bit=(uint16_t)(1U<<(unsigned)x);if(!(pointer_mask[y]&bit))return 0;*color=(pointer_source[y]&bit)?0x000000:0xffffff;return 1;}
	if(resize_cursor_black(shape,x,y)){*color=0x000000;return 1;}
	for(ny=y-1;ny<=y+1;ny++)for(nx=x-1;nx<=x+1;nx++)
		if(nx>=0&&ny>=0&&nx<CURSOR_WIDTH&&ny<CURSOR_HEIGHT&&
		    resize_cursor_black(shape,nx,ny)){*color=0xffffff;return 1;}
	return 0;
}
static uint32_t*window_pixels_alloc(uint16_t width,uint16_t height,uint32_t color)
{
	size_t count=(size_t)width*height,i;uint32_t*p;if(!width||!height||(width&&count/width!=height)||count>SIZE_MAX/sizeof(*p))return NULL;p=malloc(count*sizeof(*p));if(!p)return NULL;for(i=0;i<count;i++)p[i]=color;return p;
}
...
{
	int row,column;if(x<0){width+=x;x=0;}if(y<0){height+=y;y=0;}if(x+width>(int)s->mode.width)width=(int)s->mode.width-x;if(y+height>(int)s->mode.height)height=(int)s->mode.height-y;if(width<=0||height<=0)return;for(row=y;row<y+height;row++)for(column=x;column<x+width;column++){struct window*w=top_at(s,column,row);int local_x=column-w->x,local_y=row-w->y;s->screen[(size_t)row*s->mode.width+column]=w->pixels[(size_t)local_y*w->width+local_x];}
}
static void window_fill(struct server*s,struct window*w,int x,int y,int width,int height,uint32_t color)
{
	int row,column;if(x<0){width+=x;x=0;}if(y<0){height+=y;y=0;}
	if(x+width>w->width)width=w->width-x;
	if(y+height>w->height)height=w->height-y;
	if(width<=0||height<=0)return;
	for(row=y;row<y+height;row++)for(column=x;column<x+width;column++)w->pixels[(size_t)row*w->width+column]=color;
...
{
	int row,column;if(x<0){width+=x;x=0;}if(y<0){height+=y;y=0;}if(x+width>p->width)width=p->width-x;if(y+height>p->height)height=p->height-y;if(width<=0||height<=0)return;for(row=y;row<y+height;row++)for(column=x;column<x+width;column++)p->pixels[(size_t)row*p->width+column]=color;
}
...
{struct client*c=owner_client(s,w->owner);uint8_t e[32];if(!c||!(w->event_mask&(1U<<17)))return;memset(e,0,sizeof(e));e[0]=22;wr16(e+2,c->sequence,c->order);wr32(e+4,w->id,c->order);wr32(e+8,w->id,c->order);wr32(e+12,0,c->order);wr16(e+16,(uint16_t)w->x,c->order);wr16(e+18,(uint16_t)w->y,c->order);wr16(e+20,w->width,c->order);wr16(e+22,w->height,c->order);wr16(e+24,w->border,c->order);(void)write_all(c->fd,e,sizeof(e));}
...
static int request(struct server *s,unsigned ci,const uint8_t *q,size_t n)
{
	...
	case 1: /* CreateWindow */
		...
		w->pixels=window_pixels_alloc(w->width,w->height,w->background);if(!w->pixels){s->window_count--;memset(w,0,sizeof(*w));error_reply(c,11,id,op);return 0;}
		return 0;
	...
	case 12: /* ConfigureWindow */ if(n>=12&&(w=find_window(s,rd32(q+4,c->order)))!=NULL){uint16_t mask=rd16(q+8,c->order);size_t off=12;unsigned bit;int oldx=w->x,oldy=w->y,oldw=w->width,oldh=w->height,newx=w->x,newy=w->y,raise=0;uint16_t neww=w->width,newh=w->height,newborder=w->border;for(bit=0;bit<7&&off+4<=n;bit++)if(mask&(1U<<bit)){uint32_t v=rd32(q+off,c->order);if(bit==0)newx=(int16_t)v;else if(bit==1)newy=(int16_t)v;else if(bit==2)neww=(uint16_t)v;else if(bit==3)newh=(uint16_t)v;else if(bit==4)newborder=(uint16_t)v;else if(bit==6&&v==Above)raise=1;off+=4;}if((neww!=w->width||newh!=w->height)&&window_pixels_resize(w,neww,newh)){error_reply(c,11,w->id,op);return 0;}w->x=(int16_t)newx;w->y=(int16_t)newy;w->width=neww;w->height=newh;w->border=newborder;if(w->x!=oldx||w->y!=oldy){unsigned j;for(j=1;j<s->window_count;j++)if(s->windows[j].parent==w->id){s->windows[j].x+=(int16_t)(w->x-oldx);s->windows[j].y+=(int16_t)(w->y-oldy);}}mark_dirty(s,oldx,oldy,oldw,oldh);mark_dirty(s,w->x,w->y,w->width,w->height);if(neww!=(uint16_t)oldw||newh!=(uint16_t)oldh)expose(s,w);if(raise)raise_window(s,top_level_window(s,w));return 0;}break;
	case 14: /* GetGeometry */
	...
	case 15: /* QueryTree: return all direct children in stacking order. */
		if((w=find_window(s,rd32(q+4,c->order)))!=NULL){uint8_t r[32+MAX_WINDOWS*4];unsigned i,count=0;memset(r,0,sizeof(r));wr32(r+8,ROOT_XID,c->order);wr32(r+12,w->parent,c->order);for(i=1;i<s->window_count;i++)if(s->windows[i].id&&s->windows[i].parent==w->id){wr32(r+32+count*4,s->windows[i].id,c->order);count++;}wr16(r+16,(uint16_t)count,c->order);simple_reply(c,r,32+count*4);return 0;}break;
	...
	case 65: /* PolyLine */
		if(n>=16&&(w=find_window(s,rd32(q+4,c->order)))!=NULL){struct graphics_context*g=find_gc(s,rd32(q+8,c->order));size_t off;int x=0,y=0;uint32_t color=g?g->foreground:0xffffff;for(off=12;off+4<=n;off+=4){int nx=(int16_t)rd16(q+off,c->order),ny=(int16_t)rd16(q+off+2,c->order);if(off!=12){int x0=x,y0=y,dx=abs(nx-x0),sx=x0<nx?1:-1,dy=-abs(ny-y0),sy=y0<ny?1:-1,err=dx+dy;for(;;){if(x0>=0&&y0>=0&&x0<w->width&&y0<w->height)w->pixels[(size_t)y0*w->width+x0]=color;if(x0==nx&&y0==ny)break;{int e2=2*err;if(e2>=dy){err+=dy;x0+=sx;}if(e2<=dx){err+=dx;y0+=sy;}}}mark_dirty(s,w->x+(x<nx?x:nx),w->y+(y<ny?y:ny),abs(nx-x)+1,abs(ny-y)+1);}x=nx;y=ny;}}return 0;
...
static void mouse(struct server *s)
{
	...
	while ((n = read(s->mouse, ev, sizeof(ev))) > 0) {
		...
		for (i = 0; i < (size_t)n / sizeof(ev[0]); i++) {
			...
			struct client *c;
			unsigned b;
			s->pointer_x += ev[i].dx; s->pointer_y += ev[i].dy;
			if (s->pointer_x < 0) s->pointer_x = 0;
			if (s->pointer_y < 0) s->pointer_y = 0;
			if (s->pointer_x >= (int)s->mode.width) s->pointer_x = (int)s->mode.width - 1;
			if (s->pointer_y >= (int)s->mode.height) s->pointer_y = (int)s->mode.height - 1;
			w = s->pointer_grab_owner >= 0 ? find_window(s, s->pointer_grab_window) :
			...
			for (b = 0; b < 3; b++) if (changed & (1U << b)) {
				...
				if (c) send_event(c, (ev[i].buttons & (1U << b)) ? 4 : 5,
				    window_id, b + 1, (uint32_t)(ev[i].timestamp_ns / 1000000),
				    s->pointer_x, s->pointer_y, (uint16_t)s->buttons);
				if (ev[i].buttons & (1U << b)) {
					struct window *top = top_level_window(s, w);
					/* A desktop panel must be clickable without stealing the
					 * application's focus used for toggle-minimize semantics. */
				...
				if ((ev[i].buttons & (1U << b)) && s->pointer_grab_owner < 0 && c) {
	...
	if (redraw) {
		mark_dirty(s, oldx - CURSOR_WIDTH, oldy - CURSOR_HEIGHT,
		    CURSOR_WIDTH * 2, CURSOR_HEIGHT * 2); present(s);
		mark_dirty(s, s->pointer_x - CURSOR_WIDTH,
		    s->pointer_y - CURSOR_HEIGHT, CURSOR_WIDTH * 2,
		    CURSOR_HEIGHT * 2); present(s);
	}
	if (n < 0 && errno != EAGAIN && errno != EINTR) stopped = 1;
}
	...
	struct graphics_mode *chosen)
{
	...
	for (pass = 0; pass < 4 && best < 0; pass++) {
		...
		for (i = 0; i < list.count; i++) {
			...
			int depth_ok = preferred_depth == 0 ||
			...
			if ((pass < 2 && !size_ok) || ((pass == 0 || pass == 2) && !depth_ok))
				continue;
			if (best < 0 || (size_ok ? area > best_area : area < best_area)) {
				best = (int)i; best_area = area;
			}
		}
	}
	if (best < 0) { errno = ENODEV; return -1; }
	...
	unsigned preferred_height, unsigned preferred_depth)
{
	...
	s->console=open("/dev/console",O_RDONLY|O_NONBLOCK|O_CLOEXEC);if(s->console<0)return -1;if(ioctl(s->console,ZEDBSD_CONSOLE_GET_INPUT_MODE,&s->old_console_mode)||ioctl(s->console,ZEDBSD_CONSOLE_SET_INPUT_MODE,&m))return -1;
	s->mouse=open("/dev/mouse",O_RDONLY|O_NONBLOCK|O_CLOEXEC);
	s->graphics=open("/dev/graphics",O_RDWR|O_CLOEXEC);if(s->graphics<0)return -1;if(ioctl(s->graphics,ZEDBSD_GRAPHICS_GET_CAPS,&caps))return -1;if(!(caps.capabilities&ZEDBSD_GRAPHICS_CAP_FLUSH)||!(caps.capabilities&ZEDBSD_GRAPHICS_CAP_GLYPH)||!(caps.capabilities&ZEDBSD_GRAPHICS_CAP_BLIT_RGB24)){errno=ENOTSUP;return -1;}if(choose_mode(s->graphics,preferred_width,preferred_height,preferred_depth,&s->mode))return -1;if(ioctl(s->graphics,ZEDBSD_GRAPHICS_ENTER,&s->mode))return -1;
	(void)mkdir("/tmp/.X11-unix",0777);(void)unlink("/tmp/.X11-unix/X0");s->listener=socket(AF_UNIX,SOCK_STREAM|SOCK_CLOEXEC,0);if(s->listener<0)return -1;memset(&a,0,sizeof(a));a.sun_family=AF_UNIX;strcpy(a.sun_path,"/tmp/.X11-unix/X0");if(bind(s->listener,(struct sockaddr*)&a,sizeof(a))||listen(s->listener,8))return -1;(void)fcntl(s->listener,F_SETFL,fcntl(s->listener,F_GETFL)|O_NONBLOCK);
	s->windows[0]=(struct window){.id=ROOT_XID,.owner=MAX_CLIENTS,.background=0x203040,.width=(uint16_t)s->mode.width,.height=(uint16_t)s->mode.height,.mapped=1};s->window_count=1;s->focus=ROOT_XID;s->pointer_x=(int)s->mode.width/2;s->pointer_y=(int)s->mode.height/2;
	s->windows[0].pixels=window_pixels_alloc(s->windows[0].width,s->windows[0].height,s->windows[0].background);s->screen=malloc((size_t)s->mode.width*s->mode.height*sizeof(*s->screen));s->transfer=malloc((size_t)s->mode.width*s->mode.height*3U);if(!s->windows[0].pixels||!s->screen||!s->transfer){errno=ENOMEM;return -1;}repaint(s);present(s);return 0;
}
static void cleanup(struct server *s)
{
	unsigned i;for(i=0;i<MAX_CLIENTS;i++)close_client(s,i);for(i=0;i<s->window_count;i++)free(s->windows[i].pixels);for(i=0;i<s->pixmap_count;i++)free(s->pixmaps[i].pixels);free(s->screen);free(s->transfer);if(s->listener>=0)close(s->listener);(void)unlink("/tmp/.X11-unix/X0");if(s->mouse>=0)close(s->mouse);if(s->console>=0){(void)ioctl(s->console,ZEDBSD_CONSOLE_SET_INPUT_MODE,&s->old_console_mode);close(s->console);}if(s->graphics>=0)close(s->graphics);
}
int main(int argc,char **argv)
{
	struct server s;struct pollfd p[3+MAX_CLIENTS];unsigned i,count;int arg=1;
	unsigned preferred_width=0,preferred_height=0,preferred_depth=24;
	extern char **environ;
	while(arg<argc){
		if(strcmp(argv[arg],":0")==0){arg++;continue;}
	...
	while(!stopped){int ready;count=0;p[count++]=(struct pollfd){s.listener,POLLIN,0};p[count++]=(struct pollfd){s.console,POLLIN,0};if(s.mouse>=0)p[count++]=(struct pollfd){s.mouse,POLLIN,0};for(i=0;i<MAX_CLIENTS;i++)if(s.clients[i].fd>=0)p[count++]=(struct pollfd){s.clients[i].fd,POLLIN,0};ready=poll(p,count,20);if(ready<0){if(errno==EINTR)continue;break;}count=0;if(p[count++].revents&POLLIN){int fd=accept(s.listener,NULL,NULL);if(fd>=0){int descriptor_flags=fcntl(fd,F_GETFD);int status_flags=fcntl(fd,F_GETFL);if(descriptor_flags<0||status_flags<0||fcntl(fd,F_SETFD,descriptor_flags|FD_CLOEXEC)<0||fcntl(fd,F_SETFL,status_flags|O_NONBLOCK)<0){close(fd);}else{for(i=0;i<MAX_CLIENTS&&s.clients[i].fd>=0;i++);if(i==MAX_CLIENTS)close(fd);else{s.clients[i].fd=fd;s.clients[i].base=(i+1U)<<22;}}}}if(p[count++].revents&POLLIN)keyboard(&s);if(s.mouse>=0&&p[count++].revents&POLLIN)mouse(&s);for(i=0;i<MAX_CLIENTS;i++)if(s.clients[i].fd>=0)read_client(&s,i);present(&s);}
	cleanup(&s);return 0;
}
