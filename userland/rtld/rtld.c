...
register_tls_module(struct rtld_object *object)
{
	...
	for (i = 0; i < object->phnum; i++)
		if (object->phdr[i].p_type == PT_TLS) {
			...
			object->tls_module_id = ++tls_module_count;
...
parse_dynamic(struct rtld_object *object)
{
	...
	if (i == object->dynamic_count || object->strsz == 0 ||
	    strtab_value == 0 || symtab_value == 0 || hash_value == 0 ||
	...
	object->symtab = (Elf_Sym *)object_pointer(object, (Elf_Addr)symtab_value,
	    sizeof(Elf_Sym), PF_R);
	object->hash = (uint32_t *)object_pointer(object, (Elf_Addr)hash_value,
	    2U * sizeof(uint32_t), PF_R);
	...
	(void)object_pointer(object, (Elf_Addr)hash_value,
	    (size_t)(2U + object->hash[0] + object->hash[1]) * sizeof(uint32_t),
	    PF_R);
	if (rel_value != 0) {
	...
	for (i = 0; i < object->needed_count; i++) {
		...
		if (offset >= object->strsz ||
		    rtld_strlen(object->strtab + offset) >= object->strsz - offset)
	...
	for (i = 0; i < header->e_phnum; i++) {
		...
		if (phdr[i].p_memsz == 0) {
			if (phdr[i].p_filesz != 0)
				rtld_fatal("invalid empty load segment");
			continue;
		}
		loads++;
		if (phdr[i].p_filesz > phdr[i].p_memsz ||
			...
			rtld_fatal("invalid load segment");
		(void)segment_prot(phdr[i].p_flags);
		...
		for (j = 0; j < i; j++) {
			...
			if (start < other_end && other_start < end)
				rtld_fatal("overlapping load segments");
		}
	}
...
find_identity(const struct stat *status)
{
	...
	for (i = 0; i < object_count; i++)
		if (objects[i].has_identity && objects[i].device == status->st_dev &&
...
load_dependencies(struct rtld_object *object)
{
	...
	for (i = 0; i < object->needed_count; i++)
		object->needed[i] = load_object(object->strtab +
		    object->needed_offset[i]);
}

static struct rtld_object *
load_object(const char *name)
{
	char path[RTLD_PATH_MAX];
	struct stat status;
	Elf_Ehdr header;
	...
	rtld_memcpy(path, "/lib/", 5);
	rtld_memcpy(path + 5, name, length + 1U);
	fd = syscall6(ZEDBSD_SYS_open, (uintptr_t)path, O_RDONLY, 0, 0, 0, 0);
	if (raw_error(fd))
	...
	validate_file_programs(&header, phdr, status.st_size);
...
static Elf_Sym *
lookup_in_object(struct rtld_object *object, const char *name)
{
	uint32_t buckets, index, *bucket, *chain;
	unsigned traversed = 0;
	if (object == NULL || object->hash == NULL)
		return NULL;
...
static uintptr_t
lookup_symbol(const char *name, int weak)
{
	Elf_Sym *symbol;
	unsigned i;
	if (reserved_loader_symbol(name)) {
		symbol = lookup_in_object(interpreter_object, name);
		if (symbol != NULL)
	...
	symbol = lookup_in_object(main_object, name);
	if (symbol != NULL)
		return symbol_value(main_object, symbol);
	for (i = 0; i < object_count; i++) {
		struct rtld_object *object = &objects[i];
		if (object == main_object || object == interpreter_object)
			continue;
		symbol = lookup_in_object(object, name);
		if (symbol != NULL)
...
resolve_relocation_symbol(struct rtld_object *object, uint32_t index)
{
	...
	return lookup_symbol(name, binding == STB_WEAK);
}
	...
	struct rtld_object **owner)
{
	...
	for (i = 0; i < object_count; i++) {
		...
		if (candidate == main_object || candidate == interpreter_object)
			continue;
		symbol = lookup_in_object(candidate, name);
		if (symbol != NULL) {
			*owner = candidate;
			return symbol;
		}
	...
	uint32_t symbol_index, uintptr_t addend, int is_rela)
{
	...
	if (!object_contains(object, address, sizeof(uintptr_t), PF_W))
...
static uintptr_t
lookup_global_optional(const char *name, int *found)
{
	Elf_Sym *symbol;
	unsigned i;
...
__zedbsd_rtld_dlopen(const char *path, int flags)
{
	...
	for (i = 0; i < object_count; i++)
		if (rtld_strcmp(objects[i].path, path) == 0 ||
		    (objects[i].path[0] == '/' &&
...
__zedbsd_rtld_dlsym(void *value, const char *name)
{
	...
	if (handle == NULL) {
		...
		result = lookup_global_optional(name, &found);
	} else {
		result = lookup_handle_graph(handle->object, name, &visited, &found);
	}
...
rtld_main(uintptr_t *initial_stack)
{
	...
	setup_premapped_object(main_object, 0, (Elf_Phdr *)at_phdr,
	    (unsigned)at_phnum, ET_EXEC);
