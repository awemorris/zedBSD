...
copy_head(int input, unsigned long long limit, int bytes)
{
	...
	while (count < limit) {
		...
		if (bytes) {
			if (command_write_all(STDOUT_FILENO, buffer, (size_t)got) != 0)
				return -1;
			...
			while (used < (size_t)got && count < limit) {
				...
				if (end < (size_t)got) { end++; count++; }
				if (command_write_all(STDOUT_FILENO, buffer + used,
				    end - used) != 0) return -1;
				used = end;
			}
			/* Bytes after the requested newline were read speculatively.  This
			 * is harmless for named files but violates pipeline semantics, so
			 * use one-byte reads for line mode below instead. */
			if (used < (size_t)got) { errno = ESPIPE; return -1; }
		}
	}
	return 0;
}
...
copy_head_lines(int input, unsigned long long limit)
{
	...
	while (lines < limit) {
		...
		if (command_write_all(STDOUT_FILENO, &byte, 1) != 0) return -1;
		if (byte == '\n') lines++;
	}
	return 0;
}
...
main(int argc, char **argv)
{
	...
	if (index < argc && (!strcmp(argv[index], "-n") ||
		...
		if (++index == argc || command_parse_ull(argv[index++], &limit) != 0)
			goto usage;
	}
	...
	for (; index < argc; index++) {
		int descriptor = !strcmp(argv[index], "-") ? STDIN_FILENO :
		    open(argv[index], O_RDONLY);
		if (descriptor < 0) { command_error("head", argv[index]); failed = 1; continue; }
		if (files > 1)
			printf("%s==> %s <==\n", index == argc - files ? "" : "\n",
			    argv[index]);
		if ((bytes ? copy_head(descriptor, limit, 1) :
		    copy_head_lines(descriptor, limit)) != 0) {
			command_error("head", argv[index]); failed = 1;
		}
