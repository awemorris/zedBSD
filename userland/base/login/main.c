...
main(int argc, char **argv)
{
	...
	if (argc > 1) {
		...
		printf("login: ");
		fflush(stdout);
		if (read_line(name, sizeof(name), 1) != 0)
			return 1;
	}
	if (getpwnam_r(name, &account, pwbuf, sizeof(pwbuf), &found) != 0 ||
	    found == NULL ||
	    getspnam_r(name, &shadow, spbuf, sizeof(spbuf), &shadow_found) !=
		0 ||
	    shadow_found == NULL) {
		printf("Password: ");
		fflush(stdout);
