...
write_resolver(const char *interface, const struct dhcp_lease *lease)
{
	...
	if (used >= sizeof(buffer))
		return -1;
	descriptor =
	    open("/etc/resolv.conf", O_WRONLY | O_CREAT | O_TRUNC, 0644);
	if (descriptor < 0)
		return -1;
	{
		size_t offset = 0;
...
usage(void)
{
	puts("usage: dhcpcd [-v] [-t seconds] [interface]");
	return 2;
}
...
main(int argc, char **argv)
{
	...
	if (control < 0) {
		printf("dhcpcd: socket: %s\n", strerror(errno));
		return 1;
	}
	if (interface[0] == '\0' && select_interface(control, interface) != 0) {
		printf("dhcpcd: select interface: %s\n", strerror(errno));
		close(control);
		return 1;
	}
	...
	while (netutil_monotonic_us() < deadline && !got_offer) {
		if (dhcp_build(packet, sizeof(packet), &packet_length,
			...
			goto socket_fail;
		if (verbose)
			printf("dhcpcd: %s: broadcasting discover\n",
			       interface);
		for (;;) {
	...
	}
	{
		...
		printf("dhcpcd: %s: offered %s", interface, text);
	...
	}
	{
		...
		inet_ntop(AF_INET, &value, text, sizeof(text));
		printf("dhcpcd: %s: address %s/%u\n", interface, text, prefix);
	}
	if (lease.router_count != 0) {
		struct in_addr value = {lease.routers[0]};
		inet_ntop(AF_INET, &value, text, sizeof(text));
		printf("dhcpcd: %s: default route %s\n", interface, text);
	}
	if (lease.dns_count != 0) {
		...
		for (i = 0; i < lease.dns_count; i++) {
			...
			inet_ntop(AF_INET, &value, text, sizeof(text));
			printf("dhcpcd: %s: dns %s\n", interface, text);
		}
		if (write_resolver(interface, &lease) != 0)
			printf("dhcpcd: warning: cannot write "
			       "/etc/resolv.conf: %s\n",
			       strerror(errno));
	}
