...
int
sh_alias_set(const char *name, const char *value)
{
	struct shell_alias *item;
	char *copy;
	if (!valid_name(name)) {
...
int
sh_alias_unset(const char *name)
{
...
static int
token_unquoted(const struct sh_token *token)
{
	size_t index;
...
static int
assignment_word(const char *text)
{
...
static int
splice_alias(struct sh_token_list *list, size_t position, const char *value,
...
int
sh_alias_expand(struct sh_token_list *list, const char **error_text)
{
	size_t position = 0;
