...
static int
append_parameter(struct expand_buffer *buffer, const char *name, size_t length,
		 int quoted, const struct sh_expand_context *context)
{
	char *copy;
	const char *value;
...
expand_raw(const struct sh_token *token,
	   ...
	   struct expand_buffer *buffer, const char **error_text)
{
	size_t index = 0;
	memset(buffer, 0, sizeof(*buffer));
	...
	while (index < token->length) {
		...
		if (value == '(') {
			if (index + 2U < token->length &&
				...
				if (expression == NULL)
					goto no_memory;
				memcpy(expression, token->text + start,
				       scan - start);
		...
		if (value == '{') {
			...
			parameter = lookup_parameter(
			    context, token->text + start,
			    (size_t)(strchr(token->text + start,
					    operation == '\0' ? '}'
					    : colon	      ? ':'
							      : operation) -
				     (token->text + start)),
...
field_append(struct sh_field_list *list, const char *data,
	     const unsigned char *quoted_data, size_t length,
	     int quoted_default)
{
	char **larger;
...
sh_expand_fields(const struct sh_token *token,
		 const struct sh_expand_context *context,
		 struct sh_field_list *list, const char **error_text)
{
	struct expand_buffer buffer;
	const char *ifs = context->lookup == NULL
			      ? getenv("IFS")
