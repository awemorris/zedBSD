...
static char
state_letter(unsigned state)
{
	static const char map[] = "?RTXZX";
	return state < sizeof(map) - 1U ? map[state] : '?';
}
static const char *
user_name(uid_t uid, char b[16])
{
	if (uid == 0)
		return "root";
...
static int
snapshot(int fd, struct process_info *p, size_t *n)
{
	int32_t cursor = -1;
	size_t used = 0;
...
static void
human(uint64_t bytes, char out[16])
{
...
static void
draw(int fd, int batch)
{
	struct vm_statistics vm;
	struct process_info p[TOP_MAX_PROCESSES];
	struct timespec now;
	size_t count = 0, i;
	unsigned running = 0, sleeping = 0, stopped = 0, zombie = 0;
	...
	for (i = 0; i < count; i++) {
		char ub[16], virt[16];
		const char *u = user_name(p[i].uid, ub);
...
{
	int fd, batch = 0, iterations = -1, delay = 1000, i;
