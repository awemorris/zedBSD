...
static int
join_path(const char *a, const char *b, char *out, size_t cap)
{
...
static mode_t
copied_bits(mode_t mode, char source, int who)
{
...
static int
symbolic_mode(const char *spec, mode_t original, mode_t umask_value,
	      int directory, mode_t *result)
{
	const char *p = spec;
	mode_t mode = original;
	while (*p) {
		int who = 0, explicit_who = 0;
		char operation;
		mode_t bits = 0, affected;
...
static int
apply_path(const char *path, const char *spec, int numeric, mode_t numeric_mode,
	   mode_t mask, int recursive, int depth)
{
	struct stat status;
	mode_t mode;
	int ok = 1;
...
{
	int recursive = 0, index = 1, failed = 0, numeric;
	unsigned parsed = 0;
	mode_t mask;
	const char *spec;
