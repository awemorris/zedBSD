...
main(int argc, char **argv)
{
	...
	if (argc == 3 &&
		...
		if (strchr(argv[3], '/') != NULL) {
			if (netutil_parse_cidr(argv[3], &address, &mask,
					       &prefix) != 0 ||
			    argc != 4) {
				close(descriptor);
				return usage();
			...
			if (netutil_parse_ipv4(argv[3], &address) != 0 ||
			    argc != 6 || strcmp(argv[4], "netmask") != 0 ||
			    netutil_parse_ipv4(argv[5], &mask) != 0 ||
		...
		if (set_sockaddr(descriptor, argv[1], SIOCSIFNETMASK, mask) !=
			0 ||
		    set_sockaddr(descriptor, argv[1], SIOCSIFBRDADDR,
				 broadcast) != 0 ||
		    set_sockaddr(descriptor, argv[1], SIOCSIFADDR, address) !=
			0)
			status = 1;
	} else if (argc == 4 && strcmp(argv[2], "broadcast") == 0) {
		struct in_addr broadcast;
		if (netutil_parse_ipv4(argv[3], &broadcast) != 0) {
			close(descriptor);
			return usage();
		}
