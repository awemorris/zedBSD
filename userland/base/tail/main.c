...
#include <unistd.h>
static int
tail_fd(int fd, unsigned long long wanted)
{
	unsigned char *data = NULL, buffer[4096];
	size_t used = 0, capacity = 0, start;
	for (;;) {
		...
		if (used + (size_t)n > capacity) {
			...
			while (next < used + (size_t)n)
				next *= 2;
			unsigned char *larger = realloc(data, next);
			if (!larger) {
				free(data);
				return -1;
			}
