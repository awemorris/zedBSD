...
/*
 * Implements the zedBSD C library crypt support.
 */
...
static void sha_init(struct sha512_ctx *c);
static void sha_add(struct sha512_ctx *c, const void *data, size_t n);
static void sha_block(struct sha512_ctx *c, const uint8_t *p);
static uint64_t load64(const uint8_t *p);
static uint64_t ror(uint64_t x, unsigned n);
static void sha_final(struct sha512_ctx *c, uint8_t out[64]);
static void store64(uint8_t *p, uint64_t v);
static void add_repeat(struct sha512_ctx *c, const uint8_t *p, size_t plen, size_t n);
static void b64(char **out, unsigned b2, unsigned b1, unsigned b0, unsigned count);

/*
 * Implements the crypt operation.
 */
char *
crypt(
	const char *password,
	const char *setting)
{
	...
	/* Handles the password condition. */
	...
	/* Handles the slen condition. */
	if (slen > 16)
		slen = 16;
	plen = strlen(password);

	/* Handles the plen condition. */
	if (plen > 4096) {
		errno = E2BIG;
...
/* Supports the sha init operation. */
static void
sha_init(
	struct sha512_ctx *c)
{
...
/* Supports the sha add operation. */
static void
sha_add(
	struct sha512_ctx *c,
	const void *data,
...
/* Supports the sha block operation. */
static void
sha_block(
	struct sha512_ctx *c,
	const uint8_t *p)
...
/* Supports the sha final operation. */
static void
sha_final(
	struct sha512_ctx *c,
	uint8_t out[64])
...
/* Supports the store64 operation. */
static void
store64(
	uint8_t *p,
	uint64_t v)
...
/* Supports the add repeat operation. */
static void
add_repeat(
	struct sha512_ctx *c,
	const uint8_t *p,
