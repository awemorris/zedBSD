...
{
	return (int)socket_call(ZEDBSD_SYS_socket, domain, type, protocol, 0, 0,
				0);
}
...
sockatmark(int descriptor)
{
	...
	return getsockopt(descriptor, SOL_SOCKET, SO_ATMARK, &at_mark,
			  &length) == 0
		   ? at_mark
		   : -1;
}
...
int
socketpair(int domain, int type, int protocol, int descriptors[2])
{
...
{
	return (int)socket_call(ZEDBSD_SYS_bind, descriptor, (uintptr_t)address,
				length, 0, 0, 0);
}
...
{
	return (int)socket_call(ZEDBSD_SYS_listen, descriptor, backlog, 0, 0, 0,
				0);
}
...
{
	return (int)socket_call(ZEDBSD_SYS_shutdown, descriptor, how, 0, 0, 0,
				0);
}
...
int
setsockopt(int descriptor, int level, int option, const void *value,
	   socklen_t length)
{
...
int
getsockopt(int descriptor, int level, int option, void *value,
	   socklen_t *length)
{
...
int
inet_pton(int family, const char *text, void *address)
{
	if (family != AF_INET) {
		errno = EAFNOSUPPORT;
		return -1;
...
static char *
append_decimal(char *output, unsigned value)
{
	if (value >= 100U)
