...
static int
encode_name(uint8_t *message, size_t capacity, size_t *offset, const char *name)
{
...
static int
decode_name(const uint8_t *message, size_t length, size_t *offset, char *output,
	    size_t capacity)
{
...
resolver_dns_parse(const uint8_t *message, size_t length, uint16_t id,
		   const char *question, uint16_t qtype,
		   struct resolver_result *result, int *truncated)
{
