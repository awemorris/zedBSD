...
timer_signal_handler(int signo, siginfo_t *information, void *context)
{
	...
	/* usync wake is a raw syscall and therefore does not run cancellation or
	 * errno machinery in signal context.  The generation comparison in the
	 * worker makes this safe even when the wake runs just before it sleeps. */
	(void)__atomic_add_fetch(&timer_wake_generation, 1, __ATOMIC_RELEASE);
	(void)__syscall6(ZEDBSD_SYS_usync,
	    (uintptr_t)&timer_wake_generation, ZEDBSD_USYNC_WAKE, 0, 0,
	    UINT32_MAX, ZEDBSD_USYNC_PRIVATE);
...
timer_dispatch_pending(unsigned slot, uint32_t id, uint32_t count)
{
	while (count-- != 0) {
		...
		/* Notification happens asynchronously, so POSIX provides no caller to
		 * which resource-exhaustion can be reported.  The kernel overrun value
		 * remains authoritative; a callback whose thread cannot be allocated is
		 * dropped instead of spinning the single delivery worker forever. */
		if (error != 0)
			free(callback);
	}
}
...
timer_dispatch_slot(unsigned slot)
{
	 ...
	 * lock-free load followed by exchange could otherwise sample an old ID and
	 * consume a newly-created timer's first notification from the same slot. */
	(void)pthread_mutex_lock(&timer_lock);
	...
	/* Keep the ID sampled with the pending count.  If delete/recreate reuses
	 * this slot while the worker waits for timer_lock, the generation check in
	 * timer_dispatch_pending() rejects the old work instead of invoking the
	 * new timer's callback. */
...
{
	sigset_t signal_set = (sigset_t)1ULL <<
	    (LIBC_TIMER_WAKE_SIGNAL - 1U);

	(void)argument;
	/* At least the worker must always be able to receive the reserved signal,
	 * even when the thread that created the first timer had it blocked.  Build
	 * the private bit directly because the public sigset helpers intentionally
	 * reject every signal above SIGRTMAX. */
	(void)timer_syscall(ZEDBSD_SYS_sigprocmask, SIG_UNBLOCK,
	    (uintptr_t)&signal_set, 0, 0);
	for (;;) {
		uint32_t observed = __atomic_load_n(&timer_wake_generation,
		    __ATOMIC_ACQUIRE);
		unsigned slot;
		...
		if (__atomic_load_n(&timer_wake_generation,
		    __ATOMIC_ACQUIRE) == observed)
			(void)__syscall6(ZEDBSD_SYS_usync,
...
__timer_sigev_thread_fork_child(void)
{
	 ...
	 * The fixed slot array intentionally needs no allocation or freeing here. */
...
timer_service_start_locked(void)
{
	...
	if (timer_syscall(ZEDBSD_SYS_sigaction, LIBC_TIMER_WAKE_SIGNAL,
	    (uintptr_t)&action, (uintptr_t)&timer_previous_action, 0) < 0) {
		error = errno;
		return error;
	}
...
timer_create(clockid_t clock, const struct sigevent *event, timer_t *result)
{
	...
	if (event == NULL || event->sigev_notify != SIGEV_THREAD)
		return (int)timer_syscall(ZEDBSD_SYS_timer_create, clock,
		    (uintptr_t)event, (uintptr_t)result, 0);
	...
	if (timer_syscall(ZEDBSD_SYS_timer_create, clock,
	    (uintptr_t)&kernel_event, (uintptr_t)&kernel_id, 0) < 0) {
		error = errno;
		goto fail_locked;
	}
...
timer_delete(timer_t timer)
{
	...
	__atomic_store_n(&record->state, TIMER_SLOT_DELETING,
	    __ATOMIC_RELEASE);
	if (timer_syscall(ZEDBSD_SYS_timer_delete,
	    (uintptr_t)record->kernel_id, 0, 0, 0) < 0) {
		error = errno;
	...
	/* A handler that observed the old generation must finish before this slot
	 * becomes reusable.  It never blocks, allocates, or takes timer_lock. */
	...
	struct itimerspec *old_value)
{
	...
	if (!timer_is_libc_id(timer))
		return (int)timer_syscall(ZEDBSD_SYS_timer_settime,
		    (uintptr_t)timer, flags, (uintptr_t)value,
		    (uintptr_t)old_value);
	(void)pthread_mutex_lock(&timer_lock);
	record = timer_lookup_locked(timer);
	if (record == NULL) {
	...
	result = timer_syscall(ZEDBSD_SYS_timer_settime,
	    (uintptr_t)record->kernel_id, flags, (uintptr_t)value,
	    (uintptr_t)old_value);
	saved_errno = errno;
	(void)pthread_mutex_unlock(&timer_lock);
	errno = saved_errno;
...
timer_gettime(timer_t timer, struct itimerspec *value)
{
	...
	if (!timer_is_libc_id(timer))
		return (int)timer_syscall(ZEDBSD_SYS_timer_gettime,
		    (uintptr_t)timer, (uintptr_t)value, 0, 0);
	(void)pthread_mutex_lock(&timer_lock);
	record = timer_lookup_locked(timer);
	if (record == NULL) {
		(void)pthread_mutex_unlock(&timer_lock);
	...
	result = timer_syscall(ZEDBSD_SYS_timer_gettime,
