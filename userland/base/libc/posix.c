...
static int
environment_name(const char *entry, const char *name)
{
	size_t length;
...
open(const char *path, int flags, ...)
{
	...
	return (int)call(ZEDBSD_SYS_open, (uintptr_t)path, flags, mode, 0, 0,
			 0);
}
int
openat(int dirfd, const char *path, int flags, ...)
{
	...
	return (int)call(ZEDBSD_SYS_openat, dirfd, (uintptr_t)path, flags, mode,
			 0, 0);
}
int
close(int fd)
...
dup(int fd)
{
	return (int)call(ZEDBSD_SYS_dup, fd, 0, 0, 0, 0, 0);
}
int
dup2(int oldfd, int newfd)
{
	return (int)call(ZEDBSD_SYS_dup2, oldfd, newfd, 0, 0, 0, 0);
}
int
dup3(int oldfd, int newfd, int flags)
{
	return (int)call(ZEDBSD_SYS_dup3, oldfd, newfd, flags, 0, 0, 0);
}
int
fcntl(int fd, int command, ...)
{
	...
	{
		int result =
		    (int)call(ZEDBSD_SYS_fcntl, fd, command, argument, 0, 0, 0);
		if (result == 0 &&
...
int
pipe2(int result[2], int flags)
{
	return (int)call(ZEDBSD_SYS_pipe2, (uintptr_t)result, flags, 0, 0, 0,
			 0);
}
int
pipe(int result[2])
{
	return pipe2(result, 0);
}
ssize_t
read(int fd, void *p, size_t n)
{
	ssize_t r;
	cancel_point();
...
ssize_t
write(int fd, const void *p, size_t n)
{
	ssize_t r;
	cancel_point();
...
ssize_t
pread(int fd, void *p, size_t n, off_t offset)
{
	return (ssize_t)call(ZEDBSD_SYS_pread, fd, (uintptr_t)p, n, offset, 0,
			     0);
}
ssize_t
pwrite(int fd, const void *p, size_t n, off_t offset)
{
	return (ssize_t)call(ZEDBSD_SYS_pwrite, fd, (uintptr_t)p, n, offset, 0,
			     0);
}
ssize_t
readv(int fd, const struct iovec *iov, int count)
{
	return (ssize_t)call(ZEDBSD_SYS_readv, fd, (uintptr_t)iov, count, 0, 0,
			     0);
}
ssize_t
writev(int fd, const struct iovec *iov, int count)
{
	return (ssize_t)call(ZEDBSD_SYS_writev, fd, (uintptr_t)iov, count, 0, 0,
			     0);
}
static ssize_t
positional_vector_io(int fd, const struct iovec *iov, int count, off_t offset,
		     int writing)
{
	ssize_t total = 0;
	int index;
...
{
	cancel_point();
	ssize_t result = positional_vector_io(fd, iov, count, offset, 0);
	cancel_point();
	return result;
}
ssize_t
...
{
	cancel_point();
	ssize_t result = positional_vector_io(fd, iov, count, offset, 1);
	cancel_point();
	return result;
}
int
fsync(int fd)
{
	return (int)call(ZEDBSD_SYS_fsync, fd, 0, 0, 0, 0, 0);
}
void
sync(void)
{
...
fdatasync(int fd)
{
	return (int)call(ZEDBSD_SYS_fdatasync, fd, 0, 0, 0, 0, 0);
}
off_t
lseek(int fd, off_t off, int whence)
{
	return (off_t)call(ZEDBSD_SYS_lseek, fd, off, whence, 0, 0, 0);
}
int
fstat(int fd, struct stat *st)
{
	return (int)call(ZEDBSD_SYS_fstat, fd, (uintptr_t)st, 0, 0, 0, 0);
}
int
chdir(const char *p)
{
	return (int)call(ZEDBSD_SYS_chdir, (uintptr_t)p, 0, 0, 0, 0, 0);
}
int
fchdir(int fd)
{
	return (int)call(ZEDBSD_SYS_fchdir, fd, 0, 0, 0, 0, 0);
}
char *
getcwd(char *p, size_t n)
{
	return (char *)call(ZEDBSD_SYS_getcwd, (uintptr_t)p, n, 0, 0, 0, 0);
}
static int
ioctl_has_argument(unsigned long request)
{
	     ...
	     size_t newlen)
{
	...
	if (sysctl(query, 2, oid, &oidlen, name, strlen(name) + 1U) != 0)
		return -1;
	return sysctl(oid, (unsigned)(oidlen / sizeof(oid[0])), oldp, oldlenp,
		      newp, newlen);
}
void *
mmap(void *address, size_t length, int prot, int flags, int fd, off_t offset)
{
...
{
	return (int)call(ZEDBSD_SYS_munmap, (uintptr_t)p, n, 0, 0, 0, 0);
}
int
mprotect(void *p, size_t n, int prot)
{
	return (int)call(ZEDBSD_SYS_mprotect, (uintptr_t)p, n, prot, 0, 0, 0);
}
int
msync(void *p, size_t n, int flags)
{
	return (int)call(ZEDBSD_SYS_msync, (uintptr_t)p, n, flags, 0, 0, 0);
}
...
brk(void *address)
{
	...
	intptr_t value =
	    call(ZEDBSD_SYS_brk, (uintptr_t)address, 0, 0, 0, 0, 0);
	if (value == -1)
		return -1;
...
size_t
confstr(int name, char *buffer, size_t size)
{
...
{
	return (int)call(ZEDBSD_SYS_mkdir, (uintptr_t)path, mode, 0, 0, 0, 0);
}
int
mkdirat(int dirfd, const char *path, mode_t mode)
{
	return (int)call(ZEDBSD_SYS_mkdirat, dirfd, (uintptr_t)path, mode, 0, 0,
			 0);
}
int
mkfifoat(int dirfd, const char *path, mode_t mode)
{
	return (int)call(ZEDBSD_SYS_mknodat, dirfd, (uintptr_t)path,
...
int
mknodat(int dirfd, const char *path, mode_t mode, dev_t device)
{
	return (int)call(ZEDBSD_SYS_mknodat, dirfd, (uintptr_t)path, mode,
			 device, 0, 0);
}
int
...
{
	return (int)call(ZEDBSD_SYS_setpriority, which, who, value, 0, 0, 0);
}
int
nice(int increment)
{
...
{
	return (int)call(ZEDBSD_SYS_getrusage, who, (uintptr_t)usage, 0, 0, 0,
			 0);
}
int
getitimer(int which, struct itimerval *value)
{
	return (int)call(ZEDBSD_SYS_getitimer, which, (uintptr_t)value, 0, 0, 0,
			 0);
}
int
setitimer(int which, const struct itimerval *value, struct itimerval *old)
{
	return (int)call(ZEDBSD_SYS_setitimer, which, (uintptr_t)value,
			 (uintptr_t)old, 0, 0, 0);
}
int
unlink(const char *path)
{
	return (int)call(ZEDBSD_SYS_unlink, (uintptr_t)path, 0, 0, 0, 0, 0);
}
int
unlinkat(int dirfd, const char *path, int flags)
{
	return (int)call(ZEDBSD_SYS_unlinkat, dirfd, (uintptr_t)path, flags, 0,
			 0, 0);
}
int
rmdir(const char *path)
{
	return (int)call(ZEDBSD_SYS_rmdir, (uintptr_t)path, 0, 0, 0, 0, 0);
}
int
rename(const char *oldpath, const char *newpath)
{
	return (int)call(ZEDBSD_SYS_rename, (uintptr_t)oldpath,
			 (uintptr_t)newpath, 0, 0, 0, 0);
}
int
renameat(int olddirfd, const char *oldpath, int newdirfd, const char *newpath)
{
	return (int)call(ZEDBSD_SYS_renameat, olddirfd, (uintptr_t)oldpath,
...
linkat(int olddirfd, const char *oldpath, int newdirfd, const char *newpath,
       int flags)
{
	return (int)call(ZEDBSD_SYS_linkat, olddirfd, (uintptr_t)oldpath,
...
{
	return (int)call(ZEDBSD_SYS_symlinkat, (uintptr_t)target, dirfd,
...
ssize_t
readlinkat(int dirfd, const char *path, char *buffer, size_t size)
{
	return (ssize_t)call(ZEDBSD_SYS_readlinkat, dirfd, (uintptr_t)path,
...
{
	return (int)call(ZEDBSD_SYS_truncate, (uintptr_t)path, length, 0, 0, 0,
			 0);
}
int
ftruncate(int fd, off_t length)
{
	return (int)call(ZEDBSD_SYS_ftruncate, fd, length, 0, 0, 0, 0);
}
mode_t
umask(mode_t mask)
{
	return (mode_t)call(ZEDBSD_SYS_umask, mask, 0, 0, 0, 0, 0);
}
int
clock_gettime(clockid_t id, struct timespec *ts)
{
	return (int)call(ZEDBSD_SYS_clock_gettime, id, (uintptr_t)ts, 0, 0, 0,
			 0);
}
int
clock_getres(clockid_t id, struct timespec *ts)
{
	return (int)call(ZEDBSD_SYS_clock_getres, id, (uintptr_t)ts, 0, 0, 0,
			 0);
}
int
clock_settime(clockid_t id, const struct timespec *ts)
{
	return (int)call(ZEDBSD_SYS_clock_settime, id, (uintptr_t)ts, 0, 0, 0,
			 0);
}
...
int
aio_read(struct aiocb *control)
{
	return aio_submit(control, 0, 1);
}
int
aio_write(struct aiocb *control)
{
	return aio_submit(control, 1, 1);
}
...
int
lio_listio(int mode, struct aiocb *restrict const controls[restrict], int count,
...
{
	return (int)call(ZEDBSD_SYS_mount, (uintptr_t)type, (uintptr_t)dir,
			 flags, (uintptr_t)data, 0, 0);
}
int
unmount(const char *dir, int flags)
{
	return (int)call(ZEDBSD_SYS_unmount, (uintptr_t)dir, flags, 0, 0, 0, 0);
}
int
statvfs(const char *path, struct statvfs *status)
{
	return (int)call(ZEDBSD_SYS_statvfs, (uintptr_t)path, (uintptr_t)status,
			 0, 0, 0, 0);
}
int
fstatvfs(int fd, struct statvfs *status)
{
	return (int)call(ZEDBSD_SYS_fstatvfs, fd, (uintptr_t)status, 0, 0, 0,
			 0);
}
int
quotactl(const char *path, struct quota_control *request)
{
	return (int)call(ZEDBSD_SYS_quotactl, (uintptr_t)path,
			 (uintptr_t)request, 0, 0, 0, 0);
}
int
snapshotctl(const char *path, struct snapshot_control *request)
{
	return (int)call(ZEDBSD_SYS_snapshotctl, (uintptr_t)path,
			 (uintptr_t)request, 0, 0, 0, 0);
}
ssize_t
getxattr(const char *path, const char *name, void *value, size_t size)
{
	return (ssize_t)call(ZEDBSD_SYS_getxattr, (uintptr_t)path,
			     (uintptr_t)name, (uintptr_t)value, size, 0, 0);
}
ssize_t
lgetxattr(const char *path, const char *name, void *value, size_t size)
{
	return (ssize_t)call(ZEDBSD_SYS_lgetxattr, (uintptr_t)path,
			     (uintptr_t)name, (uintptr_t)value, size, 0, 0);
}
ssize_t
fgetxattr(int fd, const char *name, void *value, size_t size)
{
	return (ssize_t)call(ZEDBSD_SYS_fgetxattr, fd, (uintptr_t)name,
...
setxattr(const char *path, const char *name, const void *value, size_t size,
	 int flags)
{
	return (int)call(ZEDBSD_SYS_setxattr, (uintptr_t)path, (uintptr_t)name,
...
lsetxattr(const char *path, const char *name, const void *value, size_t size,
	  int flags)
{
	return (int)call(ZEDBSD_SYS_lsetxattr, (uintptr_t)path, (uintptr_t)name,
			 (uintptr_t)value, size, flags, 0);
}
int
fsetxattr(int fd, const char *name, const void *value, size_t size, int flags)
{
	return (int)call(ZEDBSD_SYS_fsetxattr, fd, (uintptr_t)name,
			 (uintptr_t)value, size, flags, 0);
}
ssize_t
listxattr(const char *path, char *list, size_t size)
{
	return (ssize_t)call(ZEDBSD_SYS_listxattr, (uintptr_t)path,
			     (uintptr_t)list, size, 0, 0, 0);
}
ssize_t
llistxattr(const char *path, char *list, size_t size)
{
	return (ssize_t)call(ZEDBSD_SYS_llistxattr, (uintptr_t)path,
			     (uintptr_t)list, size, 0, 0, 0);
}
ssize_t
flistxattr(int fd, char *list, size_t size)
{
	return (ssize_t)call(ZEDBSD_SYS_flistxattr, fd, (uintptr_t)list, size,
			     0, 0, 0);
}
int
removexattr(const char *path, const char *name)
{
	return (int)call(ZEDBSD_SYS_removexattr, (uintptr_t)path,
			 (uintptr_t)name, 0, 0, 0, 0);
}
int
lremovexattr(const char *path, const char *name)
{
	return (int)call(ZEDBSD_SYS_lremovexattr, (uintptr_t)path,
			 (uintptr_t)name, 0, 0, 0, 0);
}
int
fremovexattr(int fd, const char *name)
{
	return (int)call(ZEDBSD_SYS_fremovexattr, fd, (uintptr_t)name, 0, 0, 0,
			 0);
}
int
...
int
waitid(idtype_t type, id_t id, siginfo_t *information, int options)
{
	int result;
	cancel_point();
...
setrlimit(int resource, const struct rlimit *limit)
{
	...
	return (int)call(ZEDBSD_SYS_setrlimit, resource, (uintptr_t)&wire, 0, 0,
			 0, 0);
}
enum {
...
spawn_action_add(posix_spawn_file_actions_t *actions)
{
...
int
posix_spawn_file_actions_addclose(posix_spawn_file_actions_t *actions, int fd)
{
...
int
posix_spawn_file_actions_adddup2(posix_spawn_file_actions_t *actions, int fd,
				 int newfd)
{
	struct __spawn_action *a;
	if (fd < 0 || newfd < 0)
...
int
posix_spawn_file_actions_addopen(posix_spawn_file_actions_t *actions, int fd,
...
int
posix_spawnattr_getsigmask(const posix_spawnattr_t *attr, sigset_t *set)
{
...
int
posix_spawnattr_getsigdefault(const posix_spawnattr_t *attr, sigset_t *set)
{
	if (attr == NULL || set == NULL)
		return EINVAL;
	*set = attr->sigdefault;
...
static int
spawn_child_setup(const posix_spawn_file_actions_t *actions,
...
exec_search(const char *file, char *const argv[], char *const envp[])
{
	...
	if (strchr(file, '/') != NULL) {
		...
		return errno == ENOEXEC ? exec_with_shell(file, argv, envp)
					: -1;
	}
	path = getenv("PATH");
	if (path == NULL)
...
static int
exec_varargs(const char *path, const char *first, va_list arguments, int search,
...
getpid(void)
{
	return (pid_t)call(ZEDBSD_SYS_getpid, 0, 0, 0, 0, 0, 0);
}
int
sched_yield(void)
{
	return (int)call(ZEDBSD_SYS_sched_yield, 0, 0, 0, 0, 0, 0);
}
pid_t
getppid(void)
{
	return (pid_t)call(ZEDBSD_SYS_getppid, 0, 0, 0, 0, 0, 0);
}
pid_t
getpgrp(void)
{
	return (pid_t)call(ZEDBSD_SYS_getpgrp, 0, 0, 0, 0, 0, 0);
}
pid_t
getpgid(pid_t pid)
{
	return (pid_t)call(ZEDBSD_SYS_getpgid, pid, 0, 0, 0, 0, 0);
}
int
setpgid(pid_t pid, pid_t pgid)
{
	return (int)call(ZEDBSD_SYS_setpgid, pid, pgid, 0, 0, 0, 0);
}
pid_t
setsid(void)
{
	return (pid_t)call(ZEDBSD_SYS_setsid, 0, 0, 0, 0, 0, 0);
}
pid_t
getsid(pid_t pid)
{
	return (pid_t)call(ZEDBSD_SYS_getsid, pid, 0, 0, 0, 0, 0);
}
uid_t
getuid(void)
{
	return (uid_t)call(ZEDBSD_SYS_getuid, 0, 0, 0, 0, 0, 0);
}
int
getresuid(uid_t *real, uid_t *effective, uid_t *saved)
{
	return (int)call(ZEDBSD_SYS_getresuid, (uintptr_t)real,
			 (uintptr_t)effective, (uintptr_t)saved, 0, 0, 0);
}
uid_t
geteuid(void)
{
	return (uid_t)call(ZEDBSD_SYS_geteuid, 0, 0, 0, 0, 0, 0);
}
gid_t
getgid(void)
{
	return (gid_t)call(ZEDBSD_SYS_getgid, 0, 0, 0, 0, 0, 0);
}
int
getresgid(gid_t *real, gid_t *effective, gid_t *saved)
{
	return (int)call(ZEDBSD_SYS_getresgid, (uintptr_t)real,
			 (uintptr_t)effective, (uintptr_t)saved, 0, 0, 0);
}
gid_t
getegid(void)
{
	return (gid_t)call(ZEDBSD_SYS_getegid, 0, 0, 0, 0, 0, 0);
}
int
getgroups(int count, gid_t groups[])
{
	return (int)call(ZEDBSD_SYS_getgroups, count, (uintptr_t)groups, 0, 0,
			 0, 0);
}
int
setuid(uid_t id)
{
	return (int)call(ZEDBSD_SYS_setuid, id, 0, 0, 0, 0, 0);
}
int
seteuid(uid_t id)
{
	return (int)call(ZEDBSD_SYS_seteuid, id, 0, 0, 0, 0, 0);
}
int
setgid(gid_t id)
{
	return (int)call(ZEDBSD_SYS_setgid, id, 0, 0, 0, 0, 0);
}
int
setegid(gid_t id)
{
	return (int)call(ZEDBSD_SYS_setegid, id, 0, 0, 0, 0, 0);
}
int
setgroups(size_t count, const gid_t groups[])
{
	return (int)call(ZEDBSD_SYS_setgroups, count, (uintptr_t)groups, 0, 0,
			 0, 0);
}
int
setreuid(uid_t real, uid_t effective)
{
	return (int)call(ZEDBSD_SYS_setreuid, real, effective, 0, 0, 0, 0);
}
int
setresuid(uid_t real, uid_t effective, uid_t saved)
{
	return (int)call(ZEDBSD_SYS_setresuid, real, effective, saved, 0, 0, 0);
}
int
setregid(gid_t real, gid_t effective)
{
	return (int)call(ZEDBSD_SYS_setregid, real, effective, 0, 0, 0, 0);
}
int
setresgid(gid_t real, gid_t effective, gid_t saved)
{
	return (int)call(ZEDBSD_SYS_setresgid, real, effective, saved, 0, 0, 0);
}

int
stat(const char *path, struct stat *status)
{
	return (int)call(ZEDBSD_SYS_stat, (uintptr_t)path, (uintptr_t)status, 0,
			 0, 0, 0);
}
int
lstat(const char *path, struct stat *status)
{
	return (int)call(ZEDBSD_SYS_lstat, (uintptr_t)path, (uintptr_t)status,
			 0, 0, 0, 0);
}
int
fstatat(int dirfd, const char *path, struct stat *status, int flags)
{
	return (int)call(ZEDBSD_SYS_fstatat, dirfd, (uintptr_t)path,
			 (uintptr_t)status, flags, 0, 0);
}
int
access(const char *path, int mode)
{
	return (int)call(ZEDBSD_SYS_access, (uintptr_t)path, mode, 0, 0, 0, 0);
}
int
faccessat(int dirfd, const char *path, int mode, int flags)
{
	return (int)call(ZEDBSD_SYS_faccessat, dirfd, (uintptr_t)path, mode,
			 flags, 0, 0);
}
int
chmod(const char *path, mode_t mode)
{
	return (int)call(ZEDBSD_SYS_chmod, (uintptr_t)path, mode, 0, 0, 0, 0);
}
int
fchmod(int fd, mode_t mode)
{
	return (int)call(ZEDBSD_SYS_fchmod, fd, mode, 0, 0, 0, 0);
}
int
fchmodat(int dirfd, const char *path, mode_t mode, int flags)
{
	return (int)call(ZEDBSD_SYS_fchmodat, dirfd, (uintptr_t)path, mode,
			 flags, 0, 0);
}
int
chown(const char *path, uid_t uid, gid_t gid)
{
	return (int)call(ZEDBSD_SYS_chown, (uintptr_t)path, uid, gid, 0, 0, 0);
}
int
fchown(int fd, uid_t uid, gid_t gid)
{
	return (int)call(ZEDBSD_SYS_fchown, fd, uid, gid, 0, 0, 0);
}
int
lchown(const char *path, uid_t uid, gid_t gid)
{
	return (int)call(ZEDBSD_SYS_lchown, (uintptr_t)path, uid, gid, 0, 0, 0);
}
int
fchownat(int dirfd, const char *path, uid_t uid, gid_t gid, int flags)
{
	return (int)call(ZEDBSD_SYS_fchownat, dirfd, (uintptr_t)path, uid, gid,
			 flags, 0);
}
int
utimensat(int dirfd, const char *path, const struct timespec times[2],
	  int flags)
{
	return (int)call(ZEDBSD_SYS_utimensat, dirfd, (uintptr_t)path,
			 (uintptr_t)times, flags, 0, 0);
}
int
futimens(int fd, const struct timespec times[2])
{
	return (int)call(ZEDBSD_SYS_futimens, fd, (uintptr_t)times, 0, 0, 0, 0);
}
int
isatty(int fd)
{
...
int
getlogin_r(char *buffer, size_t size)
{
...
int
gethostname(char *buffer, size_t size)
{
	size_t length = size;
	if (buffer == NULL) {
		errno = EFAULT;
...
{
	return (int)call(ZEDBSD_SYS_getentropy, (uintptr_t)buffer, length, 0, 0,
			 0, 0);
}
int
...
int
ttyname_r(int fd, char *buffer, size_t size)
{
...
stream_read_direct(FILE *stream, void *buffer, size_t length)
{
	if (stream->cookie_read != NULL) {
		if (length > (size_t)INT_MAX)
			length = INT_MAX;
		return stream->cookie_read(stream->context, buffer,
					   (int)length);
	}
...
stream_flush_locked(FILE *stream)
{
	if (stream->last_operation == 2 && stream->buffer_length != 0) {
		...
		off_t unread =
		    (off_t)(stream->buffer_length - stream->buffer_start);
		off_t result =
	...
	while (done < total) {
		...
		if (stream->buffer_start < stream->buffer_length) {
			size_t available =
			    stream->buffer_length - stream->buffer_start;
			size_t take =
			    available < total - done ? available : total - done;
...
fwrite(const void *buffer, size_t size, size_t count, FILE *stream)
{
	...
	if (stream->buffering_mode == _IONBF) {
		...
		while (done < total) {
			size_t space =
			    stream->buffer_size - stream->buffer_length;
			size_t put =
			    space < total - done ? space : total - done;
...
timezone_rule_yday(int year, const struct timezone_rule *rule)
{
		       ...
		       (calendar_leap_year(year) && rule->first >= 60);
	{
		...
		if (rule->second == 5) {
			int next_month =
			    rule->first == 12 ? 1 : rule->first + 1;
			int next_year = rule->first == 12 ? year + 1 : year;
			int month_days =
			    (int)(days_from_civil(next_year,
...
{
	struct tm local;
	return localtime_r(value, &local) != NULL ? asctime_r(&local, buffer)
						  : NULL;
}
...
iso_weeks_in_year(int year)
{
	...
	if (weekday < 0)
		weekday += 7;
	return weekday == 4 || (weekday == 3 && calendar_leap_year(year)) ? 53
									  : 52;
}
	   ...
	   locale_t locale)
{
	...
	while (*format != '\0') {
		...
		case 'Y':
			...
			break;
		case 'z': {
			long z = value->tm_isdst > 0 ? timezone_daylight_east
						     : timezone_east;
			char b[8];
			snprintf(b, sizeof(b), "%c%02ld%02ld",
				 z < 0 ? '-' : '+', (z < 0 ? -z : z) / 3600,
				 ((z < 0 ? -z : z) % 3600) / 60);
			if (strftime_append(&output, &remaining, b))
				return 0;
			break;
		}
		case 'Z':
		...
		case 'g': {
			...
			iso_week(value, &y, &w);
			(void)w;
			if (strftime_number(&output, &remaining, y % 100, 2,
					    '0'))
				return 0;
			break;
		}
		case 'G': {
			...
			iso_week(value, &y, &w);
			(void)w;
			if (strftime_number(&output, &remaining, y, 4, '0'))
				return 0;
			break;
		}
		case 'R': {
			char b[8];
			snprintf(b, sizeof(b), "%02d:%02d", value->tm_hour,
				 value->tm_min);
			if (strftime_append(&output, &remaining, b))
				return 0;
			break;
		...
		case 'X': {
			char b[12];
			snprintf(b, sizeof(b), "%02d:%02d:%02d", value->tm_hour,
		...
		case 'r': {
			char b[16];
			int h = value->tm_hour % 12;
			if (!h)
				h = 12;
			snprintf(b, sizeof(b), "%02d:%02d:%02d %s", h,
				 value->tm_min, value->tm_sec,
				 value->tm_hour < 12 ? "AM" : "PM");
			if (strftime_append(&output, &remaining, b))
				return 0;
		...
		case 'c': {
			char b[32];
			if (strftime_l(b, sizeof(b), "%a %b %e %T %Y", value,
				       locale) == 0 ||
			    strftime_append(&output, &remaining, b))
				return 0;
			break;
		}
		case 'U':
		...
		case 'V': {
			...
			iso_week(value, &y, &w);
			(void)y;
			if (strftime_number(&output, &remaining, w, 2, '0'))
				return 0;
			break;
		}
		case 'W': {
			int wd = value->tm_wday ? value->tm_wday - 1 : 6;
			if (strftime_number(&output, &remaining,
...
strftime(char *restrict destination, size_t capacity,
	 const char *restrict format, const struct tm *restrict value)
{
	return strftime_l(destination, capacity, format, value,
			  LC_GLOBAL_LOCALE);
}
...
int
gettimeofday(struct timeval *result, void *timezone)
{
	struct timespec now;
	(void)timezone;
	if (result == NULL) {
...
static size_t
user_heap_grow(void *context, void *end, size_t minimum)
{
	size_t amount;
	void *old;
	(void)context;
