...
main(int argc, char **argv)
{
	...
	if (serial) {
		for (; i < argc; i++) {
			FILE *f =
			    !strcmp(argv[i], "-") ? stdin : fopen(argv[i], "r");
			char *l = NULL;
			size_t c = 0;
			long n;
			size_t di = 0;
		...
		int count = argc - i, j, active;
		FILE **fs = calloc((size_t)count, sizeof(*fs));
		char **ls = calloc((size_t)count, sizeof(*ls));
		size_t *cs = calloc((size_t)count, sizeof(*cs));
		if (!fs || !ls || !cs)
			return 1;
		for (j = 0; j < count; j++) {
		...
		do {
			...
			for (j = 0; j < count; j++) {
				long n =
				    command_read_line(fs[j], &ls[j], &cs[j]);
				if (n > 0) {
					active = 1;
					if (ls[j][n - 1] == '\n')
						n--;
