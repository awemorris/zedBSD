...
$(BUILD)/zedbsd-x68k.hd: $(BUILD)/stage1.bin $(BUILD)/stage2.bin \
	$(BUILD)/vmunix $(BUILD)/bin/sh tools/build/make-x68k-image.py \
	tools/build/check-x68k-image.py
	$(PYTHON) tools/build/make-x68k-image.py --force \
	...
	$(PYTHON) tools/build/check-x68k-image.py \
		--stage1 $(BUILD)/stage1.bin --stage2 $(BUILD)/stage2.bin \
		--kernel $(BUILD)/vmunix --shell $(BUILD)/bin/sh $@
...
ROOTFS_IMAGE_BUILD_TARGET := $(BUILD)/zedbsd-x68k.hd
ROOTFS_IMAGE_ARTIFACT := $(BUILD)/zedbsd-x68k.hd
BOOT_DISK_ARTIFACT := $(BUILD)/zedbsd-x68k.hd
...
x68k-image-check: $(BUILD)/zedbsd-x68k.hd
	...
	$(PYTHON) tests/x68k-image-host-test.py \
		--checker tools/build/check-x68k-image.py \
		--stage1 $(BUILD)/stage1.bin --stage2 $(BUILD)/stage2.bin \
		--kernel $(BUILD)/vmunix --shell $(BUILD)/bin/sh $<
