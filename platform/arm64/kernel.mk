...
ROOTFS_IMAGE_ARTIFACT := $(AARCH64_ARCH_UFS_IMAGE)
...
$(BUILD)/ufs-root-hdd-image.img: $(BUILD)/VMUNIX.A64 \
	...
	tools/build/make-rpi4-ufs-root-hdd-image.py tools/build/check-ufs1-image.py
	$(PYTHON) tools/build/make-rpi4-ufs-root-hdd-image.py --force \
...
$(BUILD)/vmunix: $(ARM64_VMUNIX_OBJS) $(ARM64_PLATFORM)/vmunix.ld \
	tools/build/check-arm64-vmunix.py
	$(ARM64_LD) --gc-sections -z max-page-size=4096 \
		-T $(ARM64_PLATFORM)/vmunix.ld -nostdlib $(ARM64_VMUNIX_OBJS) -o $@
	@test -z "$$($(ARM64_NM) -u $@)" || { $(ARM64_NM) -u $@; exit 1; }
	$(PYTHON) tools/build/check-arm64-vmunix.py --elf $@

$(BUILD)/VMUNIX.A64: $(BUILD)/vmunix tools/build/check-arm64-vmunix.py
	$(ARM64_OBJCOPY) -O binary $< $@
	$(PYTHON) tools/build/check-arm64-vmunix.py --elf $< --image $@ --fix-image
	$(PYTHON) tools/build/check-arm64-vmunix.py --elf $< --image $@
