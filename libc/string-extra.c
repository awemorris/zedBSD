...
char *
strtok(char *s, const char *separators)
{
	static char *next;
	char *end;
	if (s == NULL)
...
void *
memmem(const void *h, size_t hn, const void *n, size_t nn)
{
...
strcasestr(const char *h, const char *n)
{
	...
	if (!length)
		return (char *)h;
	for (; *h; h++)
		if (!strncasecmp(h, n, length))
			return (char *)h;
	return NULL;
}
char *
strndup(const char *s, size_t maximum)
{
	size_t length = strnlen(s, maximum);
...
char *
strnstr(const char *h, const char *n, size_t length)
{
	size_t nl = strlen(n);
	if (!nl)
		return (char *)h;
...
char *
strsep(char **sp, const char *delimiters)
{
	char *s, *end;
	if (sp == NULL || (s = *sp) == NULL)
		return NULL;
...
int
timingsafe_memcmp(const void *a, const void *b, size_t n)
{
	const unsigned char *x = a, *y = b;
...
ffs(int x)
{
	return first_set_word((unsigned int)x);
}
int
ffsl(long x)
{
	return first_set_word((unsigned long)x);
}
...
ffsll(long long x)
{
	...
	return value >> 32 ? first_set_word((unsigned int)(value >> 32)) + 32
			   : 0;
}
...
fls(int x)
{
	return last_set_word((unsigned int)x);
}
int
flsl(long x)
{
	return last_set_word((unsigned long)x);
}
...
void
strmode(unsigned int mode, char *p)
{
