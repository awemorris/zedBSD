...
このリポジトリのエージェント向けの指示。前半は [Awesome Plan](https://github.com/awemorris/AwesomePlan/blob/main/awesome-plan.md) の skill の全文
（固定版は [docs/agent/awesome-plan/awesome-plan.md](docs/agent/awesome-plan/awesome-plan.md)、upstream main と同一を 2026-09-24 に確認）、
後半はこのプロジェクトの規則である。
...
1. この文書、[plan/config.md](plan/config.md)、[plan/guardrail.md](plan/guardrail.md)、[plan/master.md](plan/master.md) を読む。
2. [plan/queue.md](plan/queue.md)（active Queue）、[Past Log](plan/history/index.md) の最新、選んだ WS・Phase を読む。関係の無い計画や履歴は読まない。
3. 作業ツリーの未 commit の変更を確かめ、他人の変更は保つ。

## 記録の置き場所

| 記録 | 場所 |
| --- | --- |
| Master（目的・Milestone・Focus・WS の一覧と優先順位・Tools・プロジェクト固有の情報） | `plan/master.md` |
| Queue（active） / Queue の履歴 / Past Log | `plan/queue.md` / `plan/history/queue-qNNN.md` / `plan/history/index.md` |
| WS / Phase | `plan/wsNNN/ws.md` / `plan/wsNNN/phaseNNN/phase.md`。ID は `wsNNN-pNNN` |
| Guardrail・コーディング規約・設計方針 | `plan/guardrail.md`・`plan/coding-style.md`・`plan/master-design-policy.md` |
| Future Work / Bug Board / Bug | `plan/future-work.md` / `plan/known-bugs.md` / `plan/bugs/BUG-NNN.md` |
| 回帰と観察の道具 | `plan/tools/`。追加したら `plan/master.md` の Tools 節に登録する |
| Phase に固有の試験・一時物 | `plan/wsNNN/tests/`・`plan/wsNNN/temp/`（temp は git に入れない） |

- WS は一つの具体的な到達目標を持つ。完了・終了した WS を再利用して別の目標を足さない。似た領域でもまとめず、別の WS を立てる。
- WS が完了したら ws.md を完了の形（Status: completed、結果、制限・移管、Phase の一覧）に書き直し、Phase のディレクトリと
  その WS だけの試験を削除する（git の履歴に残る）。今後も使う試験は `plan/tools/` に移して Tools 節に登録する。
- `plan/history/` は履歴であり、指示ではない。
...
- `.internal/` を読まない。
- 集約の `make check` を走らせない。Phase に意味のある絞った確認を行う。
- **HAL の API の変更（`include/hal/hal.h`・HAL の責務）には差分ごとの事前承認が要る。** `src/hal/` の実装の修正・補完・最適化は
...
- 新しいコードには [plan/coding-style.md](plan/coding-style.md) の全文を適用する。
- 外部 package（`userland/packages/`）はソースツリーへ取り込まず、tarball を取得・検証して patch する。ライセンスを監査する。

## GitHub

- 運用の mode は [plan/config.md](plan/config.md) に従い、同期の手順は [plan/tools/README.md](plan/tools/README.md)。
- このチェックアウトには同期の状態（`plan/.sync/`）が無く、2026-09-23 以降の記録は GitHub に未公開である。公開はユーザーの指示で行う。
  local だけの記録を同期済みと言わない。
