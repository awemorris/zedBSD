...
def check(path: Path, machine: str) -> None:
    ...
    elf_class, expected_machine, ehsize, phentsize = MACHINES[machine]
    if len(data) < ehsize or data[:4] != b"\x7fELF":
        fail(path, "not an ELF file")
    if data[4] != elf_class or data[5] != 1 or data[6] != 1:
        fail(path, f"expected current little-endian ELF{elf_class * 32}")
    header_format = ("<16sHHIIIIIHHHHHH" if elf_class == 1 else
                     "<16sHHIQQQIHHHHHH")
