...
SLOT_SECTORS = JOURNAL_BYTES // RECORD_BYTES
PROFILE_IDS = {"bin": b"BIN\0", "lib": b"LIB\0"}
...
def empty_active_slot(name: str, epoch: int = 1) -> bytes:
    """Return a complete 128-KiB slot with an empty valid snapshot."""
...
def validate_empty_active_slot(data: bytes, name: str) -> None:
    ...
    if header[:8] != b"ZOVLSLT\0" or _record_crc(header) != struct.unpack_from(
            "<I", header, 508)[0]:
        raise ValueError("invalid overlay slot header")
    version, header_size, record_size = struct.unpack_from("<HHI", header, 8)
    if (version, header_size, record_size) != (1, 48, RECORD_BYTES):
    ...
    epoch, count, commit_sector, sequence = struct.unpack_from(
        "<QIIQ", header, 0x18)
    if epoch != 1 or count != 0 or commit_sector != 1 or sequence != 0:
    ...
    if commit[:8] != b"ZOVLCMT\0" or _record_crc(commit) != struct.unpack_from(
            "<I", commit, 508)[0]:
