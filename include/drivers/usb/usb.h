...
struct drv_usb_hcd_ops {
	...
	/*
	 * Optional: tells the controller that a device is a hub with this many
	 * downstream ports, whether its transaction translators are one per
	 * port, and their think time (0..3, in units of 8 FS bit times), so
	 * that it can reach the devices below it.  Called by the core before
	 * any of those devices is enumerated.
	 */
	int (*hub_configure)(struct drv_usb_hcd *, struct drv_usb_device *, unsigned ports, unsigned multi_tt, unsigned think_time);
};
 ...
 * Hubs below the root.
 *
 * A hub class driver reads its hub's ports through these operations, in
 * the form of the hub class requests: port_status returns wPortStatus in
 * the low 16 bits and wPortChange in the high 16, clear_feature clears one
 * port feature (such as C_PORT_CONNECTION), and port_reset resets a port
 * and returns once it is enabled or has failed.
 */
struct drv_usb_hub_ops {
	int (*port_status)(void *context, unsigned port, uint32_t *status);
	int (*clear_feature)(void *context, unsigned port, uint16_t feature);
	int (*port_reset)(void *context, unsigned port);
};

/*
 * Makes a hub device's ports known to the core, tells the controller, and
 * enumerates what is already connected.  The operations and context stay
 * borrowed until the hub device is destroyed; the core destroys the
 * devices below a hub before it detaches the hub's own driver.
 *
 * This and drv_usb_hub_changed() only try to take the core's topology
 * lock: EBUSY means the core is busy (possibly destroying this hub), and
 * the caller retries later from its own thread.  ENODEV means the hub is
 * going away.  Neither may be called from a driver's attach or detach.
 */
int
...
/* Rescans a hub's ports after it reported a change. */
int
drv_usb_hub_changed(
	struct drv_usb_device *hub);
 ...
 * Reports ENOENT past the last device.
 */
