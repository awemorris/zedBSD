...
int vm_object_page_pin(struct vm_object_page *);
void vm_object_page_unpin(struct vm_object_page *);
int vm_object_page_pin_read(struct vm_object_page *, size_t, void *, size_t);
int vm_object_page_pin_write(struct vm_object_page *, size_t, const void *,
	size_t);
...
void vm_object_mapping_remove_locked(struct vm_object_page *,
	struct vm_page *);
...
int vm_object_inode_io_wait(struct inode *);
int vm_object_inode_resize_active(struct inode *);
int vm_object_content_read_begin(struct inode *);
void vm_object_content_read_end(struct inode *);
...
int vm_object_cache_published(struct inode *);
int vm_object_content_begin(struct file *, off_t, size_t,
	struct vm_object_resize *, struct vm_object_content *);
int vm_object_content_prepare(struct vm_object_content *);
void vm_object_content_commit(struct vm_object_content *, const void *,
	size_t);
void vm_object_content_abort(struct vm_object_content *);
/* Return ENOENT when no published cache exists; callers may then use the
 * backend while retaining i_io_lock. */
int vm_object_read_coherent(struct inode *, off_t, void *, size_t,
	ssize_t *);
int vm_object_resize_begin(struct inode *, off_t,
	struct vm_object_resize *);
int vm_object_resize_prepare(struct vm_object_resize *);
void vm_object_resize_commit(struct vm_object_resize *, off_t);
void vm_object_resize_abort(struct vm_object_resize *);
