...
struct thread {
	 ...
	 * kernel retirement request advances this before touching scheduler state,
	 * so a waiter can detect an interrupt delivered immediately before it
	 * publishes THREAD_SLEEPING. */
	 ...
	 * Protected by proc->lock; STOP redispatch may re-enter only for this owner. */
	tid_t join_owner_tid;
	/* Sticky pthread-cancellation request.  Wait registration can use a lock
	 * other than proc->lock, so readers outside that lock use atomic access. */
	volatile unsigned cancel_pending;
	unsigned terminate_requested;
	/* Published under proc->lock before scheduler retirement begins.  Process
	 * exit ownership counts this flag rather than racing THREAD_EXITING state,
	 * which belongs to the scheduler lock domain. */
	...
	/* Nonzero from a user-origin syscall/fault entry until the architecture is
	 * ready to return to user mode.  A depth, rather than a boolean, keeps the
	 * accounting contract valid across nested kernel execution. */
...
int thread_abort_new(struct thread *thread);
